...
| ----- | -------------- | ------------- | ---- |
| ① cross-graph | `head_cross_edges.bin` stitching the bundle nodes | existing `CrossEdges=1` + `CrossExtraEdges=N` build the sidecar atomically **before** STATIC Phase 4. Attribute-aware builds use local `NeighborhoodSize=20` and at least 32 cross edges per head; the native builder, launcher, and `augmentheadgraph` fallback clamp both candidate search and retained cross degree to 32. The builder reuses `BuildSSDIndex.NumberOfThreads`. `CrossEdges=0` skips it. Search-time kill switch: env `SPTAG_DISABLE_CROSS_EDGES=1`; filter queries skip cross edges unless `SPTAG_FILTER_KEEP_CROSS=1` | shared builder `HeadCrossEdgeBuilder.cpp`; build-time tail callback and runtime traversal in `SPANNIndex.cpp` |

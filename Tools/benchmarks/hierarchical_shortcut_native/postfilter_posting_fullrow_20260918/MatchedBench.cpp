...
bool ApplySearchIni(TenantIndexManager& p_manager, const std::string& p_path)
{
    ...
    for (const auto& parameter : parameters) {
        if (parameter.first=="postfilterpostingmode" || parameter.first=="postingactivationratio") continue;
...
Options ReadProtocol(const char* path)
{
    ...
    const std::set<std::string> allowedSearch{
        ...
        "postfilterpostingmode","postingactivationratio"};
    ...
    if (ShortcutFull::profile || ShortcutFull::capture ||
        out.caseName != "postfilter_"+ShortcutFull::mode+"_ratio_"+params.at("postingactivationratio"))
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (options.caseName != "h3") {
                    ...
                    operatorWork.push_back({n.outerExpansions,n.ordinaryNeighborEntries,n.ordinaryQualificationRequests});
                    ...
                    postingWork.push_back({n.sparseActivations,n.selectedActions,
                        n.secondHopLoops,n.classifierPrescanEntries,
                    ...
                    for (const auto& d:n.decisions) {
                        ...
                        put(i);put(d.head);put(d.d);put(d.e);put(d.complete);put(d.activated);

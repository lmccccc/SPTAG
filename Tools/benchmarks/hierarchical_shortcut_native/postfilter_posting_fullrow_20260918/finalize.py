...
def main():
    ...
    assert all(s["csr_partial_rows"]==0 for s in summary),"Incomplete selected CSR row"
    ...
    report={
        ...
        "full_row_correctness_correction":{
            "superseded_attempt":str(DATA/"comparisons/postfilter_posting_20260918"),
            "reason":"First attempt incorrectly applied the ordinary per-edge stop to selected CSR rows; four sparse captured rows were truncated. Its test/validator also incorrectly accepted truncation.",
            "fix":"Dedicated completeAuxiliaryRow flag only for an already-selected CSR row. Ordinary rows retain per-edge budget truncation. No new action can start after budget exhaustion.",
            "validation":"Every selected H1/H3 CSR row consumed==size; native budget-crossing fixture with diagnostics on/off; zero partial CSR rows in all corrected captures.",
            "no_parameter_change":True,"first_attempt_preserved":True},

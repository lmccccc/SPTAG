                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    const auto qualifies = [&](SizeType id) {
                ...
                bool completeAuxiliaryRow=false)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                if (observation) {
                    ++observation->d;
                    observation->e+=qualifies(targetLocal);
                    if (p_space.m_nativeHooks->diagnostics)
                        ++p_space.m_nativeHooks->ordinaryQualificationRequests;
                }
                if (p_space.m_nativeHooks && p_space.m_nativeHooks->injected) {
    ...
    const auto expandPostfilter = [&](SizeType head,const SizeType* row,int width) {
        ...
        if (hook.diagnostics) hook.graphWork+=graph;
        if (decision.complete && decision.d>0 && double(decision.e)/decision.d<hook.activationRatio &&
    ...
    while (true)
    {
        ...
        if (!EnableCrossEdges && p_space.m_nativeHooks && p_space.m_nativeHooks->gather)
...
{
    if (hooks.gather) Postfilter::ValidateRatio(hooks.activationRatio);
    ...
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.gather) {

...
This is the full-row correctness correction of the unaccepted
`postfilter_posting_20260918` attempt. That attempt and its measurements remain
unchanged: four sparse CSR rows were incorrectly truncated at the ordinary
per-edge budget. This successor changes only that boundary and strengthens its
native fixture and capture validator; it does not change the activation ratio.
...
A row can start only with remaining budget; once selected it completes even if
MaxCheck is reached inside it. The next auxiliary action cannot start after
budget exhaustion. Ordinary graph rows retain the original per-edge boundary.
There is no arbitrary CSR member or distance cap.
...
```sh
python3 SPTAG/Tools/benchmarks/hierarchical_shortcut_native/postfilter_posting_fullrow_20260918/prepare.py --initialize
python3 SPTAG/Tools/benchmarks/hierarchical_shortcut_native/postfilter_posting_fullrow_20260918/build.py
python3 SPTAG/Tools/benchmarks/hierarchical_shortcut_native/postfilter_posting_fullrow_20260918/run.py
python3 SPTAG/Tools/benchmarks/hierarchical_shortcut_native/postfilter_posting_fullrow_20260918/finalize.py
```
...
in `datasets/sift1m_zipf200_sparse193_numeric/comparisons/postfilter_posting_fullrow_20260918`.

...
def run(config, resume=False):
    ...
    for case in cases:
        ...
        shared = {k: v for k, v in pair[0].items()
                  if k not in ("shortcutmode", "shortcutcap", "headnavigationmode",
                               "shortcuteffectivedegree", "shortcutedges")}
    ...
    for name in ("authentication.json", "integration.json", "scenario_integration.json",
                 "supplier_integration.json", "final_source_manifest.json", "configure.log", "build.log",
                 "supplier-tests-pass.log"):
        if not resume:
    ...
    protected.update((config.parent.parent / "supplier_native").rglob("*"))
    protected.update((config.parent.parent / "online_native").rglob("*"))
    protected.update((config.parent.parent / "degree16_native").rglob("*"))
    protected.update((config.parent.parent / "fullrow_initial_native").rglob("*"))
    protected.update((config.parent.parent / "fullrow_native").rglob("*"))
    ...
    def execute(scenario, case, mode, repeat, frozen=False):
        ...
        if frozen:
            ...
            payload = [{"heads": r["heads"], "results": r["results"],
                        ...
                        "qualifications": r["qualifications"], "degree_audits": r["degree_audits"],
                        "row_audits": r["row_audits"], "signature_rejected": r["signature_rejected"],
                        "predicate_rejected": r["predicate_rejected"],
                        "entry_ids": r["entry_ids"], "entry_scored": r["entry_scored"],
                        "startup_audits": r["startup_audits"], "discoveries": r["discoveries"],
            ...
            if mode == "profile":
                require(all(r["calls"] > 0 and not r["exhausted"]
                            and r["csr_distances"] <= r["calls"] for r in data),
                        "Actual native distance accounting failure")
                if case.startswith(("supplier", "control")):
                    require(all(r["calls"] == r["supplyGraph"] + r["supplyRouting"] +
                                r["supplyParent"] + r["supplyChild"]
                                for r in data), "Unified supplier callback sum mismatch")
                    require(all(r["supplyStarts"] == 1 and r["supplyCalls"] == r["supplyReturns"]
                                and r["supplyH2Rows"] == r["supplyH2Completed"]
                                and r["supplyH3Rows"] == r["supplyH3Completed"]
                                for r in data), "Native continuation/full-row failure")
    ...
    try:
        ...
        # Only the untouched H1 baseline retains this cross-predicate requirement.
        for case in ("h1",):
        ...
        for scenario in scenarios:
            usage[scenario] = {}
            for case in (c for c in cases if c.startswith(("supplier", "control"))):
                data = rows(result_dir(scenario, case, "profile", 1) / "queries.jsonl")
                usage[scenario][case] = {
                    "queries_with_degree_shortage": sum(r["supplyTriggers"] > 0 for r in data),
                    "queries_triggered": sum(r["supplyCalls"] > 0 for r in data),
                    "queries_using_h3": sum(r["supplyH3Rows"] > 0 for r in data),
                    "native_overshoot_queries": sum(r["supplyNativeOvershoot"] > 0 for r in data),
                    "native_maxcheck_reached_queries": sum(
                        r["graph_checked"] >= r["supplyNativeMaxCheck"] for r in data),
                    "queries_with_signature_rejects": sum(r["supplySignatureRejects"] > 0 for r in data)}
                usage[scenario][case].update({
                    "startup_calls": sum(r["supplyStartupCalls"] for r in data),
                    "startup_empty_before": sum(r["supplyStartupCalls"] and r["supplyStartupBefore"] == 0 for r in data),
                    "startup_empty_to_nonempty": sum(r["supplyStartupCalls"] and r["supplyStartupBefore"] == 0
                                                     and r["supplyStartupAfter"] > 0 for r in data),
                    "startup_blocked": sum(r["supplyStartupBlocked"] for r in data),
                    "zero_h1_pop_queries": sum(r["supplyPops"] == 0 for r in data)})
        ...
        write_json(output / "validation.json", {"original_h1_h3_heads_and_native_work_equal": True,
            ...
            "original_h1_spatial_traces_predicate_invariant": True,
            "supplier_predicate_invariant_trace_required": False,

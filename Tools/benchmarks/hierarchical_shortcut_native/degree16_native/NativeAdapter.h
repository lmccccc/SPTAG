...
template<class T, class Catalogs, class Postings, class Maps, class Admit, class Own, class Qualify>
void NativeSearch(SPTAG::BKT::Index<T>* bkt, SPTAG::COMMON::QueryResultSet<T>* result,
                  ...
                  Admit admission, Own own, Qualify qualify) {
    ...
    e.Reset(local.model, ShortcutFull::cap, ShortcutFull::mode == "supplier",
        ...
        [&](int id, float d) {
            own(id, &d);
            const bool admitted = admission(id);
            if (admitted) { ++e.stats.eligible; result->AddPoint(id, d); }
            return qualify(id, admitted);
        });
    e.degree = ShortcutFull::effectiveDegree;
    e.memberCap = ShortcutFull::memberLimit;
    ...
    if (s.starts != 1 || e.inHelper || s.calls != s.returns || s.members > e.memberCap ||
        s.maxQualified > static_cast<unsigned>(e.degree) || s.Used() > ShortcutFull::cap ||
    ...
    o.supplyH2Exhausted = s.h2Exhausted; o.supplyDistanceStops = s.distanceStops;
    o.supplyMemberStops = s.memberStops; o.supplyNativeStops = s.nativeStops;

...
def render(root):
    ...
    text = ["## Completed paired experiment", "",
            ...
            "Counted/uncounted ordered outputs and repeated same-predicate traces match. "
            "Native own heaps equal the exact nearest10 matches among **all** evaluated "
    ...
    text += ["", "### Native navigation and helper work by scenario", "",
             "Per-query means; upper work is supplier row choice, never an upper graph search. "
             "All parent distances also represent canonical H1 candidates offered to native admission.", "",
    ...
    keys = ["supplyGraph", "supplyParent", "supplyChild", "supplyCandidates", "supplyMembers",
            "supplyCalls", "supplyNeighbors", "supplyQualified", "supplyQueued", "supplyH2Rows", "supplyH3Rows"]
    for scenario in scenarios:
        for case in cases[2:]:
            w = rows[scenario, case]["supplier_work"]
            text.append("| " + scenario + "/" + case + " | " + " | ".join(f"{w[k]:.3f}" for k in keys) + " |")
    text += ["", "### Effective degree and completion", "",
             "Degrees average over native adjacency expansions, including partial final expansions. "
             "Fill/stop counts are means per query; H2 fills are a subset of all fills.", "",
    ...
    for scenario in scenarios:
        for case in cases[2:]:
            w = rows[scenario, case]["supplier_work"]
            degree_values = [w[k] / max(w["supplyExpansions"], 1) for k in
                             ["supplyRawBefore", "supplyEffectiveBefore", "supplyEffectiveAfter"]]
            values = degree_values + [w[k] for k in
                ["supplyTriggers", "supplyRequested", "supplyQualified", "supplyFills",
                 "supplyH2Fills", "supplyH2Exhausted", "supplyDistanceStops", "supplyMemberStops",
                 "supplyNativeStops", "supplyScopeStops"]]
            text.append("| " + scenario + "/" + case + " | " + " | ".join(f"{v:.3f}" for v in values) + " |")
    text += ["", "Effective degree among **started helper calls**, excluding later expansions where "
             "a shared ceiling prevents starting another helper:", "",
             "| Scenario/case | Effective before | Effective after | Requested per call | Qualified per call |",
             "|---|---:|---:|---:|---:|"]
    for audit in validation["audits"]:
        if not audit["case"].startswith("supplier"):
            continue
        values = [audit[k] for k in ["mean_effective_before_started_call",
                  "mean_effective_after_started_call", "mean_requested_per_started_call",
                  "mean_qualified_per_started_call"]]
        text.append("| " + audit["scenario"] + "/" + audit["case"] + " | " +
                    " | ".join("-" if v is None else f"{v:.3f}" for v in values) + " |")
    usage = json.loads((root / "supplier_usage.json").read_text())
    text += ["", "### Observed query counts (out of1000)", "",
             "| Scenario/case | Helper triggered | Used H3 | Distance cap exhausted | Member ceiling reached |",
             "|---|---:|---:|---:|---:|"]
    for scenario, cases_usage in usage.items():
        for case, value in cases_usage.items():
            text.append("| " + scenario + "/" + case + " | " + " | ".join(str(value[k]) for k in
                        ["queries_triggered", "queries_using_h3", "distance_cap_exhausted_queries",
                         "member_ceiling_queries"]) + " |")
    text += ["", "Caps are ceilings, not forced expenditure: the original checked-leaf limit and "
             ...
             "synchronously with at most the requested qualified deficit and2048 CSR entries shared "
             "across the entire query. Raw supplied neighbors can exceed16 when some are ineligible. "

...
void SupplierFixtures() {
    ...
    std::function<int(int)> qualification = [](int) { return 3; };
    auto reset = [&](int cap) {
        admitted.clear();
        e.Reset(m, cap, true, true, true,
                [&](int id) { return float((id - q) * (id - q)); },
                [&](int id, float) { Check(admitted.insert(id).second); return qualification(id); });
        e.memberCap = 2048;
    };
    reset(2000);
    ...
    auto offer = [&](int id) {
        ...
        e.Score(id, 2, false);
        if (enqueue) ++e.stats.queued;
        return 1 | (e.qualifications[id] ? 2 : 0);
    };
    const auto allowed = [] { return true; };
    e.Supply(0, 16, 16, offer, allowed);
    Check(e.stats.calls == 0 && visited.empty());
    e.Supply(0, 15, 15, offer, allowed);
    Check(e.stats.qualified == 1 && e.stats.neighbors == 1 && e.stats.requested == 1);
    Check(e.stats.h2Fills == 1 && e.stats.ascents == 0 && e.stats.h3Rows == 0);
    reset(2000); visited.clear(); enqueue = false;
    e.Supply(0, 15, 15, offer, allowed);
    Check(e.stats.qualified == 1 && e.stats.queued == 0 && e.stats.fills == 1);
    enqueue = true;
    qualification = [](int id) { return id == 5 ? 3 : 0; };
    reset(2000); visited.clear();
    e.Supply(0, 28, 15, offer, allowed);
    Check(e.stats.qualified == 1 && e.stats.neighbors == 6 && e.stats.queued == 6);
    Check(e.stats.h3Rows == 0);
    qualification = [](int id) { return id == 0 ? 1 : (id == 1 ? 2 : 0); };
    reset(2000); visited.clear();
    e.Supply(0, 30, 14, offer, allowed);
    Check(e.stats.qualified == 2 && e.stats.neighbors == 2 && e.stats.requested == 2);
    qualification = [](int id) { return id >= 32 ? 3 : 0; };
    reset(2000); visited.clear();
    for (int i = 0; i < 32; ++i) visited.insert(i);
    e.Supply(0, 0, 0, offer, allowed);
    Check(e.stats.qualified == 16 && e.stats.neighbors == 16 && e.stats.h3Rows > 0);
    Check(e.stats.h2Exhausted == 1 && e.stats.ascents == 1 && e.stats.h2Fills == 0);
    Check(e.stats.calls == e.stats.returns && !e.inHelper);
    Check(e.stats.members <= 2048 && e.stats.maxQualified <= 16 && e.stats.cache > 0);
    auto cursors = e.cursors;
    e.Supply(0, 0, 0, offer, allowed);
    for (int l = 0; l < 2; ++l)
        for (std::size_t i = 0; i < cursors[l].size(); ++i) Check(e.cursors[l][i] >= cursors[l][i]);
    Check(e.stats.members <= 2048 && e.stats.maxQualified <= 16 && e.stats.qualified == 32);
    qualification = [](int) { return 0; };
    reset(2000); visited.clear(); e.memberCap = 5;
    e.Supply(0, 0, 0, offer, allowed);
    Check(e.stats.members == 5 && e.stats.qualified == 0 && e.stats.memberStops == 1);
    Check(e.stats.h2Exhausted == 0 && e.stats.ascents == 0);
    reset(2); visited.clear();
    e.Supply(0, 0, 0, offer, allowed);
    Check(e.stats.Used() == 2 && e.stats.distanceStops == 1 && e.stats.ascents == 0);
    reset(2);
    ...
    Check(rejected);
    e.Supply(0, 0, 0, offer, allowed); Check(e.stats.calls == 0);
}
void NativeFixtures() {
...
int main() {
    ...
    std::cout << "PASS: degree15/16, exact deficit, posting OR own eligibility, duplicate/ineligible bridges, "
                 "exhausted H2 to H3, no ascent on work ceiling, quotas/cursors, "

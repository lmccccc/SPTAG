...
FIELDS += ("supplyRawBefore supplyEffectiveBefore supplyEffectiveAfter supplyTriggers supplyRequested "
           ...
           "supplyDistanceStops supplyMemberStops supplyNativeStops supplyScopeStops").split()
...
def hooks(t):
    t = sub(t, 'inline bool profile = false, capture = false;',
            ...
            'inline std::uint64_t memberLimit = 2048;')
    ...
    t = sub(t, 'p.first != "shortcutoutput" && p.first != "shortcutedges")',
            ...
            '            p.first != "shortcuteffectivedegree" && p.first != "shortcutmemberlimit")')
    ...
    if (mode == "supplier" || mode == "control") {
        ...
        memberLimit = Unsigned(get("shortcutmemberlimit"));
        if (effectiveDegree != 16 || memberLimit != 2048)
            throw std::runtime_error("Require preregistered effective16/member2048 controls");

...
template<class Parameters> void Configure(const Parameters& params) {
    ...
    mode=get("nativepostfiltermode");
    if (mode!="graph" && mode!="posting")
        throw std::runtime_error("Invalid NativePostfilterMode");
    const auto& text=get("postingfilterhitratio");
    std::size_t used=0;
    activationRatio=std::stod(text,&used);
    if (used!=text.size()) throw std::runtime_error("Invalid PostingFilterHitRatio");

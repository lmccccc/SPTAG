"""Fixed native observed ratio, four Broad and at most four sparse processes."""
import hashlib
import json
import os
...
OPERATORS=["outer_heads","ordinary_row_slots","existing_predicate_calls","existing_predicate_passes",
           "existing_predicate_fails","checks_zero_rows","auxiliary_predicate_calls"]
PAYLOADS=(".ids.i32",".dist.f32",".work.u64")
CONTROL_TRACES=(".native.u64",".head.i32",".head.f32",".own.i32")
TRACES=CONTROL_TRACES+(".posting.u64",".operators.u64",".decisions.f64",".rows.u64")
...
def rejection_tests():
    ...
    for value in ("nan","inf","-0.01","0.05","1","0.01junk"):
        changes["ratio_"+value]=baseline.replace("PostingFilterHitRatio=0.01","PostingFilterHitRatio="+value)
    for value in ("navix","observe","postingjunk"):
        changes["bad_mode_"+value]=baseline.replace("NativePostfilterMode=posting","NativePostfilterMode="+value)
    changes["missing_ratio"]=baseline.replace("PostingFilterHitRatio=0.01","")
    results=[]
...
def validate(directory,scenario,mode,count,seconds):
    ...
    o=matrix(prefix,".operators.u64","<u8",7)
    d=matrix(prefix,".decisions.f64","<f8",11)
    rows=matrix(prefix,".rows.u64","<u8",5)
    require(n.shape==(count,18) and a.shape==(count,14) and o.shape==(count,7),"Counter schema")
    ...
    if scenario=="unfilter":
        require(np.all(n==0) and np.all(a==0) and np.all(o==0) and
    ...
    if scenario=="broad_tag" and mode=="graph":
        oracle=PARENT/"broad_tag_postfilter_graph_ratio_0.01_r1/nprobe_24"
...
def main():
    ...
    write(OUTPUT/"measurement_plan.json",{
        "broad_points":["graph","posting"],"ratio":.01,"broad_ordinary_processes":4,
        "sparse_preflight_modes":["graph","posting"],"unfilter_preflight_modes":["graph","posting"],
    ...
    for scenario in ("extreme_tag","unfilter"):
        group=[invoke(f"small_{scenario}_{case(mode)}",scenario,mode,True) for mode in ("graph","posting")]
        if scenario=="unfilter": require(group[0]["payloads"]==group[1]["payloads"],"Unfilter parity")
        small.extend(group)
    sparse=small[:2]
    require(sparse[1]["action_mean"]["sparse_activations"]>0 and sparse[1]["native_mean"]["h2_rows"]>0,
    ...
    records=[];plan=[]
    for scenario,modes in (("broad_tag",["graph","posting"]),
    ...
    for scenario,mode in dict.fromkeys((r["scenario"],r["mode"]) for r in records):
        ...
        summary.append({
            ...
            **{k:pair[0][k] for k in ("native_mean","operator_mean","action_mean",

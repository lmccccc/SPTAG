...
def audit_queries(path, count, probe, case, truth, mask):
...
def run(config, resume):
    ...
    source_proof = json.loads((toolchain / "capacity_guard_provenance.json").read_text())
    ...
    if not resume:
        ...
        for name in ("capacity_guard_provenance.json", "configure.log", "build.log",
                     "parent_final_source_manifest.json", "parent_authentication.json",
                     "parent_supplier-tests-pass.log"):
    ...
    try:
                ...
                if probe == 24:
                    ...
                    if repeat == 1:
                        previous = audit_queries(prior_root / old["directory"] / "queries.jsonl", 1000, 24,
                                                 case, truth[scenario], masks[scenario])
                        require(record["validation"]["payload_sha256"] == previous["payload_sha256"],
                                f"Guard-only rebuild changed nprobe24 ordered results/work: {key}")
                if key in first:
        ...
        with (root / "summary.csv").open("w") as stream:
            ...
            writer.writeheader()
            writer.writerows({k: r[k] for k in scalar} for r in summary)
        thresholds = []
        for scenario in prereg["scenarios"]:
            for case in prereg["cases"]:
        ...
        write_json(root / "validation.json", {
            ...
            "latest_startup_24_ordered_payload_and_work_equal": True,
            "native_source_change_only_capacity_guard": True,

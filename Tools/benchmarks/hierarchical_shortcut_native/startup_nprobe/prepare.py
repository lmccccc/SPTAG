...
OLD = DATA / "toolchains/h1_startup_20260916"
NEW = DATA / "toolchains/h1_supplier_nprobe_20260916"
OUTPUT = DATA / "comparisons/h1_supplier_nprobe_20260916"
...
def main():
    ...
    manifest = json.loads((OLD / "final_source_manifest.json").read_text())
    for name, expected in manifest["source_sha256"].items():
        assert sha(OLD / "source" / name) == expected, name
    assert sha(OLD / "source/Release/spannaclbench") == manifest["binary_sha256"]
    ...
    hook = NEW / "source/AnnService/FullHooks.h"
    original = hook.read_text()
    updated = replace_once(original, 'get("internalresultnum") != "24"',
                           'Unsigned(get("internalresultnum")) < Unsigned(get("resultnum"))')
    updated = replace_once(updated, "Shortcut integration requires isolated unfiltered flat24 controls",
                           "Shortcut integration requires isolated native controls and capacity >= ResultNum")
    hook.write_text(updated)
    spann = NEW / "source/AnnService/src/Core/SPANN/SPANNIndex.cpp"
    spann.write_text(replace_once(spann.read_text(),
                                 "p_queryResults->GetResultNum() != 24)",
                                 "p_queryResults->GetResultNum() <= 0)"))
    changed = {name for name, digest in old_sources.items() if sha(NEW / "source" / name) != digest}
    assert changed == {"AnnService/FullHooks.h", "AnnService/src/Core/SPANN/SPANNIndex.cpp"}, changed
    for name in ("authentication.json", "integration.json", "scenario_integration.json",
                 "supplier_integration.json", "final_source_manifest.json", "supplier-tests-pass.log"):
        shutil.copy2(OLD / name, NEW / f"parent_{name}")
    write_json(NEW / "capacity_guard_provenance.json", {
        "parent_toolchain": str(OLD), "parent_binary_sha256": manifest["binary_sha256"],
        "parent_source_manifest_sha256": sha(OLD / "final_source_manifest.json"),
        "before": old_sources,
        "after": {name: sha(NEW / "source" / name) for name in old_sources},
        "changed_files": sorted(changed),
        "change": "Only relax two experimental exact-24 capacity guards: Configure requires "
                  "capacity >= ResultNum; SPANN benchmark branch requires positive actual capacity. "
                  "Native search, BKT, supplier, eligibility, signatures and row scans unchanged.",
    })
    source = HERE.parent

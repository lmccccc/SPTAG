...
bool ApplySearchIni(TenantIndexManager& p_manager, const std::string& p_path)
{
    ...
    for (const auto& parameter : parameters) {
        if (parameter.first=="visitedmatchmode" || parameter.first=="postingneighbormatchratio" ||
            parameter.first=="diagnosticadmission") continue;
...
Options ReadProtocol(const char* path)
{
    ...
    const std::set<std::string> allowedSearch{
        ...
        "visitedmatchmode","postingneighbormatchratio","diagnosticadmission"};

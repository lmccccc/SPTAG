...
BASE=DATA/"toolchains/h1_postfilter_predicate_order_20260918"
PARENT=DATA/"comparisons/h1_postfilter_predicate_order_20260918"
ORACLE=DATA/"comparisons/postfilter_native_switch_20260918"
...
FILES={
    ...
    **{name:"AnnService/"+name for name in ("NativeSupplier.h","PostingSupplier.h","FullHooks.h",
        "NativeNeighborHooks.h","NativeFunctionRef.h","PostingPolicy.h")}}
...
def case(mode): return f"visited_{mode}_ratio_0.01"
...
def protect_wrapper_sources():
    protection=json.loads((OUTPUT/"protection.json").read_text())
    for path in (ROOT/"SPTAG/Wrappers").rglob("*"):
        if path.is_file() and "__pycache__" not in path.parts:
            protection.setdefault(str(path),sha(path))
    write(OUTPUT/"protection.json",protection)
...
def initialize():
    ...
    checks={
        PARENT/"report.json":"dbd9b864a8afb9e82e605a5382696749f997ed7b483b16125514fae79f97bbf4",
    ...
    protection={}
    roots=[HERE.with_name("h1_postfilter_predicate_order_20260918"),BASE,PARENT,
    ...
    for p,h in json.loads((PARENT/"protection.json").read_text()).items():
        assert sha(p)==h,p
        protection[p]=h
    first=DATA/"comparisons/postfilter_posting_20260918"
    assert sha(first/"report.json")=="fe93ab371ae5742d4dba0d3a4b6e061f4fdd5d80f26df642cef735b6dedeb090"
    for p,h in json.loads((first/"milestone_manifest.json").read_text()).items():
        assert sha(p)==h,p
        protection[p]=h
    protection[str(first/"milestone_manifest.json")]=sha(first/"milestone_manifest.json")
    write(OUTPUT/"protection.json",protection)
    protect_inputs_and_modules()
    protect_wrapper_sources()
    shutil.copytree(BASE/"source",TOOL/"source",symlinks=True,
        ignore=shutil.ignore_patterns("Release","CMakeCache.txt","CMakeFiles","__pycache__", ".venv",
            "Qualification.h","configs","diagnostic_configs"))
    ...
    for scenario in ("broad_tag","extreme_tag","unfilter"):
        ...
        text=(HERE.with_name("postfilter_native_switch_20260918")/
            f"{folder}/{scenario}_native_graph_hitratio_0.01.ini").read_text()
        for mode in ("graph","match","posting"):
            value=text.replace("native_graph_hitratio_0.01",case(mode))
            value=value.replace("postfilter_native_switch_20260918",HERE.name)
            value=value.replace("NativePostfilterMode=graph",f"VisitedMatchMode={mode}")
            value=value.replace("PostingFilterHitRatio","PostingNeighborMatchRatio")
            name=f"{scenario}_{case(mode)}.ini"
            if scenario=="broad_tag": (configs/name).write_text(value)
            else: (diagnostics/name).write_text(value)
    for p in configs.glob("extreme_tag*.ini"): p.unlink()
    for folder in (configs,diagnostics):
        for p in folder.glob("*.ini"):
            text=p.read_text()
            if "DiagnosticAdmission=" not in text:
                p.write_text(text.replace("[SearchSweep]","DiagnosticAdmission=original\n\n[SearchSweep]"))
    write(OUTPUT/"preparation.json",{"base":str(BASE),"checks":{str(p):h for p,h in checks.items()},
        ...
        "corrects_unaccepted_first_attempt":str(first),
        "first_attempt_report_sha256":sha(first/"report.json"),
        "docs":"Main-owned GettingStart divergence explicitly excluded from protection; never edited here"})
    protect()

def install():
    assert not (OUTPUT/"milestone_manifest.json").exists(),"Sealed milestone; do not modify or rebuild"
    protect()

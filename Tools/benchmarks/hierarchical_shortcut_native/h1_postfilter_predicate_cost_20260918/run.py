"""Original predicate order: four Broad ordinary processes; sparse/unfilter32 only."""
import hashlib
import json
import os
...
OPERATORS=["outer_heads","ordinary_row_slots","existing_predicate_calls","existing_predicate_passes",
           "existing_predicate_fails","checks_zero_rows","auxiliary_predicate_calls","native_result_logical_predicate_checks"]
...
TRACES=TRAJECTORY+(".decisions.f64",".rows.u64",".match.u64",".storage.u64",".admission.u64")
...
def validate(directory,scenario,mode,count,seconds):
    ...
    admission=matrix(prefix,".admission.u64","<u8",4)
    require(admission.shape==(count,4),"H1 admission schema")
    require(np.all(admission[:,0]==admission[:,1]),"Nonmatching H1 output")
    require(np.all(admission[:,2]+admission[:,3]==o[:,7]),"Actual predicate versus reused-bit accounting")
    if mode=="graph": require(np.all(admission[:,3]==0),"Graph used a match bit")
    ...
    record={
        ...
        "h1_admission_mean":dict(zip(("selected_heads","matching_heads","result_predicate_evaluations",
            "result_predicate_bit_reads"),admission.mean(axis=0).tolist())),
    ...
    if scenario!="unfilter":
        require(np.array_equal(np.fromfile(str(prefix)+".own.i32",dtype="<i4"),np.zeros(count)),
                "Supplemental own collector active")
        require(np.all(n[:,15]==1) and np.all(n[:,16]==0),"H1 result-level postfilter is disabled")
    record["baseline_identity"]="native H1 result postfilter using Supports, without supplementary own heap"
...
def original_replays():
    original=TOOL/"harness/head-reference"
    reference=OUTPUT/"original_reference";reference.mkdir(exist_ok=True)
    records=[]
    for scenario in ("broad_tag","extreme_tag","unfilter"):
        text=(HERE/"diagnostic_configs"/f"{scenario}_{case('graph')}.ini").read_text()
        text=text.replace("Case="+case("graph"),"Case=h1_original")
        text=text.replace("VisitedMatchMode=graph\n","").replace("PostingNeighborMatchRatio=0.01\n","")
        cfg=reference/(scenario+".ini");cfg.write_text(text)
        directory=reference/scenario
        binary=TOOL/"harness/original-reference" if scenario=="unfilter" else original
        args=["--config",str(cfg)] if scenario=="unfilter" else [str(cfg)]
        command=["numactl","--cpunodebind=2","--membind=2",str(binary),*args]
        if directory.exists():
            require(json.loads((directory/"exit.json").read_text())["returncode"]==0,
                    "Never rerun a failed native reference silently")
            require(json.loads((directory/"command.json").read_text())["command"]==command,
                    "Reference command changed")
            seconds=None
        else:
            directory.mkdir()
            if scenario=="unfilter":
                seconds=process(command,directory,max_seconds=180)
            else:
                # H1-only references never open the SSD posting file.
                ...
                write(directory/"exit.json",{"returncode":result.returncode})
                require(result.returncode==0,"H1 reference failed")
                seconds=time.monotonic()-start
        current=invoke(f"small_{scenario}_{case('graph')}",scenario,"graph",True)
        if scenario=="unfilter":
            expected=hashes(directory/"nprobe_24",PAYLOADS)
            require(current["payloads"]==expected,"Original unfilter full-query output/work mismatch")
        else:
            expected={suffix:sha(directory/("head."+suffix)) for suffix in ("i32","f32")}
            for suffix,digest in expected.items():
                require(digest==sha(OUTPUT/current["label"]/("nprobe_24.head."+suffix)),
                        "Authenticated H1 result-postfilter trace mismatch")
        records.append({"scenario":scenario,"queries":32,"hashes":expected,"config_sha256":sha(cfg),
            "reference_binary_sha256":sha(binary),"process_seconds":seconds,
            "scope":"full unfilter IDs/distances/SSD" if scenario=="unfilter" else "native H1 postfiltered IDs/distances"})
    write(OUTPUT/"original_reference_parity.json",records)
...
def main():
    ...
    require(all(p["unmodified_original_callback_sequence_exact"] and
        p["original_and_graph_all_events_exact"] and
        p["bit_native_helper_inputs_values_admission_visited_distance_exact"] for p in proof),
        "Independent native ordered-call proof must pass before timing")
    diagnostic=HERE/"diagnostic_configs"/f"broad_tag_{case('graph')}.ini"
    if not diagnostic.exists():
        text=(HERE/"configs"/diagnostic.name).read_text()
        diagnostic.write_text(text.replace("Warmup=1000","Warmup=32").replace("MaxQueries=1000",
            "MaxQueries=32\nDiagnosticOnly=true"))
    freeze()
    rejection_tests()
    original_replays()
    write(OUTPUT/"measurement_plan.json",{
        "broad_points":["graph","match"],"ratio":.01,"broad_ordinary_processes":4,
    ...
    records=[];plan=[]
    for scenario,modes in (("broad_tag",["graph","match"]),):
    ...
    for scenario,mode in dict.fromkeys((r["scenario"],r["mode"]) for r in records):
        ...
        summary.append({
            ...
            **{k:pair[0][k] for k in ("native_mean","operator_mean","action_mean","match_mean","match_raw_totals","effective_degree_measured","h1_admission_mean",

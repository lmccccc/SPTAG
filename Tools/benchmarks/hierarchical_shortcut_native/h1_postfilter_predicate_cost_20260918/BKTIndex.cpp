                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    SizeType currentMatchID=-1;
    const bool* currentMatch=nullptr;
    const auto admitFilteredResult =
        [&](SizeType p_key,
            ...
            const bool shortcut=ShortcutFull::diagnosticAdmission!=ShortcutFull::DiagnosticAdmission::Original &&
                !preserveCollapsedStop && p_distance>p_query.worstDist();
            if (shortcut && ShortcutFull::diagnosticAdmission==ShortcutFull::DiagnosticAdmission::Shortcut)
                return false;
            if (activeOutcome) ++activeOutcome->checks;
            const bool reused=currentMatch && p_result==currentMatchID;
            const bool valid=reused ? *currentMatch : p_resultFilter(p_result);
            if (p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics) {
                ++p_space.m_nativeHooks->nativePredicateCalls;
                if (reused) ++p_space.m_nativeHooks->nativePredicateBitReads;
                else ++p_space.m_nativeHooks->nativePredicateEvaluations;
            }
    ...
    const auto auxiliaryAdmissible = [&](SizeType id) {
        return p_resultFilter && p_resultFilter(id);
    };
                ...
                bool completeAuxiliaryRow=false)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                if (matchEnabled) {
                    currentMatchID=targetLocal;
                    currentMatch=&visitedMatch.second;
                }
                if (p_resultFilter)
                {
                    const DimensionType
    ...
    int maxCheck, bool searchDeleted) const
{
    ...
    hooks.filteredAdmission = bool(resultFilter) || bool(traversalFilter);
    if (hooks.ownPoint || hooks.ownGain)
        throw std::runtime_error("Supplemental own admission is not supported");
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.posting && !hooks.matchEnabled) {

...
int main() {
    try {
        ...
        for(auto control:{ShortcutFull::DiagnosticAdmission::Shortcut,
                          ShortcutFull::DiagnosticAdmission::PredicateBeforeShortcut}) {
            ShortcutFull::diagnosticAdmission=control;
            auto diagnostic=observed(false);
            std::map<int,int> calls;
            const auto result=run(diagnostic,[&](int id){++calls[id];return true;},2048,1);
            CHECK(result==nearestOne && diagnostic.evaluated==nativeOrder.evaluated &&
                  diagnostic.allVisited==nativeOrder.allVisited);
            for(int i=0;i<127;++i)
                CHECK(calls[fresh[i]]==int(control==ShortcutFull::DiagnosticAdmission::PredicateBeforeShortcut));
            CHECK(diagnostic.nativePredicateEvaluations==
                (control==ShortcutFull::DiagnosticAdmission::Shortcut ? 1 : nativeOrder.nativePredicateEvaluations));
        }
        ShortcutFull::diagnosticAdmission=ShortcutFull::DiagnosticAdmission::Original;
        std::cout<<"PASS diagnostic A-prime executes actual predicate for all distant neighbors with unchanged navigation/results\n";

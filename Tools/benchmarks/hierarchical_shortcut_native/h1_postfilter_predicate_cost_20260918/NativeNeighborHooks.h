...
struct NativeNeighborHooks {
    ...
    bool matchEnabled=false,immutableSnapshot=false;
    ...
    std::uint64_t nativePredicateCalls=0;
    std::uint64_t nativePredicateEvaluations=0,nativePredicateBitReads=0;
    std::uint64_t auxiliaryPredicateCalls=0;
    std::uint64_t outerExpansions=0,ordinaryNeighborEntries=0;
    std::uint64_t visitedProbes=0,firstMatchEvaluations=0,reusedMatchReads=0,matchEvaluations=0;
    std::uint64_t rejectedAuxiliaryEvaluations=0,oracleEvaluations=0;
    std::uint64_t matchComponentCalls=0;
    std::uint64_t physicalDegree=0,effectiveDegree=0,eligibleVisited=0;

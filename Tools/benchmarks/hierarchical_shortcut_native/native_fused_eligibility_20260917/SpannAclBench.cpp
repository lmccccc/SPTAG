...
int Run(int argc, char** argv)
{
    ...
    std::cout << "NATIVE_OPERATION {\"operation\":\"load\",\"seconds\":"
    ...
    for (const auto& searchPoint : searchPoints) {
        ...
        const bool nativeH1 = searchPoint.parameters.at("headnavigationmode") == "H1Only";
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            std::vector<FilterCost::Counters> validationCosts, ordinaryCosts(measuredQueries);
            std::vector<std::pair<int, float>> validationResults;
            using NativeStats = decltype(VectorIndex::GetThreadLocalPostingScanStats());
            std::vector<NativeStats> validationStats, ordinaryStats(measuredQueries);
            std::vector<int> validationScanned, ordinaryScanned(measuredQueries);
            const auto captureStart = std::chrono::steady_clock::now();
            if (ShortcutFull::capture) {
                for (std::size_t i = 0; i < measuredQueries; ++i) {
                    auto r = search(options.measureOffset + i);
                    if (!r || ShortcutFull::Last().invoked != nativeH1)
                        throw std::runtime_error("Full-query validation search failed");
                    validationStats.push_back(VectorIndex::GetThreadLocalPostingScanStats());
                    validationScanned.push_back(r->GetScanned());
                    validation.push_back(std::move(ShortcutFull::Last()));
                    validationCosts.push_back(FilterCost::Last());
            ...
            const auto warmupFinish = std::chrono::steady_clock::now();
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (ShortcutFull::Last().invoked != nativeH1)
                    throw std::runtime_error("Unexpected native H1/hierarchy observation route");
                observations[i] = std::move(ShortcutFull::Last());
                ordinaryCosts[i] = FilterCost::Last();
                const auto postingStats = VectorIndex::GetThreadLocalPostingScanStats();
                ordinaryStats[i] = postingStats;
                ...
                ordinaryScanned[i] = result->GetScanned();
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (!validation.empty()) {
                    ...
                    if (a.calls != b.calls || a.parentDistances != b.parentDistances ||
                        ...
                        (nativeH1 && (!a.originalDistanceFunction || !b.originalDistanceFunction)) ||
                    ...
                    o = v;
                    const auto& x = validationStats[i];
                    const auto& y = ordinaryStats[i];
                    if (x.m_matchedPostings != y.m_matchedPostings)
                        std::cout << "NATIVE_CONTRIBUTION nprobe=" << searchPoint.nprobe << " query=" << i
                                  << " capture=" << x.m_matchedPostings << " ordinary=" << y.m_matchedPostings << "\n";
                    if (x.m_readPostings != y.m_readPostings ||
                        x.m_uniqueMatchedPostings != y.m_uniqueMatchedPostings ||
                        x.m_scannedVectors != y.m_scannedVectors || x.m_matchedVectors != y.m_matchedVectors ||
                        x.m_dedupSkippedVectors != y.m_dedupSkippedVectors ||
                        x.m_uniqueMatchedVectors != y.m_uniqueMatchedVectors ||
                        x.m_postingPageReads != y.m_postingPageReads ||
                        x.m_postingLogicalBytes != y.m_postingLogicalBytes ||
                        x.m_postingPhysicalBytes != y.m_postingPhysicalBytes ||
                        validationScanned[i] != ordinaryScanned[i])
                        throw std::runtime_error("Capture changed native SSD work");
                    for (int j = 0; j < options.topk; ++j) {
                ...
                const auto& costs = validation.empty() ? ordinaryCosts[i] : validationCosts[i];
                if (costs.exactPredicate != ordinaryCosts[i].exactPredicate ||
                    costs.numericPredicate != ordinaryCosts[i].numericPredicate ||
                    costs.observer != ordinaryCosts[i].observer)
                    throw std::runtime_error("Capture changed native predicate or observer branch");
                ...
                dump << "{\"query\":" << i
                     << ",\"filter_cost\":{\"exact_predicate\":" << costs.exactPredicate
                     << ",\"numeric_predicate\":" << costs.numericPredicate
                     << ",\"observer_installed\":" << costs.observer
                     << ",\"posting_calls\":" << costs.posting
                     << ",\"traversal_calls\":" << costs.traversal
                     << ",\"own_notifications\":" << costs.own
                     << ",\"exact_head_predicate_calls\":" << costs.exactHead
                     << ",\"degree_rows\":" << costs.degreeRows
                     << ",\"physical_members\":" << costs.physicalMembers
                     << ",\"qualification_calls\":" << costs.qualifications
                     << ",\"unique_qualification_ids\":" << costs.uniqueQualifications
                     << ",\"repeated_qualification_calls\":" << costs.repeatedQualifications
                     << ",\"fused_rows\":" << n.fusedRows
                     << ",\"fused_members\":" << n.fusedMembers
                     << ",\"degree_visited_members\":" << n.degreeVisited
                     << ",\"degree_fresh_members\":" << n.degreeFresh
                     << ",\"traversal_handoffs\":" << n.traversalReuse
                     << ",\"result_handoffs\":" << n.resultReuse
                     << ",\"short_row_proof_only_qualifications\":" << n.shortRowProofQualifications
                     << ",\"partial_rows_without_helper\":" << n.partialRows << "}"
                     << ",\"native_h1_hooks_invoked\":" << o.invoked
            ...
            dump.flush();
            if (!dump) throw std::runtime_error("Shortcut query result flush failed");
            const auto serializationFinish = std::chrono::steady_clock::now();
            const auto seconds = [](const auto& end, const auto& begin) {
                return std::chrono::duration<double>(end - begin).count();
            };
            std::cout << "NATIVE_OPERATION {\"operation\":\"batch\",\"nprobe\":" << searchPoint.nprobe
                      << ",\"capture_seconds\":" << seconds(warmupStart, captureStart)
                      << ",\"warmup_seconds\":" << seconds(warmupFinish, warmupStart)
                      << ",\"ordinary_seconds\":" << elapsed
                      << ",\"validation_serialization_seconds\":" << seconds(serializationFinish, finish)
                      << ",\"batch_seconds\":" << seconds(serializationFinish, captureStart) << "}\n";

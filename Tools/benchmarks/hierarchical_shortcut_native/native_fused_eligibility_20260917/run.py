...
class Experiment:
    ...
    def fixtures(self):
        ...
        for scenario in ("medium_tag", "extreme_tag", "numeric", "mixed_dnf"):
            ...
            actual = list(curve.rows(self.root / r["directory"] / r["native"]["capture_file"]))
            previous = list(curve.rows(Path(self.plan["BoundResults"]) /
                f"fixture_{scenario}/queries.jsonl.nprobe_24"))
            require(len(actual) == len(previous) == 8, "Truncated filtered fixture")
            ignored = {"filter_cost", "row_eligibility_evaluations", "degree_frames"}
            require(all({k: v for k, v in x.items() if k not in ignored} ==
                        {k: v for k, v in y.items() if k not in ignored}
                        for x, y in zip(actual, previous)), "Filtered native reference changed")
            for x, y in zip(actual, previous):
                old_frames = iter(y["degree_frames"])
                for frame in x["degree_frames"]:
                    require(any(frame == old for old in old_frames), "Changed completed degree/helper frame")
            records.append(r)

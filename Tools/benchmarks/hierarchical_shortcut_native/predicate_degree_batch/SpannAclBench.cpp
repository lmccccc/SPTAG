...
std::vector<SearchPoint> ReadSearchIni(const std::string& p_path)
{
    ...
    const auto probes = NativeBatch::Probes(probeField->second, NativeBatch::Positive(resultField->second));
...
int Run(int argc, char** argv)
{
    ...
    if (!options.searchSweepInis.empty() && defaults.size() > 1)
        throw std::runtime_error("Array --search-ini cannot also specify --search-sweep-ini");
    std::set<std::string> outputs;
    for (auto& point : searchPoints) {
        if (NativeBatch::Positive(point.parameters.at("resultnum")) != options.topk)

...
META = {"nprobe", "probe_position", "probe_count", "native_index_load_calls",
        "native_workspace_resets", "capture_file"}
...
def parse(logs, probes, count):
    ...
    for i, row in enumerate(results):
        require(row["probe_position"] == i and row["probe_count"] == len(probes) and
                row["native_index_load_calls"] == 1 and row["native_workspace_resets"] == i + 1,
...
class Experiment:
    def __init__(self):
        ...
        if not self.root.exists():
            ...
            write(self.root / "provenance.json", {
                ...
                "previous_scalar_protocol": "Completed and preserved; superseded for current batch timing, not relabeled incomplete.",
            })
        ...
        self.references = {(r["job"]["scenario"], r["job"]["case"], r["job"]["nprobe"]): r
        ...
        io = json.loads((self.previous / "plain_r1_unfilter_h1_p24/native.io.json").read_text())
    ...
    def sweep(self):
        ...
        write(self.root / "validation.json", {
            ...
            "timing_protocol": "Fresh load-once native arrays; no previous per-process timings spliced.",
        })

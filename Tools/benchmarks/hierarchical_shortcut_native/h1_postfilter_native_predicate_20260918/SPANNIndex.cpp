...
template <typename T> ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
{
    ...
    if (!usedHeadBundleSearch)
    {
        ...
        if (!m_metadataOnlyHeadStore) {
            ...
            ret = NativeReuse::Search(nativeBKT, p_queryResults, m_nativePostingModel.get(), m_secondLevelCatalogs,
                m_secondLevelPostings, [&](int level, int id) {
                    ...
                    return !useLimitedTagPure || limitedSupportMatchesPredicate(head);
                });
            if (ret != ErrorCode::Success) return ret;
        }
    }
    ...
    if (ShortcutFull::capture && !nativeDefaultAdmission) {
        ...
        for (int i = 0; i < p_queryResults->GetResultNum(); ++i) {
            const auto head=p_queryResults->GetResult(i)->VID;
            ShortcutFull::Last().heads.push_back(head);
            ShortcutFull::Last().distances.push_back(p_queryResults->GetResult(i)->Dist);
            if (head>=0) {
                ++ShortcutFull::Last().selectedHeads;
                const bool matching=!useLimitedTagPure || limitedSupportMatchesPredicate(head);
                ShortcutFull::Last().matchingHeads+=matching;
                if (!matching) throw std::runtime_error("H1 post-filter admitted a nonmatching routing head");
            }
        }
    }
    ...
    if (m_extraSearcher != nullptr)
    {
        ...
        if (ShortcutFull::capture && !nativeDefaultAdmission && headPointResults==nullptr) {
            for (int j=0;j<p_queryResults->GetResultNum();++j)
                if (p_queryResults->GetResult(j)->VID>=0)
                    ShortcutFull::Last().nativeSelectedHeadRecords.push_back(p_queryResults->GetResult(j)->VID);
        }
        if (headPointResults != nullptr)
        {
            p_queryResults->Reset();

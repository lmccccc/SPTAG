...
int main() {
    ...
    for (double threshold:{.05,.1,.5}) for (bool postingMode:{false,true}) {
        for (int degree:{20,21}) {
            ...
            auto h=observed();
            h.twoHopThreshold=threshold; h.postingThreshold=postingMode?.05:0;
            int calls=0;
            ...
            const bool posting=postingMode && double(validCount)/degree<.05;
        ...
        clear();
        auto h=observed();h.twoHopThreshold=threshold;h.postingThreshold=postingMode?.05:0;
    ...
    std::cout<<"PASS cutoffs .05/.1/.5: native graph and posting, exact equality and just below, "
        "sentinel d, visited-valid e, d0, .05 posting fallback two-hop, diagnostics/determinism\n";
    for (int e:{0,1,5,10}) {
        clear();

...
volatile sig_atomic_t active=0, samples=0, dropped=0;
...
void finish() {
    ...
    std::fprintf(output,"query_tid\t%d\npid\t%d\ninterval_ns\t250000\ncapacity\t%d\nsamples\t%d\ndropped\t%d\n",
        queryThread,getpid(),capacity,samples,dropped);
    std::fprintf(output,"query_thread_cpu_seconds\t%.9f\n",
        cpuFinish.tv_sec-cpuStart.tv_sec+(cpuFinish.tv_nsec-cpuStart.tv_nsec)*1e-9);
...
void clockBoundary(void* caller) {
    ...
    if (boundary==5) {
        ...
        itimerspec period{};
        period.it_value.tv_nsec=period.it_interval.tv_nsec=250000;
        active=1;

...
void ConnectivityFixtures() {
    ...
    m.BuildOwners();
    H1Supplier::Engine e;
    int checked = 0, eligibleMax = 63;
    bool signatures = true;
    const auto reset = [&] {
        checked = 0;
        e.Reset(m, true, true, true, [](int id) { return float(id * id); },
                [](int, float) {}, [&](int id) { return id <= eligibleMax ? 3 : 0; },
                [&](int, int) { return signatures; });
    };
    std::array<int, 32> ordinary;
    std::iota(ordinary.begin(), ordinary.end(), 1);
    for (int visitedCount : {0, 20, 32}) {
        reset();
        std::set<int> visited;
        for (int i = 0; i < visitedCount; ++i) visited.insert(ordinary[i]);
        {
            H1Supplier::Connectivity degree(e, 0, 0, ordinary.data(), 32, &checked);
            Check(degree.audit.before == 32);
            for (int id : ordinary) {
                if (!visited.insert(id).second) { ++e.stats.ordinaryVisited; continue; }
                ++e.stats.rawBefore; ++e.stats.effectiveBefore;
                e.Score(id, 0, true); ++checked;
            }
            e.Supply(0, 32 - visitedCount, degree.audit.before,
                     [](int) { throw std::runtime_error("Connected row invoked supplier"); return 0; },
                     [] { return true; });
            Check(e.stats.calls == 0 && e.stats.graph == static_cast<unsigned>(32 - visitedCount));
        }
        const auto& a = e.connectivityAudits.back();
        Check(a.before == 32 && a.after == 32 && a.calls == 0 &&
              a.visited == static_cast<unsigned>(visitedCount));
    }
    eligibleMax = 15; signatures = false; reset();
    {
        H1Supplier::Connectivity degree(e, 0, 0, ordinary.data(), 32, &checked);
        Check(degree.audit.ordinary.size() == 32 && degree.audit.before == 15);
        e.Supply(0, 0, degree.audit.before, [](int) { return 0; }, [] { return true; });
        Check(e.stats.calls == 1 && e.stats.Used() == 0 && e.stats.members == 0);
    }
    eligibleMax = 63; signatures = false; reset();
    int malformed[] = {0, 1, 1, 64, 2, -1, 3};
    {
        H1Supplier::Connectivity degree(e, 0, 0, malformed, 7, &checked);
        Check(degree.audit.before == 2 && degree.audit.ordinary == std::vector<int>({1, 2}) &&
              degree.audit.stopSlot == 5);
        e.Supply(0, 0, degree.audit.before, [](int) { return 0; }, [] { return true; });
        Check(e.stats.calls == 1 && e.stats.members == 0);
    }
    reset(); checked = 2048;
    {
        H1Supplier::Connectivity degree(e, 0, 0, ordinary.data(), 32, &checked);
        Check(degree.audit.before == 32 && degree.audit.checkedBefore == 2048);
        e.Supply(0, 0, degree.audit.before, [](int) { return 0; }, [] { return false; });
    ...
    {
        H1Supplier::Connectivity degree(e, 0, 0, ordinary.data(), 32, &checked);
        Check(degree.audit.before == 31 && degree.audit.terminator == -2);
    }
    reset();
    {
        // All supplied nodes are already query-visited: connectivity still grows.
        H1Supplier::Connectivity degree(e, 0, 0, ordinary.data(), 15, &checked);
        Check(degree.audit.before == 15);
        e.Supply(0, 0, degree.audit.before, [&](int id) {
            return degree.AddSupplied(id) ? 4 : 0;
        }, [] { return true; });
        Check(e.stats.connAdded == 16 && e.stats.qualified == 0 && e.stats.child == 0 &&
              e.stats.h2Completed == 1 && e.stats.h3Rows == 0 && e.stats.members == 32);
        Check(degree.audit.after == 31 && !degree.AddSupplied(16) &&
              !degree.AddSupplied(1) && !degree.AddSupplied(0));
        Check(!e.RowAllowed(1, static_cast<int>(e.rowAudits[0][1])));
    }
    Check(e.connectivityAudits.back().after == 31);
}
void NativeFixtures() {
...
int main() {
    ...
    std::cout << "PASS: 32 eligible ordinary neighbors with 0/20/32 visited never supply; "
                 "predicate-low, short, duplicate, self, invalid and sentinel rows; "

...
def verify(root):
    ...
    for point in summary:
        ...
        if point["case"] == "supplier":
            assert point["supplier_degree_semantics"] == "predicate_valid_neighbors"
            assert point["connectivity_evidence"] == pair[0]["validation"]["connectivity_evidence"]
        for r in pair:
    ...
    proof = json.loads((snapshot / "predicate_degree_provenance.json").read_text())
    assert set(proof["changed_files"]) == {
        "AnnService/FullHooks.h", "AnnService/NativeAdapter.h", "AnnService/Supplier.h",
        "AnnService/src/Core/BKT/BKTIndex.cpp", "Tools/benchmarks/SpannAclBench.cpp"}
    assert proof["supplier_degree_semantics"] == "predicate_valid_neighbors"
    ...
    destination.write_text(json.dumps({
        ...
        "supplier_summary_rows_with_corrected_semantics": 66,

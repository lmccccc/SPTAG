                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    Postfilter::Decision* ordinaryObservation=nullptr;
    const auto admitFilteredResult =
        [&](SizeType p_key,
            ...
            if (p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics)
                ++p_space.m_nativeHooks->nativePredicateCalls;
            if (ordinaryObservation) {
                ++ordinaryObservation->checks;
                ordinaryObservation->passes+=valid;
            } else if (p_space.m_nativeHooks && p_space.m_nativeHooks->injected &&
                       p_space.m_nativeHooks->diagnostics) {
                ++p_space.m_nativeHooks->auxiliaryPredicateCalls;
            }
    ...
    const auto auxiliaryAdmissible = [&](SizeType id) {
        auto* hook=p_space.m_nativeHooks;
        const auto match=[&](SizeType candidate) {
            if (!notDeleted(m_deletedID,candidate)) return false;
            if (hook && hook->diagnostics && hook->injected) ++hook->auxiliaryPredicateCalls;
            return (hook && hook->auxiliaryEligible ? hook->auxiliaryEligible(candidate) :
        ...
        const auto expandEdges =
            [&](SizeType currentLocal, int currentNode, const Index<T>* currentIndex,
                ...
                DimensionType p_end,
                bool p_crossEncoded,
                Postfilter::Work* work=nullptr,
                Postfilter::Decision* observation=nullptr,
                bool completeAuxiliaryRow=false)
        {
            for (DimensionType edge = p_begin;
                 edge < p_end; ++edge)
            {
                if (!completeAuxiliaryRow && p_space.m_iNumberOfCheckedLeaves >=
                    p_space.m_iMaxCheck)
                {
                    return false;
                }
                ...
                if (targetLocal < 0 || targetLocal >= targetIndex->m_pSamples.R())
                {
                    if (p_space.m_nativeHooks && p_space.m_nativeHooks->injected)
                        throw std::runtime_error("Invalid auxiliary H1 member");
                    continue;
                }
                if (work) ++work->members;
                if (p_space.m_nativeHooks && !p_space.m_nativeHooks->injected && p_space.m_nativeHooks->diagnostics)
                    ++p_space.m_nativeHooks->ordinaryNeighborEntries;
                const bool matchEnabled=p_space.nodeCheckStatus.MatchEnabled();
                if (!matchEnabled && p_space.m_nativeHooks && p_space.m_nativeHooks->injected) {
                    if (work) ++work->predicateChecks;
                    if (!auxiliaryAdmissible(targetLocal)) continue;
                }
                const auto visitedMatch=matchEnabled
                    ? p_space.CheckAndSetMatch(targetKey,p_space.m_nativeHooks->injected)
                    : std::make_pair(p_space.CheckAndSet(targetKey),false);
                if (observation) {
                    ++observation->degree;
                    if (matchEnabled) {
                        observation->eligible+=visitedMatch.second;
                        observation->eligibleVisited+=visitedMatch.first && visitedMatch.second;
                    }
                }
                const bool visited=visitedMatch.first;
                if (matchEnabled && p_space.m_nativeHooks->injected && !visitedMatch.second) continue;
    ...
    const auto expandPostfilter = [&](SizeType head,const SizeType* row,int width) {
        auto& hook=*p_space.m_nativeHooks;
        Postfilter::Decision decision;
        ...
        Postfilter::Work graph;
        ordinaryObservation=&decision;
        decision.complete=expandEdges(head,0,this,row,0,width,false,
            hook.diagnostics?&graph:nullptr,&decision);
        ordinaryObservation=nullptr;
        if (hook.diagnostics) {
            hook.graphWork+=graph;
            hook.ordinaryPredicateCalls+=decision.checks;
            hook.ordinaryPredicatePasses+=decision.passes;
            hook.checksZeroRows+=decision.checks==0;
            hook.physicalDegree+=decision.degree;
            hook.effectiveDegree+=decision.eligible;
            hook.eligibleVisited+=decision.eligibleVisited;
            if (hook.matchEnabled && decision.complete) {
                // Independent untimed full physical-row oracle; never feeds the gate.
                for (int edge=0;edge<width && row[edge]>=0;++edge) {
                    const auto id=row[edge];
                    if (id>=m_pSamples.R()) continue;
                    ++decision.oracleDegree;
                    ++hook.oracleEvaluations;
                    decision.oracleEligible+=auxiliaryAdmissible(id);
                }
                if (decision.oracleDegree!=decision.degree || decision.oracleEligible!=decision.eligible)
                    throw std::runtime_error("Visited full-row ratio differs from independent native oracle");
            }
        }
        if (hook.matchEnabled && decision.complete && decision.degree>0 && decision.eligible<hook.activationRatio*decision.degree &&
            p_space.m_iNumberOfCheckedLeaves<p_space.m_iMaxCheck && hook.posting) {
            decision.activated=1;
            if (hook.diagnostics) ++hook.sparseActivations;
            auto consume=[&](const std::uint32_t* ids,int count) {
                hook.injected=true;
                Postfilter::Work work;
                expandEdges(head,0,this,ids,0,count,false,&work,nullptr,true);
                hook.injected=false;
                return work;
            };
            auto within=[&] { return p_space.m_iNumberOfCheckedLeaves<p_space.m_iMaxCheck; };
            COMMON::NativeEdgeConsumer native{consume,within};
            hook.posting(head,native);
        }
    ...
    while (true)
    {
        ...
        if (p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics)
            ++p_space.m_nativeHooks->outerExpansions;
        if (!EnableCrossEdges && p_space.m_nativeHooks &&
            (p_space.m_nativeHooks->matchEnabled || p_space.m_nativeHooks->posting || p_space.m_nativeHooks->diagnostics))
            expandPostfilter(currentLocal,node,localEdgeCount);
        else {
            auto* hook=p_space.m_nativeHooks;
            expandEdges(currentLocal,currentNode,currentIndex,node,0,localEdgeCount,false,
                hook && hook->diagnostics?&hook->graphWork:nullptr);
        }
...
{
    ShortcutFull::SnapshotScope snapshot(hooks.matchEnabled);
    if (hooks.posting) Postfilter::ValidateRatio(hooks.activationRatio);
    ...
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.posting) {
    ...
    if (hooks.matchEnabled) {
        if (!hooks.immutableSnapshot || !resultFilter || traversalFilter)
            throw std::runtime_error("Visited match requires immutable H1 snapshot and unpruned navigation");
        const auto matchPhysical=[&](SizeType id) {
            if (id<0 || id>=GetNumSamples()) throw std::out_of_range("Invalid physical match ID");
            const auto match=[&](SizeType candidate) {
                if (hooks.diagnostics) ++hooks.matchComponentCalls;
                return (searchDeleted || !m_deletedID.Contains(candidate)) &&
                    (hooks.auxiliaryEligible ? hooks.auxiliaryEligible(candidate) : resultFilter(candidate));
            };
            if (match(id)) return true;
            const auto marker=m_pGraph[id][m_pGraph.m_iNeighborhoodSize-1];
            if (marker>=-1) return false;
        ...
        workspace->m_matchPredicate=matchPhysical;
        workspace->nodeCheckStatus.EnableNativeMatch(GetNumSamples());
    }

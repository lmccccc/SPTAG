...
void HashTests() {
    ...
    Storage::StorageDiagnostics stats;
    Storage::Diagnostics()=&stats;
    Table calibration;calibration.TrackNavigation();calibration.Init(64,0);
    calibration.EnableMatch(true);
    CHECK(!calibration.CheckAndSetMatch(0,[](int){return true;}).first);
    CHECK(!calibration.CheckAndSetMatch(MaxSize,[](int){return true;}).first);
    CHECK(calibration.CheckAndSetMatch(0,[](int){CHECK(false);return false;}).second);
    CHECK(stats.allocations==2 && stats.conversions==1 &&
          stats.allocatedBytes==calibration.Capacity()*12);
    calibration.DoubleSize();
    CHECK(stats.allocations==3 && stats.conversions==1);
    Storage::Diagnostics()=nullptr;stats={};
    std::cout<<"STORAGE_CALIBRATION init+promotion+growth allocations=3 conversions=1\n";
    Table packed;packed.TrackNavigation();packed.Init(1,0);packed.EnableNativeMatch(MaxSize);
    int calls=0;
    auto exact=[&](int id){++calls;return id%2==0;};
    for (int id=0;id<4096;++id) CHECK(!packed.CheckAndSetMatch(id,exact).first);
    CHECK(!packed.CheckAndSetMatch(MaxSize-1,exact).first);
    CHECK(packed.CheckAndSetMatch(MaxSize-1,exact).second && packed.Contains(MaxSize-1));
    CHECK(packed.SlotBytes()==4 && calls==4097);
    packed.EnableNativeMatch(MaxSize);
    CHECK(packed.CheckAndSetMatch(MaxSize-1,exact).first && calls==4097);
    packed.DoubleSize();
    for (int id=0;id<4096;++id) CHECK(packed.CheckAndSetMatch(id,exact).second==(id%2==0));
    CHECK(packed.CheckAndSetMatch(MaxSize-1,exact).first && calls==4097);
    error=false;
    try{packed.CheckAndSetMatch(MaxSize,exact);}catch(const std::out_of_range&){error=true;}
    CHECK(error && packed.SlotBytes()==4 && calls==4097);
    const void* allocation=packed.StorageAddress();
    Storage::Diagnostics()=&stats;
    for (int query=0;query<100;++query) {
        packed.ResetQuery();
        if (query%4!=0) packed.EnableNativeMatch(MaxSize);
        for (int id=0;id<128;++id) {
            if (query%4==0) CHECK(!packed.CheckAndSet(id));
            else {
                const auto r=packed.CheckAndSetMatch(id,[&](int key){return key%2==query%2;});
                CHECK(!r.first && r.second==(id%2==query%2));
                CHECK(packed.CheckAndSetMatch(id,[](int){CHECK(false);return false;}).first);
            }
        }
        CHECK(packed.StorageAddress()==allocation && packed.SlotBytes()==4 && packed.occupied()==128);
    }
    Storage::Diagnostics()=nullptr;
    CHECK(stats.allocations==0 && stats.conversions==0 && stats.allocatedBytes==0 && stats.resets==100);
    table.EnableMatch(true);
    CHECK(table.SlotBytes()==4); // Init above restored a fresh generic table.
    CHECK(!table.CheckAndSetMatch(MaxSize,[](int){return true;}).first);
    allocation=table.StorageAddress();table.TrackNavigation();stats={};
    Storage::Diagnostics()=&stats;
    for(int query=0;query<100;++query) {
        table.ResetQuery();
        table.EnableMatch(query%2);
        if(query%2) CHECK(table.CheckAndSetMatch(MaxSize,[](int){return true;}).second);
        else CHECK(!table.CheckAndSet(MaxSize));
        CHECK(table.Contains(MaxSize) && table.StorageAddress()==allocation && table.SlotBytes()==8);
    }
    Storage::Diagnostics()=nullptr;
    CHECK(stats.allocations==0 && stats.conversions==0);
    CHECK(workspace.resultCheckStatus.SlotBytes()==4);
    std::cout<<"PASS packed32 native count INT_MAX highest ID INT_MAX-1, invalid sentinel INT_MAX, masked collisions/two blocks/growth; generic full INT_MAX lazy wide fallback; 100 packed mixed queries +100 sticky-wide mixed queries zero allocations/conversions, stable addresses, result table compact\n";
}
...
int main() {
    ...
    using Storage=COMMON::OptHashPosVector;
    auto mixedQuery=[&](int mode) {
        if(mode==3) {
            Result result(data.data(),1);
            CHECK(index.SearchIndexWithMaxCheck(result,2048)==ErrorCode::Success);
        } else if(mode==2) {
            Hooks h;postingRun(true,false,2048,false,h);
        } else {
            auto h=observed(mode==1);h.capture=h.diagnostics=h.auditVisited=false;
            run(h,{target},2048);
        }
    };
    for(int mode=0;mode<4;++mode) mixedQuery(mode);
    auto& pool=COMMON::ThreadLocalWorkSpaceFactory<COMMON::WorkSpace>::m_workspace;
    CHECK(pool && pool->nodeCheckStatus.SlotBytes()==4 && pool->resultCheckStatus.SlotBytes()==4);
    const void* mixedAddress=pool->nodeCheckStatus.StorageAddress();
    Storage::StorageDiagnostics mixedStats;
    Storage::Diagnostics()=&mixedStats;
    for(int query=0;query<128;++query) {
        mixedQuery(query%4);
        CHECK(pool && pool->nodeCheckStatus.StorageAddress()==mixedAddress &&
              pool->nodeCheckStatus.SlotBytes()==4 && pool->resultCheckStatus.SlotBytes()==4);
    }
    Storage::Diagnostics()=nullptr;
    CHECK(mixedStats.allocations==0 && mixedStats.conversions==0 && mixedStats.allocatedBytes==0 &&
          mixedStats.resets>=128 && mixedStats.modeChanges>0);
    std::cout<<"STORAGE_MIXED actual_queries=128 graph=32 match=32 posting=32 unfilter=32 allocations="
             <<mixedStats.allocations<<" conversions="<<mixedStats.conversions<<" slot_bytes=4 result_slot_bytes=4 stable_address=1\n";
    ...
    auto switched=[&](bool other) {
        Hooks h=observed();h.capture=h.diagnostics=h.auditVisited=false;
        Result result(other?same.data():data.data(),4);
        const auto& source=other?collapsed:index;
        CHECK(source.SearchIndexWithNativeHooks(result,h,[](int id){return id%4==1;},{},2048)==ErrorCode::Success);
    };
    switched(false);switched(true);
    mixedStats={};mixedAddress=pool->nodeCheckStatus.StorageAddress();
    Storage::Diagnostics()=&mixedStats;
    for(int query=0;query<64;++query) {
        switched(query%2);
        CHECK(pool->nodeCheckStatus.StorageAddress()==mixedAddress);
    }
    Storage::Diagnostics()=nullptr;
    CHECK(mixedStats.allocations==0 && mixedStats.conversions==0);
    std::cout<<"STORAGE_INDEX_SWITCH actual_queries=64 allocations=0 conversions=0 stable_address=1\n";

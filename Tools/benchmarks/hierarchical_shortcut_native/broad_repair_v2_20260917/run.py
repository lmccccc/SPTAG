...
import time
import numpy as np
from prepare import HERE, DATA, TOOL, OUTPUT, BASE, MATCHED, PARENT, PARENT_OUT, sha, write
...
REPAIR = ["fused_rows", "fused_members", "degree_visited", "degree_fresh", "traversal_reuse",
          ...
          "repeated_qualifications", "qualification_cache_hits", "own_eligibility_reuse"]
...
def protect():
    ...
    require(sha(PARENT_OUT / "report.json") ==
        "f88e775beb79adeea96d32349ce22ce2957d6c151367e50acdfff9cefd85b1bd", "v1 report changed")
    for target, digest in json.loads((PARENT_OUT / "repair_sources.json").read_text()).items():
        require(sha(PARENT / "source" / target) == digest, "v1 source changed")
...
def freeze():
    ...
    binaries = {"bad": TOOL / "harness/matched-bad", "repair": TOOL / "harness/matched-repair",
                "v1": TOOL / "matched-v1", "original": TOOL / "matched-original", "h3": TOOL / "matched-current"}
...
def invoke(label, variant, config, binaries, diagnostic=False):
    ...
    if variant == "repair":
        repair = np.fromfile(str(prefix) + ".repair.u64", dtype="<u8").reshape(count, len(REPAIR))
...
def measure():
    ...
    records = []
    order = [("bad", "broad_bad24"), ("v1", "broad_bad24"), ("repair", "broad_bad24"),
    ...
    for repeat, variants in enumerate((("bad", "v1", "repair"), ("repair", "v1", "bad")), 1):
    ...
    for scenario in ("broad_tag", "unfilter"):
        variants = ("bad", "v1", "repair", "original", "h3") if scenario == "broad_tag" else ("bad", "v1", "repair")
        selected = {}
        for variant in variants:
        ...
        parity(selected["v1"], selected["repair"])

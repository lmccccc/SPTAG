...
class PostingSupplier {
    ...
    struct ParentState { float distance = -1; unsigned char status = 0; };
    std::array<std::vector<ParentState>, 2> state;
    std::vector<std::vector<std::pair<float, int>>> discovered;
    bool RowAllowed(int level, int id) {
        auto& s = state.at(level - 1).at(id).status;
    ...
    bool UpperPending(int id) const {
        ...
        return std::any_of(rows.begin(), rows.end(), [&](const auto& p) {
            return state[0][p.second].status != 2 && state[0][p.second].status != 3;
        });
    }
    float ParentDistance(int level, int id) {
        auto& cached = state.at(level - 1).at(id).distance;
    ...
    struct Ranked {
        std::array<std::pair<float, int>, 64> values;
        int size = 0;
        auto begin() const { return values.begin(); }
        auto end() const { return values.begin() + size; }
    };
    Ranked Rank(int level, const int* begin, const int* end) {
        if (end - begin > 64) throw std::runtime_error("Original eight-owner rank bound exceeded");
        std::array<int, 64> ids;
        auto last = std::copy(begin, end, ids.begin());
        std::sort(ids.begin(), last);
        last = std::unique(ids.begin(), last);
        Ranked ranked;
        for (auto p = ids.begin(); p != last; ++p) {
            const int id = *p;
            if (level == 2 && state[1].at(id).status == 2) {
                if (!UpperPending(id)) continue;
            } else if (!RowAllowed(level, id)) continue;
            ranked.values[ranked.size++] = {ParentDistance(level, id), id};
        }
        std::sort(ranked.values.begin(), ranked.values.begin() + ranked.size);
        return ranked;
    }
public:
    PostingSupplier(PostingModel& m, SPTAG::COMMON::NativeNeighborHooks& h,
                    SPTAG::COMMON::NativeFunctionRef<bool(int, int)> s,
                    SPTAG::COMMON::NativeFunctionRef<float(int, int)> d)
        : model(m), hooks(h), signature(s), distance(d),
          discovered(m.counts[2]) {
        state[0].resize(m.counts[1]);
        state[1].resize(m.counts[2]);
    }
    template<class Eligible>
    void Supply(int head, int deficit, SPTAG::COMMON::NativeDeficitIdentities& identities, Eligible eligible,
        ...
        const auto open = [&](int id) {
            ...
            if (native.consumeRow) {
                SPTAG::COMMON::NativeSupplyAccounting accounting{identities, added, frame, deficit};
                ...
                if (identities.NeedsQualification(child, added, deficit) && eligible(child)) {
                    identities.Accept(child, added, deficit);
                    ++added;
        ...
        for (auto parent : Rank(1, own.data(), own.data() + own.size())) {
            if (!native.withinBudget()) return;
            open(parent.second);
            if (added >= deficit) return;
        }
        if (!std::all_of(own.begin(), own.end(), [&](int id) {
                return state[0][id].status == 2 || state[0][id].status == 3;
            }) || !native.withinBudget()) return;
        std::array<int, 64> upper;
        auto upperEnd = upper.begin();
        for (int parent : own) {
            const auto& owners = model.owners[1][parent];
            upperEnd = std::copy(owners.begin(), owners.end(), upperEnd);
        }
        for (auto parent : Rank(2, upper.data(), upper.data() + (upperEnd - upper.begin()))) {
            if (!native.withinBudget()) return;
            const int id = parent.second;
            auto& choices = discovered[id];
            if (state[1][id].status != 2) {
                if (!RowAllowed(2, id)) continue;
                const auto row = model.children(2, id);
                choices.reserve(row.second - row.first);
                ++hooks.h3Rows;
                std::uint64_t consumed = 0;
                for (auto p = row.first; p != row.second; ++p) {
                    ++hooks.members; ++consumed;

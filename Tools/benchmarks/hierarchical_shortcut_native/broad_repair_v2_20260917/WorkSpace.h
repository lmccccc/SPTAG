...
namespace SPTAG
{
	...
	template <typename WorkSpaceType>
        ...
        protected:
            ...
            std::vector<unsigned char> m_nativeQualification;
            bool m_nativeQualificationActive = false;

            int NativeSlot(SizeType id) const {
                if (!m_hashTable || id < 0) return -1;
                const SizeType key = id + 1;
                for (int block = 0; block < (m_secondHash ? 2 : 1); ++block) {
                    unsigned slot = hash_func(static_cast<unsigned>(key), m_poolSize);
                    for (int loop = 0; loop < m_maxLoop; ++loop) {
                        const int offset = block * (m_poolSize + 1) + slot;
                        if (m_hashTable[offset] == key) return offset;
                        slot = hash_func2(slot, m_poolSize, loop);
                    }
                }
                return -1;
            }
        ...
        public:
            ...
            {
                m_nativeQualificationActive = false;
                m_nativeQualification.clear();
                int ex = 0;
                while (size != 0) {
                    ex++;
                    size >>= 1;
            ...
            void clear()
            {
                if (m_nativeQualificationActive)
                    std::fill(m_nativeQualification.begin(), m_nativeQualification.end(), 0);
                if (!m_secondHash)
                {
                    // Clear first block.
            ...
            void EnableNativeQualification(bool enabled) {
                m_nativeQualificationActive = enabled;
                if (enabled) m_nativeQualification.assign(2 * (m_poolSize + 1), 0);
            }
            std::size_t NativeQualificationSlots() const {
                return m_nativeQualificationActive ? m_nativeQualification.size() : 0;
            }
            bool GetNativeQualification(SizeType id, unsigned char& flags) const {
                if (!m_nativeQualificationActive) return false;
                const int slot = NativeSlot(id);
                if (slot < 0 || !(m_nativeQualification[slot] & 128)) return false;
                flags = m_nativeQualification[slot];
                return true;
            }
            void PutNativeQualification(SizeType id, unsigned char flags) {
                if (!m_nativeQualificationActive) return;
                const int slot = NativeSlot(id);
                if (slot >= 0) m_nativeQualification[slot] = flags;
            }
            ...
            {
                const int oldSlots = 2 * (m_poolSize + 1);
                ...
                auto oldTable = std::move(m_hashTable);
                m_hashTable.reset(new_hashTable);
                if (m_nativeQualificationActive) {
                    auto oldFlags = std::move(m_nativeQualification);
                    m_nativeQualification.assign(2 * (m_poolSize + 1), 0);
                    for (int i = 0; i < oldSlots; ++i)
                        if (oldTable[i] && (oldFlags[i] & 128))
                            PutNativeQualification(oldTable[i] - 1, oldFlags[i]);
                }
            }

...
struct NativeEligibility {
    ...
    bool ownKnown = false, ownAllowed = false;
    unsigned char Bits() const {
        return 128 | (traversal ? 1 : 0) | (posting ? 2 : 0) |
            (postingKnown ? 4 : 0) | (ownKnown ? 8 : 0) | (ownAllowed ? 16 : 0);
    }
    static NativeEligibility FromBits(unsigned char bits) {
        return {bool(bits & 1), bool(bits & 2), bool(bits & 4), bool(bits & 8), bool(bits & 16)};
    }
};
struct NativeSupplyAccounting;
struct NativeEdgeConsumer {
    NativeFunctionRef<void(int, bool)> consume;
    NativeFunctionRef<bool()> withinBudget;
    NativeFunctionRef<int()> checked;
    NativeFunctionRef<void(const std::uint32_t*, int, NativeSupplyAccounting&)> consumeRow;
};
...
struct NativeSupplyAccounting {
    NativeDeficitIdentities& identities;
    int& added;
...
struct NativeNeighborHooks {
    ...
    NativeFunctionRef<void(int, float, NativeEligibility*)> ownQualified;
    std::function<NativeEligibility(int)> qualify;
    NativeFunctionRef<void(int, const int*, int, bool, const NativeEdgeConsumer&)> expand;
    NativeFunctionRef<void(NativeDegreeFrame&, const int*, int, const NativeEdgeConsumer&)> completeOrdinary;
    bool pureQualification = false;
    std::uint64_t qualificationCacheHits = 0, ownEligibilityReuse = 0;

                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    const auto qualifyNative = [&](SizeType id) {
        auto* hook = p_space.m_nativeHooks;
        unsigned char flags;
        if (hook && hook->pureQualification && p_space.nodeCheckStatus.GetNativeQualification(id, flags)) {
            if (hook->capture) ++hook->qualificationCacheHits;
            return COMMON::NativeEligibility::FromBits(flags);
        }
        auto eligibility = hook && hook->qualify ? hook->qualify(id)
            : COMMON::NativeEligibility{!p_traversalFilter || p_traversalFilter(id), false, false};
        if (hook && hook->pureQualification)
            p_space.nodeCheckStatus.PutNativeQualification(id, eligibility.Bits());
        return eligibility;
    };
    const auto admitFilteredResult =
        [&](SizeType p_key,
            ...
            const COMMON::NativeEligibility* eligibility = nullptr) {
            if (!p_resultFilter || p_result < 0 ||
                p_space.CheckResultAndSet(p_key)) {
                return false;
            }
            auto local = eligibility ? *eligibility : COMMON::NativeEligibility{};
            auto* hook = p_space.m_nativeHooks;
            unsigned char flags;
            if (!eligibility && hook && hook->pureQualification &&
                p_space.nodeCheckStatus.GetNativeQualification(p_key, flags))
                local = COMMON::NativeEligibility::FromBits(flags);
            if (p_space.m_nativeHooks &&
                (p_space.m_nativeHooks->ownPoint || p_space.m_nativeHooks->ownQualified) &&
                notDeleted(p_index->m_deletedID, p_local) &&
                checkFilter(p_index->m_pMetadata, p_local, filterFunc)) {
                if (hook->ownQualified) {
                    if (hook->capture && local.ownKnown) ++hook->ownEligibilityReuse;
                    hook->ownQualified(p_result, p_distance, &local);
                    // An own-only observation must not invent a traversal/posting decision.
                    if (hook->pureQualification && local.postingKnown)
                        p_space.nodeCheckStatus.PutNativeQualification(p_key, local.Bits());
                } else hook->ownPoint(p_result, p_distance);
            }
            ...
            const bool known = local.postingKnown;
            if (known && p_space.m_nativeHooks && p_space.m_nativeHooks->capture)
                ++p_space.m_nativeHooks->resultReuse;
            return (known ? local.posting : p_resultFilter(p_result)) &&
                notDeleted(
    ...
    const auto admitCollapsedResults =
        [&](int p_nodeID,
            ...
            const COMMON::NativeEligibility* representativeEligibility = nullptr) {
            const DimensionType checkPosition =
                p_index->m_pGraph
                    .m_iNeighborhoodSize -
                1;
            ...
            do
            {
                ...
                COMMON::NativeEligibility aliasEligibility;
                const auto* eligibility = collapsedLocal == p_representative
                    ? representativeEligibility : nullptr;
                if constexpr (!EnableCrossEdges)
                {
                    if (p_traversalFilter && collapsedLocal != p_representative) {
                        if (p_space.m_nativeHooks && p_space.m_nativeHooks->qualify) {
                            aliasEligibility = qualifyNative(collapsedLocal);
                            eligibility = &aliasEligibility;
                            traversalAllowed = aliasEligibility.traversal;
                        } else traversalAllowed = p_traversalFilter(collapsedLocal);
                    }
                }
                    ...
                    if (collapsedKey >= 0)
                    {
                        admitted =
                            admitFilteredResult(
                                ...
                                collapsedLocal,
                                collapsedResult,
                                collapsedDistance, true, eligibility);
                    }
                }
                else if (
                    traversalAllowed &&
    ...
    struct DeferredDegree { int id, position; };
    std::vector<DeferredDegree> deferredDegree;
    if (p_space.m_nativeHooks && p_space.m_nativeHooks->completeOrdinary)
        deferredDegree.resize(m_pGraph.m_iNeighborhoodSize);
    int deferredCount = 0;
        const auto expandEdges =
            [&](SizeType currentLocal, int currentNode, const Index<T>* currentIndex,
                ...
                DimensionType p_end,
                bool p_crossEncoded,
                COMMON::NativeDegreeFrame* degree = nullptr,
                COMMON::NativeSupplyAccounting* supply = nullptr)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                {
                    if (degree) {
                        if (p_space.m_nativeHooks->capture) ++p_space.m_nativeHooks->partialRows;
                        return false;
                    }
                    break;
                }
                ...
                if (supply && (edgeValue < 0 || edgeValue >= currentIndex->m_pSamples.R()))
                ...
                auto* hook = p_space.m_nativeHooks;
                COMMON::NativeEligibility eligibility;
                bool qualified = false;
                if (supply) {
                    ++hook->members;
                    if (supply->identities.NeedsQualification(targetLocal, supply->added, supply->deficit)) {
                        eligibility = qualifyNative(targetLocal);
                        qualified = true;
                        if (hook->capture) ++hook->csrQualifications;
                        if (eligibility.traversal) {
                            supply->identities.Accept(targetLocal, supply->added, supply->deficit);
                            ++supply->added;
                            if (hook->capture) supply->frame.supplied.push_back(targetLocal);
                        }
                    }
                    if (targetLocal == currentLocal) continue;
                }
                const bool distinct = degree && targetLocal != currentLocal &&
                    std::find(p_edges + p_begin, p_edges + edge, targetLocal) == p_edges + edge;
                if (distinct) {
                    ++degree->physical;
                    if (hook->capture) ++hook->fusedMembers;
                }
                // Rejected candidates must still enter the original native visited set.
                const bool visited = p_space.CheckAndSet(targetKey);
                if (qualified && hook && hook->pureQualification)
                    p_space.nodeCheckStatus.PutNativeQualification(targetKey, eligibility.Bits());
                if (hook && hook->auditVisited) hook->visitedEdges.emplace_back(targetKey, visited);
                if (distinct && hook->capture) {
                    if (visited) ++hook->degreeVisited;
                    else ++hook->degreeFresh;
                }
                if (distinct) {
                    const bool needsDegree = degree->physical >= hook->minimumPhysicalDegree;
                    if (p_traversalFilter && (!visited || needsDegree)) {
                        eligibility = qualifyNative(targetLocal);
                        qualified = true;
                        if (hook->capture) ++hook->rowEligibilityEvaluations;
                    }
                    if (qualified || !p_traversalFilter) degree->eligible += eligibility.traversal;
                    else deferredDegree[deferredCount++] = {targetLocal, degree->physical - 1};
                    if (hook->capture) degree->ordinary.emplace_back(targetLocal,
                        qualified || !p_traversalFilter ? eligibility.traversal : false);
                    if (needsDegree && deferredCount) {
                        for (int i = 0; i < deferredCount; ++i) {
                            const auto pending = deferredDegree[i];
                            const bool allowed = qualifyNative(pending.id).traversal;
                            degree->eligible += allowed;
                            if (hook->capture) {
                                ++hook->rowEligibilityEvaluations;
                                degree->ordinary[pending.position].second = allowed;
                            }
                        }
                        deferredCount = 0;
                    }
                ...
                if (visited) continue;
                if (p_traversalFilter) {
                    if (qualified) {
                        if (hook->capture) ++hook->traversalReuse;
                        if (supply && hook->capture) ++hook->csrTraversalReuse;
                    } else if (hook && hook->qualify) {
                        eligibility = qualifyNative(targetLocal);
                        qualified = true;
                    } else eligibility.traversal = p_traversalFilter(targetLocal);
                    if (!eligibility.traversal) continue;
                }
                ...
                if (p_resultFilter)
                {
                    ...
                    if (targetCheckNode < -1)
                    {
                        ...
                        admitCollapsedResults(
                            ...
                            targetLocalToGlobal,
                            targetLocal,
                            routeDistance, qualified ? &eligibility : nullptr);
                    }
                    else
                    {
                        ...
                        admitFilteredResult(
                            ...
                            targetLocal,
                            targetResult,
                            routeDistance, false, qualified ? &eligibility : nullptr);
                    }
                }
            ...
            // Short rows need no visited-neighbor predicate work in ordinary execution.
            if (degree && p_space.m_nativeHooks->capture && deferredCount) {
                auto* hook = p_space.m_nativeHooks;
                for (int i = 0; i < deferredCount; ++i) {
                    const auto pending = deferredDegree[i];
                    const bool allowed = qualifyNative(pending.id).traversal;
                    ...
                    degree->ordinary[pending.position].second = allowed;
                    ++hook->rowEligibilityEvaluations;
                    ++hook->shortRowProofQualifications;
                }
            }
            return true;
        };
    ...
    const auto inject = [&](SizeType head, int nodeID, const Index<T>* index,
        ...
        auto consume = [&](int id, bool fullRow) {
        ...
        auto withinBudget = [&] { return p_space.m_iNumberOfCheckedLeaves < p_space.m_iMaxCheck; };
        auto checked = [&] { return p_space.m_iNumberOfCheckedLeaves; };
        auto consumeRow = [&](const std::uint32_t* members, int count,
                                COMMON::NativeSupplyAccounting& accounting) {
            finishInjectedRow = true;
            hook->injected = true;
            if (hook->capture) ++hook->batchRows;
            COMMON::PrefetchGraphNeighbors(members, count, [index](SizeType id) -> const void* {
                return id >= 0 && id < index->m_pSamples.R() ? index->m_pSamples[id] : nullptr;
            });
            expandEdges(head, nodeID, index, members, 0, count, false, nullptr, &accounting);
            hook->injected = false;
            finishInjectedRow = false;
        };
        COMMON::NativeEdgeConsumer native{consume, withinBudget, checked, consumeRow};
        if (degree) {
            degree->before = native.checked();
            hook->completeOrdinary(*degree, row, width, native);
        } else hook->expand(head, row, width, startup, native);
    };
    if (p_space.m_nativeHooks) {
        auto& hook = *p_space.m_nativeHooks;
        hook.entry = false;
    ...
    while (true)
    {
        ...
        COMMON::NativeDegreeFrame degree{currentLocal, 0, 0, 0, 0, 0, false, 0, {}, {}};
        auto* degreeHook = !EnableCrossEdges && p_space.m_nativeHooks &&
            p_space.m_nativeHooks->completeOrdinary ? &degree : nullptr;
        if (degreeHook && p_space.m_nativeHooks->capture) ++p_space.m_nativeHooks->fusedRows;
        deferredCount = 0;
        const bool complete = expandEdges(currentLocal, currentNode, currentIndex,
                                           node, 0, localEdgeCount, false, degreeHook);
        if constexpr (!EnableCrossEdges) {
            if (complete)
                inject(currentLocal, currentNode, currentIndex, node, localEdgeCount, false, degreeHook);
        }
    ...
    int maxCheck, bool searchDeleted) const
{
    ...
    hooks.filteredAdmission = bool(resultFilter) || bool(traversalFilter) || bool(hooks.ownPoint);
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.expand) {
    ...
    workspace->nodeCheckStatus.EnableNativeQualification(hooks.pureQualification);
    ...
    workspace->nodeCheckStatus.EnableNativeQualification(false);
    m_workSpaceFactory->ReturnWorkSpace(std::move(workspace));

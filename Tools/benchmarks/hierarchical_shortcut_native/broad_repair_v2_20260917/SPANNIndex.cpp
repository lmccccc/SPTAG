    ...
    const std::string& p_baseDir)
{
    ...
    if (m_secondLevelPostings.size() == 2) {
        ...
        model->BuildOwners();
        model->defaultVectorCount = m_versionMap.Count();
        model->allOwnHeadsValid = NativeReuse::AllHeadsOwnValid(model->counts[0], [&](int head) {
            const auto mapped = *m_vectorTranslateMap[head];
            return m_index->ContainSample(head) &&
                mapped < static_cast<std::uint64_t>(MaxSize) &&
                mapped < model->defaultVectorCount &&
                !m_versionMap.Deleted(static_cast<SizeType>(mapped));
        });
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
            "NATIVE_DEFAULT_CERT heads=%d own_all_live=%d immutable=%d vectors=%llu\n",
            model->counts[0], static_cast<int>(model->allOwnHeadsValid),
            static_cast<int>(IsLimitedTagMutationReadOnly()),
            static_cast<unsigned long long>(model->defaultVectorCount));
...
template <typename T> ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
{
    ...
    FilterCost::Last().exactPredicate = hasExactFilter;
    FilterCost::Last().numericPredicate = queryDNF != nullptr && queryDNF->HasNumericLiteral();
    ...
    const auto limitedHeadMatchesExactPredicate =
        [this, queryDNF, queryTags,
         numQueryTags](SizeType p_head) {
            FilterCost::Count(&FilterCost::Counters::exactHead);
    ...
    const bool nativeDefaultAdmission = NativeReuse::Enabled() &&
        m_nativePostingModel && !primaryHeadBypassRequested &&
        m_nativePostingModel->counts[0] == m_index->GetNumSamples() &&
        m_nativePostingModel->defaultVectorCount == m_versionMap.Count() &&
        m_index->GetNumDeleted() == 0 &&
        NativeReuse::DefaultAdmissionAllowed(hasExactFilter, IsLimitedTagMutationReadOnly(),
            m_nativePostingModel->allOwnHeadsValid, p_queryResults->GetResultNum(),
            p_query.GetResultNum(), ShortcutFull::retainedRatio, ShortcutFull::minBaseDegree);
    ...
    std::function<bool(SizeType, const float*, COMMON::NativeEligibility*)> admitQualifiedHeadPoint;
    bool invalidHeadPoint = false;
    if ((useHierarchyNavigation || (NativeReuse::Enabled() && !nativeDefaultAdmission)) && m_extraSearcher != nullptr)
    {
            ...
            (!hasExactFilter || m_limitedTagSupport.AttributeCount() > 0))
        {
            ...
            admitQualifiedHeadPoint = [&](SizeType head, const float* knownDistance,
                                          COMMON::NativeEligibility* eligibility) {
                FilterCost::Count(&FilterCost::Counters::own);
                ...
                if (hierarchyState->m_deduper.CheckAndSet(vid)) return false;
                if (eligibility && eligibility->ownKnown) {
                    if (!eligibility->ownAllowed) return false;
                } else if (m_versionMap.Deleted(vid)) {
                    if (eligibility) { eligibility->ownKnown = true; eligibility->ownAllowed = false; }
                    return false;
                }
                ...
                if (!eligibility || !eligibility->ownKnown) {
                    const bool allowed = !hasExactFilter || limitedHeadMatchesExactPredicate(head);
                    if (eligibility) { eligibility->ownKnown = true; eligibility->ownAllowed = allowed; }
            ...
            admitHeadPoint = [&](SizeType head, const float* distance) {
                return admitQualifiedHeadPoint(head, distance, nullptr);
            };
        }
    }
    ...
    if (!usedHeadBundleSearch)
    {
        ...
        if (!m_metadataOnlyHeadStore) {
            ...
            if (NativeReuse::Enabled() && !nativeDefaultAdmission && (!admitHeadPoint || !headPointResults))
            ...
            if (NativeReuse::Enabled() && !IsLimitedTagMutationReadOnly())
                throw std::runtime_error("Qualification reuse requires immutable native H/O metadata");
            const auto posting = [&](int head) {
                FilterCost::Count(&FilterCost::Counters::posting);
            ...
            const auto qualify = [&](int head) {
                FilterCost::Qualification(head);
                FilterCost::Count(&FilterCost::Counters::traversal);
                if (head < 0 || head >= m_vectorTranslateMap.R())
                    throw std::runtime_error("Invalid native canonical H1 head");
                if (posting(head)) return COMMON::NativeEligibility{true, true, true};
                const auto mapped = *m_vectorTranslateMap[head];
                if (mapped >= static_cast<std::uint64_t>(m_versionMap.Count()))
                    throw std::runtime_error("Invalid native canonical H1 point");
                const bool ownAllowed = !m_versionMap.Deleted(static_cast<SizeType>(mapped)) &&
                    (!hasExactFilter || limitedHeadMatchesExactPredicate(head));
                return COMMON::NativeEligibility{ownAllowed, false, true, true, ownAllowed};
            };
            const auto traversal = [&](int head) { return qualify(head).traversal; };
            auto ownQualified = [&](int head, float distance, COMMON::NativeEligibility* eligibility) {
                admitQualifiedHeadPoint(head, &distance, eligibility);
            };
            ...
            ret = NativeReuse::Search(nativeBKT, p_queryResults, m_nativePostingModel.get(), m_secondLevelCatalogs,
                ...
                }, nativeDefaultAdmission, qualify, ownQualified, IsLimitedTagMutationReadOnly());
            if (ret != ErrorCode::Success) return ret;
        }
    }

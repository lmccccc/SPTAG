...
Options ReadProtocol(const char* path)
{
...
#ifdef MATCHED_CURRENT
    if (out.caseName != "h1" && out.caseName != "h3" && out.caseName != "supplier")
    ...
    if (ShortcutFull::profile || ShortcutFull::capture ||
        ShortcutFull::mode != (out.caseName == "supplier" ? "supplier" : "ordinary"))
...
int Run(int argc, char** argv)
{
    ...
    for (int nprobe : options.probes) {
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            std::vector<std::array<std::uint64_t, 16>> repairWork;
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (options.caseName != "h3") {
...
#ifdef MATCHED_REPAIR
                    const auto& costs = FilterCost::Last();
                    repairWork.push_back({n.fusedRows, n.fusedMembers, n.degreeVisited, n.degreeFresh,
                        n.traversalReuse, n.resultReuse, n.partialRows, n.shortRowProofQualifications,
                        n.batchRows, n.csrQualifications, n.csrTraversalReuse,
                        costs.qualifications, costs.uniqueQualifications, costs.repeatedQualifications,
                        n.qualificationCacheHits, n.ownEligibilityReuse});
#endif
                    helperCalls += n.calls;
                    ...
                    ++captureQueries;
                    if (options.caseName == "supplier" && options.scenarioName == "unfilter" &&
                        (!n.defaultAdmission || n.filteredAdmission || n.rowEligibilityEvaluations ||

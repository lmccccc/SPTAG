...
def validate(directory,scenario,mode,t,count,seconds):
    ...
    record={"label":directory.name,"scenario":scenario,"mode":mode,"two_hop_threshold":None if t is None else float(t),
        "posting_threshold":None if mode=="postfilter" else .01,
    ...
    if mode=="postfilter":
        ...
        if mode=="posting":expected=np.where((positive[:,3]/positive[:,2]<float(t)) &
            (positive[:,3]/positive[:,2]<.01),4,expected)
        ...
        oracle=None
        if scenario=="unfilter":
            oracle=PARENT/f"small_{scenario}_{mode}_twohop_{t}_posting_0.05"/"nprobe_24"
        historical=PARENT/(f"broad_tag_{mode}_twohop_{t}_posting_0.05_r1" if count==1000 else
            f"small_{scenario}_{mode}_twohop_{t}_posting_0.05")/"nprobe_24"
        record["historical_comparison"]={"path":str(historical),"timing_paired":False,
            "payloads_equal":record["payloads"]==hashes(historical,PAYLOADS),
            "traces_equal":record["trace_hashes"]==hashes(historical,TRACES)}
...
def main():
    ...
    freeze()
    points=[("postfilter",None)]+[("posting",t) for t in THRESHOLDS]
    ...
    for mode,t in points:
        ...
        summary.append({"case":case(mode,t) if t else "native_result_only_postfilter_graph",
            "mode":mode,"two_hop_threshold":float(t) if t else None,"posting_threshold":.01 if t else None,

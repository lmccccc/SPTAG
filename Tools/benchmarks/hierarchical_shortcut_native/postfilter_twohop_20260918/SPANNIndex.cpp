    ...
    const std::string& p_baseDir)
{
    ...
    if (!ShortcutFull::ExplicitPostfilter() && m_secondLevelPostings.size() == 2) {
...
template <typename T> ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
{
    ...
    NativeReuse::QualificationLease qualificationLease(
        NativeReuse::Enabled() && !ShortcutFull::PostfilterOnly() && hasExactFilter,
        m_index->GetNumSamples(),ShortcutFull::capture);
    auto* qualification=qualificationLease.Get();
    const auto evaluateHeadExactPredicate =
    ...
    const auto limitedHeadMatchesExactPredicate = [&](SizeType head) {
        const auto evaluate=[&] { return evaluateHeadExactPredicate(head); };
        return qualification ? qualification->Exact(head,evaluate) : evaluate();
    };
    ...
    bool nativeSignatureReady=false;
    ...
    const bool nativeDefaultAdmission = NativeReuse::Enabled() && !hasExactFilter;
    ...
    if ((useHierarchyNavigation || (NativeReuse::Enabled() && !nativeDefaultAdmission)) && m_extraSearcher != nullptr)
    {
            ...
            (!hasExactFilter || m_limitedTagSupport.AttributeCount() > 0))
        {
            ...
            admitHeadPoint = [&](SizeType head, const float* knownDistance) {
                ...
                nativeOwnImproved=std::make_pair(distance,vid)<headPointResults->front();
    ...
    if (!usedHeadBundleSearch)
    {
        ...
        if (!m_metadataOnlyHeadStore) {
            ...
            const auto evaluatePosting = [&](int head) {
            ...
            const auto posting = [&](int head) {
                const auto evaluate=[&] { return evaluatePosting(head); };
                return qualification ? qualification->Posting(head,evaluate) : evaluate();
            };
            auto ownGain=[&](int head,float distance) {
                admitHeadPoint(head,&distance);
                return nativeOwnImproved;
            };
            if (NativeReuse::Enabled() && !ShortcutFull::ExplicitPostfilter() && !m_nativePostingModel)
            ...
            ret = NativeReuse::Search(nativeBKT, p_queryResults, m_nativePostingModel.get(), m_secondLevelCatalogs,
                m_secondLevelPostings, posting, ownGain, [&](int level, int id) {
                    if (!nativeSignatureReady) {
                        if (useLimitedTagPure)
                            hierarchyQuerySignature=BuildHierarchyQuerySignature(
                                limitedTagQueryValues,m_limitedTagSupport,m_secondLevelPostings);
                        nativeSignatureReady=true;
                    }
                    return NativeReuse::PostingMayMatch(hierarchyQuerySignature,
                                                       m_secondLevelPostings.at(level - 1), id);
                }, nativeDefaultAdmission, [&](int head) {
                    const auto live=[&] {
                        if (head<0 || head>=m_vectorTranslateMap.R()) return false;
                        const auto vid=*m_vectorTranslateMap[head];
                        return vid<static_cast<std::uint64_t>(m_versionMap.Count()) &&
                            !m_versionMap.Deleted(static_cast<SizeType>(vid));
                    };
                    if (qualification)
                        return qualification->Eligible(head,
                            [&] { return evaluatePosting(head); },
                            [&] { return evaluateHeadExactPredicate(head); },live);
                    return evaluatePosting(head) || (live() && evaluateHeadExactPredicate(head));
                });
            if (ret != ErrorCode::Success) return ret;
        }
    }
    if (qualification && ShortcutFull::capture)
        ShortcutFull::Last().native.qualification=qualification->counts;

...
#undef DefineVectorValueType
        ...
        public:
            ...
            {
                const auto skipLeaf = [&](const BKTNode& node) {
                ...
                for (char i = 0; i < m_iTreeNumber; i++) {
                    ...
                    if (node.childStart < 0) {
                        ...
                        p_space.m_SPTQueue.insert(NodeDistPair(m_pTreeStart[i], distanceToNode(node)));
                    } else if (m_bfs) {
                        float FactorQ = 1.1f;
                        int MaxBFSNodes = 100;
                        ...
                        for (SizeType begin = node.childStart; begin < node.childEnd; begin++) {
                            ...
                            float dist = distanceToNode(m_pTreeRoots[begin]);
                        ...
                        for (int level = 1; level <= m_bfs; level++) {
                            ...
                            while (!p_curr->empty()) {
                                ...
                                if (tnode.childStart < 0) {
                                    ...
                                    p_space.m_nativeHooks->ownPoint(tnode.centerid, tmp.distance);
                        ...
                        for (SizeType begin = node.childStart; begin < node.childEnd; begin++) {
                            if (skipLeaf(m_pTreeRoots[begin])) continue;
                            p_space.m_SPTQueue.insert(NodeDistPair(begin, distanceToNode(m_pTreeRoots[begin])));
                        }
                    }
                }
            }
                ...
                const std::function<bool(SizeType)>& p_graphFilter = nullptr) const
            {
                ...
                while (!p_space.m_SPTQueue.empty())
                {
                    ...
                    if (p_space.m_nativeHooks && p_graphFilter && p_space.m_iNumberOfCheckedLeaves >= p_limits) break;
                    ...
                    if (tnode.childStart < 0) {
                        ...
                        if (p_space.m_nativeHooks && p_graphFilter) {
                            if (tnode.childEnd >= 0 && !p_graphFilter(tnode.centerid)) {
                                if (!p_space.CheckAndSet(tnode.centerid))
                        ...
                        if (!p_space.CheckAndSet(tnode.centerid)) {
                            ...
                            if (!p_graphFilter || p_graphFilter(tnode.centerid))
                                { if (p_space.m_nativeHooks && p_space.m_nativeHooks->ownPoint)
                                    p_space.m_nativeHooks->ownPoint(tnode.centerid, bcell.distance);

...
bool ApplySearchIni(TenantIndexManager& p_manager, const std::string& p_path)
{
    ...
    for (const auto& parameter : parameters) {
        if (parameter.first.rfind("navix", 0) == 0) continue;
...
Options ReadProtocol(const char* path)
{
    ...
    const std::set<std::string> allowedSearch{
        ...
        "navixmode","navixcapture","navixpostingthreshold","navixtwohopthreshold"};
    ...
    if (ShortcutFull::profile || ShortcutFull::capture ||
        out.caseName != ShortcutFull::mode+"_twohop_"+params.at("navixtwohopthreshold")+
            "_posting_"+params.at("navixpostingthreshold"))
...
int Run(int argc, char** argv)
{
    ...
    for (int nprobe : options.probes) {
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            std::vector<std::array<std::uint64_t, 19>> navixWork;
            std::vector<std::array<std::uint64_t, 6>> qualificationWork;
            std::vector<std::array<std::uint64_t, 3>> operatorWork;
            std::vector<std::array<double,11>> decisions;
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (options.caseName != "h3") {
                    ...
                    qualificationWork.push_back(n.qualification);
                    operatorWork.push_back({n.outerExpansions,n.ordinaryNeighborEntries,n.qualificationChecks});
                    ...
                    navixWork.push_back({n.branches[0],n.branches[1],n.branches[2],n.branches[3],n.branches[4],
                        n.fallbacks,n.qualificationChecks,n.secondRows,n.intermediateDistances,
                    ...
                    for (const auto& d:n.decisions) {
                        ...
                        put(i);put(d.head);put(d.d);put(d.e);put(d.route);put(d.fallback);
            ...
            save(".own.i32", ownIDs);
            save(".navix.u64", navixWork);
            save(".operators.u64", operatorWork);
            save(".qualification.u64", qualificationWork);

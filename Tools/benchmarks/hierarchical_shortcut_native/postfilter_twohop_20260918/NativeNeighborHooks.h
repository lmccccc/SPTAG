...
struct NativeNeighborHooks {
    ...
    double twoHopThreshold=.5;
    bool postingEnabled=true;
    bool postfilterTwoHop=false;
    void ValidatePolicy() const {
        if (postfilterTwoHop) {
            Navix::ValidatePostfilterThresholds(twoHopThreshold,postingThreshold);
            if (postingEnabled) throw std::runtime_error("Postfilter two-hop disables posting");
        } else Navix::ValidateThresholds(twoHopThreshold,postingThreshold,postingEnabled);
    }
    int checked=0;
    ...
    std::array<std::uint64_t,6> qualification{};

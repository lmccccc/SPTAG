...
inline void ValidatePostfilterThresholds(double threshold,double posting,bool control=false) {
    if (!std::isfinite(posting) || posting!=0)
        throw std::runtime_error("Postfilter modes require posting threshold zero");
    if (!std::isfinite(threshold) || threshold<0 || threshold>1 || (control && threshold!=0))
        throw std::runtime_error("Invalid postfilter two-hop threshold");
}

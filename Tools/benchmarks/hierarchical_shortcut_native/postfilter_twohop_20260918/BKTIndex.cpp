                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    bool finishInjectedRow = false;
                ...
                Navix::Work* work=nullptr)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                if (!finishInjectedRow && p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics)
    ...
    const auto expandNavix = [&](SizeType head,const SizeType* row,int width) {
        ...
        auto route=Navix::RouteValidated(decision.d,decision.e,hook.twoHopThreshold,
            hook.postingThreshold,hook.postingEnabled);
        ...
        if (hook.postfilterTwoHop && (route==Navix::OneHop || route==Navix::NoNeighbors)) {
            expandEdges(head,0,this,row,0,width,false);
            publish();
            return;
        }
        ...
        const auto emit=[&](int id) {
            Navix::Work work;
            expandEdges(head,0,this,&id,0,1,false,&work);
            if (hook.diagnostics) hook.graphWork+=work;
        };
        if (route==Navix::Posting) {
            auto consume=[&](const std::uint32_t* ids,int count) {
                hook.injected=true; finishInjectedRow=true;
                COMMON::PrefetchGraphNeighbors(ids,count,[this](SizeType id)->const void* {
                    return id>=0 && id<m_pSamples.R()?m_pSamples[id]:nullptr;
                });
                Navix::Work work;
            ...
            route=Navix::GraphRouteValidated(decision.d,decision.e,hook.twoHopThreshold);
        ...
        const auto second=[&](int id,int* encounters) {
            ...
            COMMON::PrefetchGraphNeighbors(neighbors,m_pGraph.m_iNeighborhoodSize,
                [this](SizeType n)->const void* {
                    return n>=0 && n<m_pSamples.R()?m_pSamples[n]:nullptr;
                });
        ...
        if (route==Navix::OneHop) {
            ...
            std::priority_queue<std::pair<float,int>,std::vector<std::pair<float,int>>,
                                std::greater<std::pair<float,int>>> intermediates;
            ...
            for (int i=0;i<decision.d;++i) {
                ...
                if (valid[i]) emit(id);
                    ...
                    if (hook.diagnostics) ++hook.intermediateDistances;
                    intermediates.emplace(d,id);
                }
            }
            while (!intermediates.empty() && encounters<decision.d) {
                const int id=intermediates.top().second;
                intermediates.pop();
...
{
    if (hooks.navix) hooks.ValidatePolicy();

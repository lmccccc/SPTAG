"""Isolated explicit postfilter/two-hop milestone; old artifacts remain sealed."""
import hashlib
import json
...
BASE=DATA/"toolchains/navix_strict_thresholds_20260918"
PARENT=DATA/"comparisons/navix_strict_thresholds_20260918"
...
def case(mode,t): return f"{mode}_twohop_{t}_posting_0"
...
def initialize():
    ...
    checks={
        PARENT/"report.json":"180718603473f70216bd5ca507ffebb5eef94d08a6cffedb1a23404fc9a2b0b2",
        PARENT/"summary.json":"5100237018dbe2996a2114c8ad21f07a83a7b81fe6c93c1b3c17d69a19049cc4",
    ...
    protection={}
    roots=[HERE.with_name("navix_strict_thresholds_20260918"),BASE,PARENT,
    ...
    for scenario in ("broad_tag","extreme_tag","unfilter"):
        for mode in ("postfilter_graph","postfilter_twohop"):
            for t in (("0",) if mode=="postfilter_graph" else THRESHOLDS):
                folder="configs" if scenario=="broad_tag" else "diagnostic_configs"
                text=(HERE.with_name("navix_strict_thresholds_20260918")/
                    f"{folder}/{scenario}_posting_twohop_0.1_posting_0.01.ini").read_text()
                value=text.replace("posting_twohop_0.1_posting_0.01",case(mode,t))
                value=value.replace("NavixPostingThreshold=0.01","NavixPostingThreshold=0")
                value=value.replace("NavixTwoHopThreshold=0.1","NavixTwoHopThreshold="+t)
                value=value.replace("NavixMode=posting","NavixMode="+mode)

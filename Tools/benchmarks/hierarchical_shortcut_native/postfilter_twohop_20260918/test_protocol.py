...
class Protocol(unittest.TestCase):
    ...
    def test_fixed_configs(self):
        for path in (HERE/"configs").glob("*.ini"):
            ...
            if True:
                ...
                self.assertEqual(cfg["SearchSSDIndex"]["NavixPostingThreshold"],"0")
                self.assertEqual(cfg["SearchSSDIndex"]["NavixCapture"],"false")
                self.assertIn(cfg["SearchSSDIndex"]["NavixTwoHopThreshold"],("0","0.1","0.05"))
                self.assertEqual(cfg["Benchmark"]["Case"],
                    cfg["SearchSSDIndex"]["NavixMode"]+"_twohop_"+
                    cfg["SearchSSDIndex"]["NavixTwoHopThreshold"]+"_posting_0")
    ...
    def test_native_ini_and_environment_rejection(self):
        ...
        template=(HERE/"configs/broad_tag_postfilter_twohop_twohop_0.1_posting_0.ini").read_text()
        ...
        inputs={f"twohop_{i}":template.replace("NavixTwoHopThreshold=0.1",f"NavixTwoHopThreshold={value}")
            for i,value in enumerate(("nan","inf","-inf","-1","1.1","0.1junk","broken"))}
        inputs.update(
        ...
        for t in ("0.05","0.1"):
            for p in (t,"0.2"):
                inputs[f"strict_{t}_{p}"]=template.replace("NavixTwoHopThreshold=0.1",
                    f"NavixTwoHopThreshold={t}").replace("NavixPostingThreshold=0",f"NavixPostingThreshold={p}")
        for i,value in enumerate(("nan","inf","-inf","-0.1","0.6","0.01junk","broken")):
            inputs[f"posting_{i}"]=template.replace("NavixPostingThreshold=0",f"NavixPostingThreshold={value}")
        for name,text in inputs.items():
            ...
            if name.startswith("strict_"):
                self.assertIn("posting threshold zero",result.stderr,name)
            records.append({"case":name,"returncode":result.returncode,"stdout":result.stdout,"stderr":result.stderr})
        valid=HERE/"configs/broad_tag_postfilter_twohop_twohop_0.1_posting_0.ini"

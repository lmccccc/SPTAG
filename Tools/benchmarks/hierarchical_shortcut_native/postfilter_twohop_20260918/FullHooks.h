...
inline bool PostfilterOnly() { return mode=="postfilter_graph"; }
inline bool PostfilterTwoHop() { return mode=="postfilter_twohop"; }
inline bool ExplicitPostfilter() { return PostfilterOnly() || PostfilterTwoHop(); }
struct Observation {
    bool invoked=false;
...
template<class Parameters> void Configure(const Parameters& params) {
    ...
    mode=get("navixmode");
    if (mode!="navix" && mode!="posting" && !ExplicitPostfilter())
        throw std::runtime_error("Invalid NaviX mode");
    ...
    if (used!=twoHopText.size())
        throw std::runtime_error("Invalid native two-hop threshold");
    if (ExplicitPostfilter()) {
        Navix::ValidatePostfilterThresholds(twoHopThreshold,postingThreshold,PostfilterOnly());
    } else Navix::ValidateThresholds(twoHopThreshold,postingThreshold,mode=="posting");
    for (const auto& p:params)

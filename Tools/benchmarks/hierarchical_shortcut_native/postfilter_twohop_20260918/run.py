"""Exactly three fresh Broad points, two reverse-order repetitions, then stop."""
import hashlib
import json
import os
...
TRACES=(".native.u64",".navix.u64",".decisions.f64",".rows.u64",".head.i32",".head.f32",".own.i32",".qualification.u64",".operators.u64")
...
def freeze():
    ...
    write(OUTPUT/"runtime.json",{"timed_body_sha256":digest,
        ...
        "frozen_oracle_binary_not_timed":{"path":str(graph),"sha256":sha(graph)},
...
def validate(directory,scenario,mode,t,count,seconds):
    ...
    require(native["case"]==case(mode,t),"Ambiguous case identifier")
    require("NATIVE_SUPPLIER_OWNER_LOAD" not in text,"Disabled posting built owners")
    ...
    record={"label":directory.name,"scenario":scenario,"mode":mode,"two_hop_threshold":None if t is None else float(t),
        "posting_threshold":0,"posting_active":False,"native":native,"process_seconds":seconds,
    ...
    operators=matrix(prefix,".operators.u64","<u8",3)
    require(operators.shape==(count,3),"Uniform operators schema")
    record["operator_mean"]=dict(zip(("outer_expansions","ordinary_neighbor_entries","qualification_requests"),
        operators.mean(axis=0).tolist()))
    require(np.all(n[:,[2,4,5,6,7,8,9,10,11]]==0),"Disabled posting upper work")
    if mode=="postfilter_graph":
        require(np.all(n[:,[2,4,5,6,7,8,9,10,11]]==0),"Postfilter upper work")
        require(np.all(n[:,15]==(scenario!="unfilter")) and
            np.all(n[:,16]==(scenario=="unfilter")),"Wrong postfilter admission")
        require(np.all(operators[:,2]==0),"Control must not classify")
        require(np.all(matrix(prefix,".qualification.u64","<u8",6)==0),"Control qualification cache")
        require(np.all(matrix(prefix,".navix.u64","<u8",19)==0),"Control auxiliary work")
        require(Path(str(prefix)+".decisions.f64").stat().st_size==0,"Control decisions")
        record["trace_hashes"]=hashes(prefix,TRACES)
        record["action_mean"]=dict.fromkeys(ACTION,0)
        record["qualification_head_phase_mean"]=dict.fromkeys(QUALIFICATION,0)
        record["classified_initial_neighbor_entries"]=0
        oracle=PARENT/"broad_tag_postfilter_graph_r1/nprobe_24" if scenario=="broad_tag" else None
    else:
        ...
        require(np.all(fallback[:,3]/fallback[:,2]<float(t)) and np.all(fallback[:,5]!=1),
            "Posting fallback outside sparse domain")
        ...
        require(np.all(a[:,[4,5,14,15,16,17,18]]==0) and len(rows)==0,"Posting work present")
        require(np.array_equal(operators[:,2],a[:,6]),"Qualification counter mismatch")
        if scenario!="unfilter":
            require(np.array_equal(operators[:,0],a[:,:4].sum(axis=1)),"Outer expansion accounting")
        if scenario=="unfilter":
        ...
        oracle=None
        if scenario=="unfilter":
            oracle=PARENT/f"small_{scenario}_posting_twohop_{t}_posting_0.01"/"nprobe_24"
        unique,counts=np.unique(d[:,2:6],axis=0,return_counts=True)
        record["classified_initial_neighbor_entries"]=float(d[:,2].sum()/count)
        record["actual_route_ratios"]=[{"d":int(x[0]),"e":int(x[1]),
            "r":float(x[1]/x[0]) if x[0] else None,"route":int(x[2]),"fallback":int(x[3]),
            "count":int(n)} for x,n in zip(unique,counts)]
    if oracle:
        require(record["payloads"]==hashes(oracle,PAYLOADS),"Frozen final/SSD parity "+directory.name)
        require(hashes(prefix,CONTROL_TRACES)==hashes(oracle,CONTROL_TRACES),
...
def main():
    ...
    freeze()
    points=[("postfilter_graph","0")]+[("postfilter_twohop",t) for t in THRESHOLDS]
    plan=[(f"broad_tag_{case(mode,t)}_r{r}",mode,t)
        for r,order in ((1,points),(2,points[::-1])) for mode,t in order]
    write(OUTPUT/"measurement_plan.json",{"points":3,"ordinary_processes":6,"plan":plan,
    ...
    for mode,t in points:
        ...
        summary.append({"case":case(mode,t) if t else "native_result_only_postfilter_graph",
            "mode":mode,"two_hop_threshold":float(t) if t else None,            "posting_threshold":0,
            ...
            "recall_runs":[r["native"]["recall"] for r in pair],
            "recall_range":[min(r["native"]["recall"] for r in pair),max(r["native"]["recall"] for r in pair)],
            **{k:pair[0][k] for k in ("native_mean","action_mean","qualification_head_phase_mean","operator_mean",
                "classified_initial_neighbor_entries","ssd_mean",
    ...
    write(OUTPUT/"operations.json",{"records":records,"operation_seconds":time.monotonic()-start,"ordinary_index_loads":6})
    protect()

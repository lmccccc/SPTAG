...
int main() {
    const auto rejects=[](auto operation,const std::string& message) {
        bool rejected=false;
        try {operation();} catch (const std::runtime_error& e) {
            rejected=true;CHECK(std::string(e.what()).find(message)!=std::string::npos);
        }
        CHECK(rejected);
    };
    for (double t:{.05,.1}) {
        for (double posting:{t,t+.01}) {
            rejects([&]{Navix::SelectRoute(100,1,t,posting);},"strictly less");
            Hooks h;h.twoHopThreshold=t;h.postingThreshold=posting;
            rejects([&]{h.ValidatePolicy();},"strictly less");
        }
        CHECK(Navix::SelectRoute(100,0,t,t,false)==Navix::FullTwoHop);
        CHECK(Navix::SelectRoute(100,0,t,0)==Navix::FullTwoHop);
        CHECK(Navix::SelectRoute(100,1,t,.01)!=Navix::Posting);
        CHECK(Navix::SelectRoute(101,1,t,.01)==Navix::Posting);
        CHECK(Navix::SelectRoute(100,2,t,.01)==Navix::FullTwoHop);
        CHECK(Navix::SelectRoute(100,int(100*t),t,.01)==Navix::OneHop);
        CHECK(Navix::SelectRoute(0,0,t,.01)==Navix::NoNeighbors);
    }
    for (double bad:{-1.,1.1,std::numeric_limits<double>::infinity(),
                     std::numeric_limits<double>::quiet_NaN()}) {
        rejects([&]{Navix::GraphRoute(0,0,bad);},"two-hop");
        rejects([&]{Navix::SelectRoute(100,0,.1,bad);},"posting");
        Hooks h;h.postingThreshold=bad;
        rejects([&]{h.ValidatePolicy();},"posting");
        h.postingThreshold=0;h.twoHopThreshold=bad;
        rejects([&]{h.ValidatePolicy();},"two-hop");
    }
    {
    ...
    {
        const auto explicitHooks=[&](double threshold) {
            auto h=observed();h.navix=h.postfilterTwoHop=true;
            h.postingEnabled=false;h.postingThreshold=0;h.twoHopThreshold=threshold;
            return h;
        };
        const int b=fresh[0],c=fresh[60];
        clear();writeRow(0,{0,b});writeRow(b,{c});
        auto eligible=[&](int id){return id==0 || id==c;};
        auto forbidden=[](int,const COMMON::NativeEdgeConsumer&)->bool {CHECK(false);return false;};
        const auto execute=[&](Hooks& h,int budget) {
            h.eligible=eligible;h.posting=forbidden;
            Result result(data.data(),1);
            CHECK(index.SearchIndexWithNativeHooks(result,h,
                [&](int id){return id==c;},{},budget)==ErrorCode::Success);
            return std::make_pair(result.GetResult(0)->VID,result.GetResult(0)->Dist);
        };
        for(double threshold:{0.,.05,.1}) for(int budget:{2,8,64}) {
            Hooks ordinary=observed();
            auto hybrid=explicitHooks(threshold);
            const auto expected=execute(ordinary,budget);
            Result reference(data.data(),1);
            CHECK(index.SearchIndexWithResultFilter(reference,[&](int id){return id==c;},budget)==ErrorCode::Success);
            CHECK(reference.GetResult(0)->VID==expected.first && reference.GetResult(0)->Dist==expected.second);
            CHECK(reference.GetScanned()==ordinary.checked);
            CHECK(execute(hybrid,budget)==expected);
            CHECK(hybrid.checked==ordinary.checked && hybrid.evaluated==ordinary.evaluated);
            CHECK(hybrid.visitedEdges==ordinary.visitedEdges && hybrid.allVisited==ordinary.allVisited);
            CHECK(hybrid.queueOffers==ordinary.queueOffers && hybrid.queueAccepted==ordinary.queueAccepted);
            CHECK(hybrid.ordinaryNeighborEntries==ordinary.ordinaryNeighborEntries);
            CHECK(hybrid.outerExpansions==ordinary.outerExpansions);
            CHECK(decision(hybrid,0).d==2 && decision(hybrid,0).e==1);
            CHECK(seen(hybrid,b));
            if(budget>=8) CHECK(expected.first==c);
            auto off=explicitHooks(threshold);off.capture=off.diagnostics=false;
            CHECK(execute(off,budget)==expected && off.checked==hybrid.checked);
            CHECK(off.allVisited==hybrid.allVisited && off.outerExpansions==0 &&
                off.ordinaryNeighborEntries==0 && off.qualificationChecks==0);
        }
        std::cout<<"PASS explicit high-r nonmatching A-B-C bridge: actual result-only BKT "
            "IDs/distances/checked/queues/edge AND all CheckAndSet encounter trace; "
            "visited-valid ratio, d0, classifier0, diagnostics parity\n";
        for(double threshold:{.05,.1}) {
            for(int degree:{20,21,120,127}) {
                clear();std::vector<int> row(fresh.begin(),fresh.begin()+degree);row[0]=0;
                writeRow(0,row);
                const int e=degree<100?int(threshold*20):6;
                const std::set<int> matches(row.begin(),row.begin()+e);
                auto h=explicitHooks(threshold);h.posting=forbidden;
                run(h,matches,64);
                const auto d=decision(h,0);
                CHECK(d.d==degree && d.e==e);
                CHECK(d.route==Navix::GraphRoute(degree,e,threshold));
                CHECK(h.calls==0 && h.parentDistances==0 && h.signatureChecks==0 && h.rows.empty());
                if(degree==127) CHECK(d.route==Navix::Directed && h.secondRows>0);
            }
            clear();writeRow(0,{b});
            std::vector<int> longRow(fresh.begin()+1,fresh.begin()+61);
            writeRow(b,longRow);
            std::set<int> matches(longRow.begin(),longRow.end());
            auto h=explicitHooks(threshold);h.posting=forbidden;
            run(h,matches,2);
            CHECK(decision(h,0).route==Navix::FullTwoHop && h.secondRows>=1);
            CHECK(h.checked>2 && seen(h,longRow.back()));
            Hooks original=observed();Result truncated(data.data(),1);
            CHECK(index.SearchIndexWithNativeHooks(truncated,original,
                [&](int id){return matches.count(id)!=0;},{},2)==ErrorCode::Success);
            CHECK(original.checked<=2 && !seen(original,longRow.back()));
            h=explicitHooks(threshold);h.posting=forbidden;
            run(h,{},64);
            CHECK(decision(h,0).e==0 && h.calls==0 && h.rows.empty());
        }
        std::vector<std::shared_ptr<VectorSet>> noCatalogs;
        std::vector<int> noPostings;
        for(const char* mode:{"postfilter_graph","postfilter_twohop"}) {
            ShortcutFull::mode=mode;ShortcutFull::twoHopThreshold=std::string(mode)=="postfilter_graph"?0:.1;
            ShortcutFull::postingThreshold=0;
            for(bool empty:{false,true}) for(bool capture:{false,true}) {
                ShortcutFull::capture=capture;
                Result result(data.data(),2),reference(data.data(),2);
                CHECK(NativeReuse::Search(&index,&result,nullptr,noCatalogs,noPostings,
                    [&](int id){CHECK(!empty);return id==c;},
                    [&](int,float){CHECK(!empty);return false;},
                    [](int,int){CHECK(false);return false;},empty,
                    [&](int id){CHECK(!empty);return id==c;})==ErrorCode::Success);
                if(empty) {
                    CHECK(index.SearchIndex(reference)==ErrorCode::Success);
                    for(int i=0;i<2;++i) CHECK(result.GetResult(i)->VID==reference.GetResult(i)->VID &&
                        result.GetResult(i)->Dist==reference.GetResult(i)->Dist);
                }
                if(capture) {
                    const auto& h=ShortcutFull::Last().native;
                    CHECK(h.calls==0 && h.parentDistances==0 && h.childDistances==0 && h.members==0);
                    CHECK(!h.ownPoint && !h.ownGain && !h.eligible && !h.posting);
                    if(empty || std::string(mode)=="postfilter_graph") CHECK(h.qualificationChecks==0);
                }
            }
        }
        ShortcutFull::capture=false;ShortcutFull::mode="posting";
        std::cout<<"PASS explicit .05/.1 sparse boundaries directed/full, complete two-hop vs native "
            "per-edge truncation, e0 no posting callback, hierarchy-free null model/catalogs, "
            "empty exact native bypass and callback lifetime\n";
    }

    for (double threshold:{.05,.1,.5}) for (bool postingMode:{false,true}) {
        for (int degree:{20,21}) {
            clear();
            std::vector<int> row(fresh.begin(),fresh.begin()+degree);
            row[0]=0;
            writeRow(0,row);
            const int validCount=int(threshold*20);
            std::set<int> matches(row.begin(),row.begin()+validCount);
            auto h=observed();
            h.twoHopThreshold=threshold; h.postingThreshold=postingMode?.01:0;
            int calls=0;
            auto reject=[&](int,const COMMON::NativeEdgeConsumer&){++calls;return false;};
            if (postingMode) h.posting=reject;
            const auto result=run(h,matches,64);
            const auto d=decision(h,0);
            CHECK(d.d==degree && d.e==validCount);
            const auto graph=Navix::GraphRoute(degree,validCount,threshold);
            CHECK((degree==20)==(graph==Navix::OneHop));
            const bool posting=postingMode && double(validCount)/degree<.01;
            CHECK(d.route==(posting?Navix::Posting:graph));
            CHECK(d.fallback==(posting?int(graph):0));
            CHECK(!posting || calls>0);
            Hooks off;off.twoHopThreshold=threshold;off.postingThreshold=h.postingThreshold;
            if (postingMode) off.posting=reject;
            CHECK(run(off,matches,64)==result && off.checked==h.checked);
            auto again=observed();again.twoHopThreshold=threshold;again.postingThreshold=h.postingThreshold;
            if (postingMode) again.posting=reject;
            CHECK(run(again,matches,64)==result && again.visitedEdges==h.visitedEdges);
            if (threshold==.05 && posting) CHECK(d.fallback==Navix::FullTwoHop);
        }
        clear();
        auto h=observed();h.twoHopThreshold=threshold;h.postingThreshold=postingMode?.01:0;
        auto forbidden=[](int,const COMMON::NativeEdgeConsumer&)->bool {CHECK(false);return false;};
        if (postingMode) h.posting=forbidden;
        run(h,{0},32);
        CHECK(decision(h,0).route==Navix::NoNeighbors && h.qualificationChecks==0);
    }
    std::cout<<"PASS cutoffs .05/.1/.5: native graph and posting, exact equality and just below, "
        "sentinel d, visited-valid e, d0, strict .01 posting, diagnostics/determinism\n";
    for (double t:{.05,.1}) {
        for (int degree:{100,101}) for (int validCount:{1,2}) {
            clear();
            std::vector<int> row(fresh.begin(),fresh.begin()+degree);row[0]=0;
            writeRow(0,row);
            auto h=observed();h.twoHopThreshold=t;h.postingThreshold=.01;
            int calls=0;
            auto reject=[&](int,const COMMON::NativeEdgeConsumer&){++calls;return false;};
            h.posting=reject;
            const std::set<int> matches(row.begin(),row.begin()+validCount);
            const auto result=run(h,matches,64);const auto d=decision(h,0);
            const bool posting=degree==101 && validCount==1;
            CHECK(d.d==degree && d.e==validCount);
            CHECK(d.route==(posting?Navix::Posting:Navix::FullTwoHop));
            CHECK(d.fallback==(posting?Navix::FullTwoHop:0));
            CHECK((calls>0)==posting);
            Hooks off;off.twoHopThreshold=t;off.postingThreshold=.01;off.posting=reject;
            CHECK(run(off,matches,64)==result && off.checked==h.checked);
        }
        clear();writeRow(0,{fresh[0]});
        Hooks disabled=observed();disabled.twoHopThreshold=t;disabled.postingThreshold=0;
        auto forbidden=[](int,const COMMON::NativeEdgeConsumer&)->bool {CHECK(false);return false;};
        disabled.posting=forbidden;run(disabled,{},32);
        CHECK(decision(disabled,0).route==Navix::FullTwoHop);
        disabled=observed();disabled.twoHopThreshold=t;disabled.postingThreshold=t;
        disabled.postingEnabled=false;disabled.posting=forbidden;run(disabled,{},32);
        CHECK(decision(disabled,0).route==Navix::FullTwoHop);
        for (double p:{t,t+.01}) {
            Hooks invalid;invalid.twoHopThreshold=t;invalid.postingThreshold=p;
            rejects([&]{run(invalid,{},32);},"strictly less");
        }
    }
    std::cout<<"PASS strict shared helper and actual hooks: equality/reversal rejected, .01 equality/below, "
        "true nonposting sparse interval, zero disabled, NaviX-only equality allowed, sparse-only fallback\n";
    for (int e:{0,1,5,10}) {
        clear();
    ...
    ownOnly=observed();ownOnly.postfilterTwoHop=true;ownOnly.postingEnabled=false;
    ownOnly.postingThreshold=0;ownOnly.twoHopThreshold=.1;ownOnly.eligible=ownEligible;
    ownOnlyIDs.clear();
    ownOnly.ownPoint=[&](int id,float){if(id==target) ownOnlyIDs.insert(id);};
    CHECK(run(ownOnly,{},64).first<0 && ownOnlyIDs.count(target));
    ...
    CHECK(cachedRun(true).first!=target);
    for(double t:{.05,.1}) {
        auto h=observed();h.postfilterTwoHop=true;h.postingEnabled=false;
        h.postingThreshold=0;h.twoHopThreshold=t;
        CHECK(run(h,{target},64).first!=target);
    }
    ...
    for(bool explicitMode:{false,true}) for(int budget:{1,2,8,32,2048}) {
        Hooks own=observed();own.navix=true;
        own.postfilterTwoHop=explicitMode;own.postingEnabled=!explicitMode;
        own.postingThreshold=0;own.twoHopThreshold=.1;
        std::set<int> ownIDs;
        ...
        alias.postfilterTwoHop=explicitMode;alias.postingEnabled=!explicitMode;
        alias.postingThreshold=0;alias.twoHopThreshold=.05;
    ...
    const std::map<std::string,std::string> validParams{
        {"navixmode","posting"},{"navixcapture","false"},{"navixpostingthreshold","0.01"},
        {"navixtwohopthreshold","0.5"},
    ...
    for(const char* mode:{"postfilter_graph","postfilter_twohop"}) {
        auto params=validParams;params["navixmode"]=mode;params["navixpostingthreshold"]="0";
        params["navixtwohopthreshold"]="0";ShortcutFull::Configure(params);
        if(std::string(mode)=="postfilter_twohop") for(const char* t:{"0.05","0.1"}) {
            params["navixtwohopthreshold"]=t;ShortcutFull::Configure(params);
        }
        for(const char* bad:{"nan","inf","-1","1.1"}) {
            params["navixtwohopthreshold"]=bad;
            rejects([&]{ShortcutFull::Configure(params);},"threshold");
        }
        params["navixtwohopthreshold"]="0";params["navixpostingthreshold"]="0.01";
        rejects([&]{ShortcutFull::Configure(params);},"posting threshold zero");
    }
    for (const char* t:{"0.05","0.1"}) {
        auto params=validParams;params["navixtwohopthreshold"]=t;params["navixpostingthreshold"]=t;
        rejects([&]{ShortcutFull::Configure(params);},"strictly less");
        params["navixmode"]="navix";ShortcutFull::Configure(params);
        params["navixmode"]="posting";params["navixpostingthreshold"]="0";ShortcutFull::Configure(params);
    }
    ...
    for (const char* value:{"nan","inf","-inf","-0.1","0","1.1","0.05junk",""}) {
        auto params=validParams;params["navixtwohopthreshold"]=value;
        bool caught=false;try{ShortcutFull::Configure(params);}catch(const std::exception&){caught=true;}
        CHECK(caught);
    }
    {
        auto params=validParams;params.erase("navixtwohopthreshold");
        bool caught=false;try{ShortcutFull::Configure(params);}catch(const std::exception&){caught=true;}
        CHECK(caught);
        params=validParams;params["navixtwohopthreshold"]="0.05";params["navixpostingthreshold"]="0.1";
        caught=false;try{ShortcutFull::Configure(params);}catch(const std::exception&){caught=true;}
        CHECK(caught);
        params["navixmode"]="navix";ShortcutFull::Configure(params);
    }
    for (const char* value:{"0.05","0.1","0.5"}) {
        auto params=validParams;params["navixtwohopthreshold"]=value;ShortcutFull::Configure(params);
    }

...
void FusedParity(SPTAG::BKT::Index<float>& index, const float* target) {
    ...
    for (int budget : {1, 2, 8, 32, 2048}) {
        ...
        next.expand = [](int, const int*, int, bool, const COMMON::NativeEdgeConsumer&) {};
        next.completeOrdinary = [&](COMMON::NativeDegreeFrame& frame, const int* row, int width,
...
int main() {
    ...
    for (int budget : {1, 2, 8, 32, 2048}) {
        ...
        NativeReuse::PostingSupplier scalarSupply(model, scalar,
            [](int, int) { return true; }, [](int, int id) { return float(id); });
        NativeReuse::PostingSupplier batchSupply(model, batch,
            [](int, int) { return true; }, [](int, int id) { return float(id); });
        const auto supplyRow = [&](NativeReuse::PostingSupplier& supplier, bool useBatch, int head,
            ...
            std::unordered_set<int> ids{head};
            COMMON::NativeDegreeFrame frame{};
            supplier.Supply(head, 1, ids, traversal, frame, consumer);
        };
        scalar.expand = [&](int head, const int*, int, bool, const COMMON::NativeEdgeConsumer& native) {
            supplyRow(scalarSupply, false, head, native);
        };
        batch.expand = [&](int head, const int*, int, bool, const COMMON::NativeEdgeConsumer& native) {
    ...
    for (bool capture : {false, true}) {
        ...
        Check(n.defaultAdmission && !n.filteredAdmission && n.rowEligibilityEvaluations == 0 &&
        ...
        for (const auto& f : n.frames) Check(f.physical == f.eligible && !f.calls);
    }

                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    const auto admitFilteredResult =
        [&](SizeType p_key,
            ...
            if (p_space.m_nativeHooks && p_space.m_nativeHooks->ownPoint &&
            ...
            const bool known = eligibility && eligibility->postingKnown;
            if (known && p_space.m_nativeHooks && p_space.m_nativeHooks->capture)
                ++p_space.m_nativeHooks->resultReuse;
            return (known ? eligibility->posting : p_resultFilter(p_result)) &&
                notDeleted(
                ...
                if constexpr (!EnableCrossEdges)
                {
                    if (p_traversalFilter && collapsedLocal != p_representative) {
                        if (p_space.m_nativeHooks && p_space.m_nativeHooks->qualify) {
                            aliasEligibility = p_space.m_nativeHooks->qualify(collapsedLocal);
    ...
    int deferredCount = 0;
                ...
                COMMON::NativeSupplyAccounting* supply = nullptr)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                if (supply) {
                    ++hook->members;
                    if (supply->identities.insert(targetLocal).second) {
                        eligibility = hook->qualify ? hook->qualify(targetLocal)
                            : COMMON::NativeEligibility{
                                !p_traversalFilter || p_traversalFilter(targetLocal), false, false};
                        qualified = true;
                ...
                if (distinct) {
                    ...
                    if (p_traversalFilter && (!visited || needsDegree)) {
                        eligibility = hook->qualify ? hook->qualify(targetLocal)
                            : COMMON::NativeEligibility{p_traversalFilter(targetLocal), false, false};
                        qualified = true;
                    ...
                    if (needsDegree && deferredCount) {
                        for (int i = 0; i < deferredCount; ++i) {
                            ...
                            const bool allowed = hook->qualify ? hook->qualify(pending.id).traversal
                                : p_traversalFilter(pending.id);
                ...
                if (p_traversalFilter) {
                    if (qualified) {
                        ...
                        eligibility = hook->qualify(targetLocal);
                        qualified = true;
    ...
    const auto inject = [&](SizeType head, int nodeID, const Index<T>* index,
        ...
        native.consumeRow = [&](const std::uint32_t* members, int count,

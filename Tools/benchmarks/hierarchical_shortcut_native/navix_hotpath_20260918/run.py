"""One bounded, interleaved n24 milestone; old extreme is a frozen oracle only."""
import hashlib
import json
import os
...
HOTPATH=["component_requests","component_evaluations","positive_hits","negative_hits",
         "cache_storage_growths","graph_scratch_growths"]
...
def invoke(label,scenario,case,version,binaries):
    ...
    wall=process(["numactl","--cpunodebind=2","--membind=2",str(binaries[version]),"--config",str(cfg)],
                 directory,max_seconds=600 if scenario=="extreme_tag" else 180)
    ...
    if version!="postfilter":
        ...
        if version=="new":
            hot=np.fromfile(str(prefix)+".hotpath.u64",dtype="<u8").reshape(1000,6)
            record["hotpath_mean"]=dict(zip(HOTPATH,hot.mean(axis=0).tolist()))
            require(np.all(hot[:,0]==hot[:,1]+hot[:,2]+hot[:,3]),"Cache accounting mismatch")
            if scenario!="unfilter":
                require(hot[:,2].sum()>0 and hot[:,3].sum()>0,"Missing positive/negative hits")
                require(np.all(hot[:,4:]==0),"Scratch/cache growth after warmup")
...
def main():
    ...
    plan.extend((f"unfilter_new_posting_r{r}","unfilter","posting","new") for r in (1,2))
    plan.extend((f"extreme_tag_new_{c}_r{r}","extreme_tag",c,"new") for r,order in
                ((1,("navix","posting")),(2,("posting","navix"))) for c in order)
    write(OUTPUT/"measurement_plan.json",{"plan":plan,"per_extreme_process_ceiling_seconds":600,
        "old_extreme":"Frozen trajectory oracle only; no paired timing claim","protocol":"1000 warmup + 1000 timed + 1000 untimed diagnostics; n24 offset0 topk10 NUMA CPU+memory2 querythread1"})
    ...
    for scenario,case,version in dict.fromkeys((r["scenario"],r["case"],r["version"]) for r in records):
        ...
        summary.append({"scenario":scenario,"case":case,"version":version,"ordinary_ms_runs":ms,
            ...
            **{k:pair[0][k] for k in ("native_mean","action_mean","hotpath_mean","ssd_mean") if k in pair[0]}})

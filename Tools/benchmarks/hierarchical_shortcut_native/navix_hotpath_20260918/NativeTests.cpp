...
int main() {
    {
        NativeReuse::PredicateLease lease(true,10000,true);
        auto& cache=*lease.Get();
        int evaluations=0;
        auto yes=[&]{++evaluations;return true;};
        auto no=[&]{++evaluations;return false;};
        CHECK(cache.Get(9000,NativeReuse::PredicateCache::Support,yes));
        CHECK(cache.Get(9000,NativeReuse::PredicateCache::Support,no));
        CHECK(!cache.Get(9000,NativeReuse::PredicateCache::Posting,no));
        CHECK(!cache.Get(9000,NativeReuse::PredicateCache::Posting,yes));
        CHECK(cache.Get(9000,NativeReuse::PredicateCache::Exact,yes));
        CHECK(evaluations==3 && cache.counts[2]==1 && cache.counts[3]==1);
        bool deleted=false;
        const auto eligible=[&] {return !deleted && cache.Get(9000,NativeReuse::PredicateCache::Exact,no);};
        CHECK(eligible());deleted=true;CHECK(!eligible());deleted=false;CHECK(eligible());
        NativeReuse::PredicateLease nested(true,10000,true);
        CHECK(nested.Get()!=&cache);
        CHECK(!nested.Get()->Get(9000,NativeReuse::PredicateCache::Exact,no));
        cache.Begin(2,true);
        CHECK(!cache.Get(1,NativeReuse::PredicateCache::Exact,no));
        cache.Begin(10001,true);
        CHECK(!cache.Get(9000,NativeReuse::PredicateCache::Support,no));
        CHECK(cache.Get(10000,NativeReuse::PredicateCache::Exact,yes));
        cache.generation=UINT64_MAX;cache.Begin(10001,true);
        CHECK(!cache.Get(10000,NativeReuse::PredicateCache::Exact,no));
        COMMON::WorkSpace scratch;
        scratch.m_navixValid.resize(8193);
        scratch.m_navixIntermediates.resize(8193);
        CHECK(scratch.m_navixValid.size()==8193 && scratch.m_navixIntermediates.size()==8193);
    }
    ...
    for (int e:{0,1,5,10}) {
        ...
        h.posting=reject;
        const auto expected=run(h,matches,64);
        ...
        Hooks quiet;quiet.auditVisited=true;quiet.postingThreshold=.05;quiet.posting=reject;
        CHECK(run(quiet,matches,64)==expected);
        CHECK(quiet.checked==h.checked && quiet.visitedEdges==h.visitedEdges);
        CHECK(quiet.graphWork.members==0 && quiet.graphWork.predicateChecks==0 &&
              quiet.headDistances==0 && quiet.decisions.empty());
    }
    clear();
    ...
    clear();writeRow(0,{bridge,target,bridge,target});
    writeRow(bridge,{target,bridge,target});writeRow(target,{bridge,target});
    NativeReuse::PredicateCache realCache;
    int selected=target, ownCalls=0;
    bool gain=true;
    const auto cachedRun=[&](bool diagnostics) {
        realCache.Begin(count,diagnostics);
        Hooks h;h.navix=true;h.capture=h.diagnostics=diagnostics;
        auto posting=[&](int id) {
            return realCache.Get(id,NativeReuse::PredicateCache::Posting,[&]{return id==selected;});
        };
        auto eligible=[&](int id) {return posting(id);};
        auto own=[&](int,float) {++ownCalls;return gain;};
        h.eligible=eligible;h.ownGain=own;
        Result r(data.data(),1);
        CHECK(index.SearchIndexWithNativeHooks(r,h,posting,{},64)==ErrorCode::Success);
        return r.GetResult(0)->VID;
    };
    CHECK(cachedRun(true)==target);
    CHECK(realCache.counts[2]>0 && realCache.counts[3]>0 && ownCalls>0);
    const int previousOwn=ownCalls;
    gain=false;
    CHECK(cachedRun(false)==target && ownCalls>previousOwn);
    selected=bridge;
    CHECK(cachedRun(true)==bridge);

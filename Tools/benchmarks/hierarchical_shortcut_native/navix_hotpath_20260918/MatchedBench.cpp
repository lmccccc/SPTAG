...
#ifdef MATCHED_CURRENT
    if (out.caseName != "navix" && out.caseName != "posting")
...
int Run(int argc, char** argv)
{
    ...
    for (int nprobe : options.probes) {
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            std::vector<std::array<std::uint64_t, 6>> hotpathWork;
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (options.caseName != "h3") {
                    ...
                    hotpathWork.push_back({n.predicateCache[0],n.predicateCache[1],n.predicateCache[2],
                        n.predicateCache[3],n.predicateCache[4],n.scratchGrowths});

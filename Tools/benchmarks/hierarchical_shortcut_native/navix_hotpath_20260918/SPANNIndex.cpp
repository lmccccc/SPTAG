...
template <typename T> ErrorCode Index<T>::SearchIndex(QueryResult &p_query, bool p_searchDeleted) const
{
    ...
    NativeReuse::PredicateLease predicateLease(NativeReuse::Enabled() && hasExactFilter,
        m_vectorTranslateMap.R(),ShortcutFull::capture);
    auto* predicateCache=predicateLease.Get();
    const auto rawLimitedSupportMatchesPredicate =
    ...
    const auto limitedSupportMatchesPredicate = [&](SizeType head) {
        const auto evaluate=[&] { return rawLimitedSupportMatchesPredicate(head); };
        return predicateCache ? predicateCache->Get(head,NativeReuse::PredicateCache::Support,evaluate) : evaluate();
    };
    const auto rawLimitedHeadMatchesExactPredicate =
    ...
    const auto limitedHeadMatchesExactPredicate = [&](SizeType head) {
        const auto evaluate=[&] { return rawLimitedHeadMatchesExactPredicate(head); };
        return predicateCache ? predicateCache->Get(head,NativeReuse::PredicateCache::Exact,evaluate) : evaluate();
    };
    ...
    if (!usedHeadBundleSearch)
    {
        ...
        if (!m_metadataOnlyHeadStore) {
            ...
            const auto posting = [&](int head) {
                const auto evaluate=[&] {
                return (useLimitedTagPure ? limitedSupportMatchesPredicate(head) : secondLevelHeadAdmission(head)) &&
                    m_extraSearcher->CheckValidPosting(head, nullptr) &&
                    (useLimitedTagPure || !candidatePostingFilter || candidatePostingFilter(head));
                };
                return predicateCache ? predicateCache->Get(head,NativeReuse::PredicateCache::Posting,evaluate) : evaluate();
            };
            ...
            ret = NativeReuse::Search(nativeBKT, p_queryResults, m_nativePostingModel.get(), m_secondLevelCatalogs,
                m_secondLevelPostings, posting, ownGain, [&](int level, int id) {
                    ...
                    return vid<static_cast<std::uint64_t>(m_versionMap.Count()) &&
                        ...
                        limitedHeadMatchesExactPredicate(head);
                }, predicateCache);
            if (ret != ErrorCode::Success) return ret;
        }
    }

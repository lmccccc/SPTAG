                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    struct Outcome { bool valid=false, useful=false; int checks=0; };
    Outcome* activeOutcome=nullptr;
    const auto admitFilteredResult =
        [&](SizeType p_key,
            ...
            SizeType p_result,
            float p_distance,
            bool preserveCollapsedStop = false) {
            if (!p_resultFilter || p_result < 0 ||
                p_space.CheckResultAndSet(p_key)) {
                return false;
            }
            if (p_space.m_nativeHooks &&
                (p_space.m_nativeHooks->ownPoint || p_space.m_nativeHooks->ownGain) &&
                notDeleted(p_index->m_deletedID, p_local) &&
                checkFilter(p_index->m_pMetadata, p_local, filterFunc)) {
                if (p_space.m_nativeHooks->ownGain) {
                    const bool gained=p_space.m_nativeHooks->ownGain(p_result,p_distance);
                    if (activeOutcome && gained) activeOutcome->valid=activeOutcome->useful=true;
                } else p_space.m_nativeHooks->ownPoint(p_result, p_distance);
            }
            // Collapsed callers use this return value to stop alias enumeration.
            if (!preserveCollapsedStop && p_distance > p_query.worstDist()) return false;
            if (activeOutcome) ++activeOutcome->checks;
            const bool valid=p_resultFilter(p_result);
            if (activeOutcome && valid && notDeleted(p_index->m_deletedID,p_local)) {
                activeOutcome->valid=true;
                if (p_distance<=p_query.worstDist()) activeOutcome->useful=true;
            }
            return valid &&
                notDeleted(
    ...
    const auto qualifies = [&](SizeType id) {
        auto* hook=p_space.m_nativeHooks;
        if (hook && hook->diagnostics) ++hook->qualificationChecks;
        const auto match=[&](SizeType candidate) {
            return notDeleted(m_deletedID,candidate) &&
                (hook && hook->eligible ? hook->eligible(candidate) :
                 p_resultFilter && p_resultFilter(candidate));
        };
        if (match(id)) return true;
        const auto marker=m_pGraph[id][m_pGraph.m_iNeighborhoodSize-1];
        if (marker>=-1) return false;
        const auto& tree=m_pTrees[-2-marker];
        for (SizeType i=-tree.childStart;i>0 && i<=tree.childEnd;++i)
            if (match(m_pTrees[i].centerid)) return true;
        return false;
    };
    bool finishInjectedRow = false;
        const auto expandEdges =
            [&](SizeType currentLocal, int currentNode, const Index<T>* currentIndex,
                const auto* p_edges,
                DimensionType p_begin,
                DimensionType p_end,
                bool p_crossEncoded,
                Navix::Work* work=nullptr)
        {
                 ...
                 edge < p_end; ++edge)
            {
                ...
                if (targetLocal < 0 || targetLocal >= targetIndex->m_pSamples.R())
                {
                    if (finishInjectedRow) throw std::runtime_error("Invalid auxiliary H1 member");
                    continue;
                }
                if (work) ++work->members;
                if (p_space.m_nativeHooks && p_space.m_nativeHooks->injected) {
                    if (work) ++work->predicateChecks;
                    if (!qualifies(targetLocal)) continue;
                }
                const bool visited=p_space.CheckAndSet(targetKey);
                if (p_space.m_nativeHooks && p_space.m_nativeHooks->auditVisited)
                    p_space.m_nativeHooks->visitedEdges.emplace_back(targetKey,visited);
                if (visited) continue;
                if (work) ++work->fresh;
                if (p_traversalFilter && !p_traversalFilter(targetLocal)) continue;
                if (p_space.m_nativeHooks && p_space.m_nativeHooks->navix)
                    _mm_prefetch(reinterpret_cast<const char*>(targetIndex->m_pSamples[targetLocal]),_MM_HINT_T0);
                const float distance =
                    m_fComputeDistance(
                        ...
                        GetFeatureDim());
                if (p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics)
                ...
                ++p_space.m_iNumberOfCheckedLeaves;
                Outcome outcome;
                activeOutcome=work && p_space.m_nativeHooks && p_space.m_nativeHooks->diagnostics?&outcome:nullptr;
                if (p_resultFilter)
                {
                    const DimensionType
                        targetCheckPosition =
                ...
                activeOutcome=nullptr;
                if (work) {
                    work->predicateChecks+=outcome.checks;
                    work->predicatePasses+=outcome.valid;
                    work->useful+=outcome.useful;
                }
                auto* observation = p_space.m_nativeHooks;
    ...
    const auto expandNavix = [&](SizeType head,const SizeType* row,int width) {
        auto& hook=*p_space.m_nativeHooks;
        Navix::Decision decision;
        decision.head=head; decision.checkedBefore=p_space.m_iNumberOfCheckedLeaves;
        const auto signatures=hook.signatureChecks, representatives=hook.parentDistances, members=hook.members;
        auto& valid=p_space.m_navixValid;
        valid.clear();
        if (hook.diagnostics && valid.capacity()<static_cast<std::size_t>(width)) ++hook.scratchGrowths;
        valid.reserve(width);
        for (int i=0;i<width && row[i]>=0;++i) {
            if (i+4<width && row[i+4]>=0 && hook.prefetchPredicate) hook.prefetchPredicate(row[i+4]);
            if (row[i]>=m_pSamples.R()) throw std::runtime_error("Invalid native graph neighbor");
            const bool match=qualifies(row[i]);
            valid.push_back(match);
            decision.e+=match;
        }
        decision.d=static_cast<int>(valid.size());
        auto route=Navix::GraphRoute(decision.d,decision.e);
        if (decision.d && double(decision.e)/decision.d<hook.postingThreshold) route=Navix::Posting;
        decision.route=route;
        if (hook.diagnostics) ++hook.branches[route];
        const auto publish=[&] {
            decision.checkedAfter=p_space.m_iNumberOfCheckedLeaves;
            decision.signatures=hook.signatureChecks-signatures;
            decision.representatives=hook.parentDistances-representatives;
            decision.members=hook.members-members;
            if (hook.capture) hook.decisions.push_back(decision);
        };
        if (!decision.d || p_space.m_iNumberOfCheckedLeaves>=p_space.m_iMaxCheck) { publish(); return; }
        const auto emit=[&](int id) {
            // Graph IDs are local, already qualified and within a complete row.
            // Use the native SIMD distance function without the generic cross-edge
            // decoder and singleton row setup.
            Navix::Work* work=hook.diagnostics?&hook.graphWork:nullptr;
            if (work) ++work->members;
            const bool visited=p_space.CheckAndSet(id);
            if (hook.auditVisited) hook.visitedEdges.emplace_back(id,visited);
            if (visited) return;
            if (work) ++work->fresh;
            if (p_traversalFilter && !p_traversalFilter(id)) return;
            _mm_prefetch(reinterpret_cast<const char*>(m_pSamples[id]),_MM_HINT_T0);
            const float distance=m_fComputeDistance(p_query.GetQuantizedTarget(),m_pSamples[id],GetFeatureDim());
            if (hook.diagnostics) hook.Distance(id,distance,false);
            ++p_space.m_iNumberOfCheckedLeaves;
            Outcome outcome;
            activeOutcome=work?&outcome:nullptr;
            if (p_resultFilter) {
                if (m_pGraph[id][m_pGraph.m_iNeighborhoodSize-1]<-1)
                    admitCollapsedResults(0,this,nullptr,id,distance);
                else admitFilteredResult(id,this,id,id,distance);
            }
            activeOutcome=nullptr;
            if (work) {
                work->predicateChecks+=outcome.checks;
            ...
            if (p_space.m_Results.insert(distance)) {
                if (work) ++hook.queueAccepted;
                p_space.m_NGQueue.insert(NodeDistPair(id,distance));
            } else if (work) ++hook.queueRejected;
        };
        if (route==Navix::Posting) {
            auto consume=[&](const std::uint32_t* ids,int count) {
                hook.injected=true; finishInjectedRow=true;
                if (count) _mm_prefetch(reinterpret_cast<const char*>(ids),_MM_HINT_T0);
                Navix::Work work;
                expandEdges(head,0,this,ids,0,count,false,&work);
                hook.injected=false; finishInjectedRow=false;
                return work;
            };
            auto within=[&] { return p_space.m_iNumberOfCheckedLeaves<p_space.m_iMaxCheck; };
            COMMON::NativeEdgeConsumer native{consume,within};
            if (hook.posting && hook.posting(head,native)) { publish(); return; }
            route=Navix::GraphRoute(decision.d,decision.e);
            decision.fallback=route;
            if (hook.diagnostics) ++hook.fallbacks;
        }
        // Native MaxCheck gates the next expansion, never truncates a selected
        // second-hop/CSR row. The same checked-leaf counter owns all distances.
        finishInjectedRow=true;
        const auto second=[&](int id,int* encounters) {
            if (hook.diagnostics) ++hook.secondRows;
            if (hook.capture) hook.intermediates.push_back(id);
            const auto* neighbors=m_pGraph[id];
            _mm_prefetch(reinterpret_cast<const char*>(neighbors),_MM_HINT_T0);
            for (int j=0;j<m_pGraph.m_iNeighborhoodSize && neighbors[j]>=0;++j) {
                if (j+4<m_pGraph.m_iNeighborhoodSize && neighbors[j+4]>=0 && hook.prefetchPredicate)
                    hook.prefetchPredicate(neighbors[j+4]);
        ...
        if (route==Navix::OneHop) {
            for (int i=0;i<decision.d;++i) if (valid[i]) emit(row[i]);
        } else if (route==Navix::Directed) {
            auto& intermediates=p_space.m_navixIntermediates;
            intermediates.clear();
            const std::greater<std::pair<float,int>> order;
            int encounters=decision.e;
            for (int i=0;i<decision.d;++i) {
                const int id=row[i];
                if (valid[i]) emit(id);
                else if (!p_space.nodeCheckStatus.Contains(id)) {
                    _mm_prefetch(reinterpret_cast<const char*>(m_pSamples[id]),_MM_HINT_T0);
                    const float d=m_fComputeDistance(p_query.GetQuantizedTarget(),m_pSamples[id],GetFeatureDim());
                    ++p_space.m_iNumberOfCheckedLeaves;
                    if (hook.diagnostics) {
                        hook.Distance(id,d,false);
                        ++hook.intermediateDistances;
                    }
                    if (hook.diagnostics && intermediates.size()==intermediates.capacity()) ++hook.scratchGrowths;
                    intermediates.emplace_back(d,id);
                    std::push_heap(intermediates.begin(),intermediates.end(),order);
                }
            }
            while (!intermediates.empty() && encounters<decision.d) {
                const int id=intermediates.front().second;
                std::pop_heap(intermediates.begin(),intermediates.end(),order);
                intermediates.pop_back();
                if (p_space.CheckAndSet(id)) continue;
                second(id,&encounters);
            }
        } else {
            for (int i=0;i<decision.d;++i) {
                if (valid[i]) emit(row[i]);
                second(row[i],nullptr);
            }
        }
    ...
    while (true)
    {
        ...
        if (!EnableCrossEdges && p_space.m_nativeHooks && p_space.m_nativeHooks->navix)
            expandNavix(currentLocal,node,localEdgeCount);
        else
            expandEdges(currentLocal,currentNode,currentIndex,node,0,localEdgeCount,false);
    ...
    int maxCheck, bool searchDeleted) const
{
    ...
    hooks.filteredAdmission = bool(resultFilter) || bool(traversalFilter) || bool(hooks.ownPoint) || bool(hooks.ownGain);
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.navix) {
        const auto status = maxCheck > 0
            ? SearchIndexWithMaxCheck(query, maxCheck, searchDeleted)
            : SearchIndex(query, searchDeleted);
        hooks.checked = query.GetScanned();
        return status;
    }

...
def validate(directory,scenario,mode,count,seconds):
    ...
    if scenario!="unfilter":
        require(np.all(o[:,[2,3,4,6,7]]==0),"Unexpected eager native predicate/own admission")
        require(np.array_equal(np.fromfile(str(prefix)+".own.i32",dtype="<i4"),np.zeros(count)),
                "Supplemental own collector active")
        require(np.all(n[:,15]==0) and np.all(n[:,16]==1),"Original distance-only head admission not restored")
    record["baseline_identity"]="authenticated pre-supplier default H1; NOT predecessor result-filtered experimental graph"
...
def original_replays():
    original=TOOL/"harness/original-reference"
    ...
    for scenario in ("broad_tag","extreme_tag","unfilter"):
        ...
        require(not directory.exists(),"Original reference already executed")
        directory.mkdir()
        seconds=process(["numactl","--cpunodebind=2","--membind=2",str(original),
                         "--config",str(cfg)],directory,max_seconds=180)
        current=invoke(f"small_{scenario}_{case('graph')}",scenario,"graph",True)
        expected=hashes(directory/"nprobe_24",PAYLOADS)
        require(current["payloads"]==expected,"Authenticated pre-supplier full-query output/work mismatch")
        records.append({"scenario":scenario,"queries":32,"payloads":expected,"config_sha256":sha(cfg),
            "reference_binary_sha256":sha(original),"process_seconds":seconds,
            "ids_distances_ssd_scanned_parity":True,
            "raw_visited_trace":"original binary lacks raw visited trace; same-core fixtures cover that trace"})

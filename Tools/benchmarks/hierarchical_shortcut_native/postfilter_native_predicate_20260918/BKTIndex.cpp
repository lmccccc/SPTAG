                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    const auto auxiliaryAdmissible = [&](SizeType id) {
        ...
        return hook && hook->auxiliaryEligible && hook->auxiliaryEligible(id);
    };
    ...
    int maxCheck, bool searchDeleted) const
{
    ...
    if (hooks.matchEnabled) {
        if (!hooks.immutableSnapshot || !hooks.auxiliaryEligible || resultFilter || traversalFilter)
        ...
        const auto matchPhysical=[&](SizeType id) {
            ...
            return hooks.auxiliaryEligible(id);
        };

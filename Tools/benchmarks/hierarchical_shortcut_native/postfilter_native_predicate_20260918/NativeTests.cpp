...
int main() {
    try {
        ...
        const auto observed=[](bool match=true) {
            ...
            h.defaultAdmission=true;h.matchEnabled=h.immutableSnapshot=match;return h;
        };
        ...
        const auto run=[&](Hooks& h,const std::function<bool(int)>& predicate,int budget=2048,int k=256) {
            ...
            if (h.matchEnabled) h.auxiliaryEligible=predicate;
            CHECK(index.SearchIndexWithNativeHooks(r,h,{},{},budget)==ErrorCode::Success);
            h.auxiliaryEligible={};
            return values(r);
        };
        const auto parity=[](const Hooks& a,const Hooks& b) {
            ...
            CHECK(a.nativePredicateCalls==0 && b.nativePredicateCalls==0);
        };
        ...
        for (int budget:{1,8,64,2048}) for (int k:{1,24,count}) {
            ...
            CHECK(index.SearchIndexWithMaxCheck(native,budget)==ErrorCode::Success);
        ...
        for (int wanted:{bridge,target}) {
            ...
            for (auto p:calls) CHECK(p.second==1);

# Native H1 reuse v2, bounded 20260917
...
V2 corrects one native admission defect exposed by the dedicated collapsed
own-only fixture: the alias was marked by `CheckResultAndSet` before entering
`admitFilteredResult`, which immediately checked it again. Result admission now
owns that single dedup operation. The first measured native-reuse runtime/results
remain preserved and explicitly superseded; they are not spliced into v2.
...
`prepare_v2.py` authenticates the first native-reuse runtime and creates a separate source/runtime.
...
`integrate.py` records the native transformations, including the alias correction.
Copied v1 bootstrap helpers are not v2 execution entrypoints.
...
Canonical evidence:
`datasets/sift1m_zipf200_sparse193_numeric/comparisons/h1_native_reuse_20260917_v2/`.

#!/usr/bin/env python3
"""Online eight-owner navigation versus matched evaluated-candidate admission controls."""
import argparse
import json
...
def run(config, resume=False):
    ...
    require(count == warmup == 1000 and repeats == 2, "Unexpected fixed online protocol")
    ...
    for name in ("authentication.json", "integration.json", "scenario_integration.json",
                 "online_integration.json", "configure.log", "build.log", "online-tests.log"):
        if not resume:
    ...
    def execute(scenario, case, mode, repeat, frozen=False):
        ...
        if frozen:
            ...
            payload = [{"heads": r["heads"], "results": r["results"],
                        ...
                        **{k: v for k, v in r.items() if k.startswith("online")}} for r in data]
            if key in payloads:
            ...
            if mode == "profile":
                ...
                if case.startswith(("online", "admit")):
                    require(all(r["calls"] == r["onlineGraph"] + r["onlineParent"] + r["onlineChild"]
                                for r in data), "Unified online callback sum mismatch")
                    if case.startswith("admit"):
                        require(all(r["onlineParent"] == r["onlineChild"] == r["csr_assignments"] == 0
    ...
    try:
        ...
        for scenario in ([] if resume and (output / "native_fixtures.json").exists() else scenarios):
            for case in (c for c in cases if c not in ("h1", "h3")):
                ...
                for mode in ("plain", "profile"):
                    ...
                    payload = [{k: row[k] for k in ("heads", "own_ids", "results", "onlineTrace")}
                               for row in data]
        ...
        for scenario in scenarios:
            for case in cases:
                ...
                summary.append({"scenario": scenario, "case": case, **quality[scenario, case],
                    ...
                    "online_work": {k: statistics.mean(q[k] for q in data)
                                    for k in data[0] if k.startswith("online") and k != "onlineTrace"},
        ...
        for case in (c for c in cases if c != "h3"):
            ...
            for scenario in scenarios:
                ...
                keys = (("heads", "calls", "adjacency", "exhausted") if case == "h1" else
                        ("onlineTrace", "calls", "adjacency", "exhausted", "onlineGraph",
                         "onlineParent", "onlineChild", "onlineCandidates", "onlineCache",
                         "onlineUpH1", "onlineUpH2", "onlineRowsH2", "onlineRowsH3"))
                require(all(all(a[k] == b[k] for k in keys) for a, b in zip(reference, data)),
                        "Predicate changed online/control spatial navigation or budget")
        require(all(any(q["onlineRowsH3"] > 0 for q in rows(
            result_dir("unfilter", case, "profile", 1) / "queries.jsonl"))
            for case in cases if case.startswith("online")), "H3 owners were never expanded")
        ...
        write_json(output / "validation.json", {"original_h1_h3_heads_and_native_work_equal": True,
            ...
            "h1_and_online_control_spatial_traces_predicate_invariant": True,

...
bool ApplySearchIni(TenantIndexManager& p_manager, const std::string& p_path)
{
    ...
    for (const auto& parameter : parameters) {
        if (parameter.first.rfind("arbitration", 0) == 0) continue;
...
#ifdef MATCHED_CURRENT
    if (out.caseName != "auto" && out.caseName != "graph" && out.caseName != "estimate_only")
        throw std::runtime_error("Current core cannot masquerade as original");
    ShortcutFull::Configure(params);
    if (ShortcutFull::profile || ShortcutFull::capture ||
        ShortcutFull::mode != out.caseName)
...
int Run(int argc, char** argv)
{
    ...
    for (int nprobe : options.probes) {
        ...
        for (const Scenario& scenario : scenarios) {
            ...
            std::vector<std::array<std::uint64_t, 14>> arbitrationWork;
            std::vector<std::array<double,44>> decisions;
            ...
            for (std::size_t i = 0; i < measuredQueries; ++i) {
                ...
                if (options.caseName != "h3") {
                    ...
                    if (!observation.invoked || !observation.native.originalDistanceFunction)
                    ...
                    arbitrationWork.push_back({n.estimates,n.graphDecisions,n.postingDecisions,n.discoveryDecisions,
                    ...
                    for (const auto& d:n.decisions) {
                        std::array<double,44> row{};
                        std::size_t at=0;
                        const auto put=[&](auto value){row[at++]=static_cast<double>(value);};
                        const auto prediction=[&](const auto& p){
                            put(p.cost);put(p.gain);put(p.novelty);put(p.usefulGivenFresh);put(p.length);
                        };
                        const auto work=[&](const auto& w){
                            put(w.members);put(w.fresh);put(w.predicateChecks);put(w.predicatePasses);put(w.useful);
                        };
                        put(i);put(d.head);put(d.level);put(d.row);put(d.chosen);
                        put(d.checkedBefore);put(d.checkedAfter);put(d.remaining);put(d.remainingChecks);
                        prediction(d.graph);prediction(d.posting);put(d.childRow);prediction(d.tailPosting);
                        work(d.previousGraph);work(d.realized);put(d.descriptorMembers);put(d.representativeDistances);
                        put(d.source);put(d.selectedDistance);put(d.postingSetup);put(d.tailSetup);
                        put(d.tailDistance);put(d.retainedCount);put(d.retainedChosen);
                    ...
                    if (options.scenarioName == "unfilter" &&
                        (!n.defaultAdmission || n.filteredAdmission ||
                         n.calls || n.parentDistances || n.childDistances || n.members))
                        throw std::runtime_error("Actual unfilter did not use native default admission");
                }
#endif
            }
#ifdef MATCHED_CURRENT
            ...
            save(".own.i32", ownIDs);
            save(".arbitration.u64", arbitrationWork);

                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    bool finishInjectedRow = false;
        const auto expandEdges =
            [&](SizeType currentLocal, int currentNode, const Index<T>* currentIndex,
                ...
                DimensionType p_end,
                bool p_crossEncoded,
                CostArbitration::Work* work=nullptr)
        {
    ...
    const auto arbitrate = [&](SizeType head,int nodeID,const Index<T>* index,
                              const CostArbitration::Work& previous,int width) {
        auto* hook = p_space.m_nativeHooks;
        if (!hook || !hook->afterGraph) return;
        auto consume=[&](const std::uint32_t* row,int count) {
                finishInjectedRow = true;
                hook->injected = true;
                COMMON::PrefetchGraphNeighbors(row,count,[index](SizeType id)->const void* {
                    return id>=0 && id<index->m_pSamples.R()?index->m_pSamples[id]:nullptr;
                });
                CostArbitration::Work work;
                expandEdges(head,nodeID,index,row,0,count,false,&work);
                hook->injected = false;
        ...
        auto remaining=[&] {
            int missing=0;
            for (int i=0;i<p_query.GetResultNum();++i) missing+=p_query.GetResult(i)->VID<0;
            return (std::max)(1,missing);
        };
        auto remainingChecks=[&] {
            return (std::max)(0,p_space.m_iMaxCheck-p_space.m_iNumberOfCheckedLeaves);
        };
        COMMON::NativeEdgeConsumer native{consume,within,checked,remaining,remainingChecks};
        hook->afterGraph(head,previous,width,native);
    };
    ...
    while (true)
    {
        ...
        CostArbitration::Work graphWork;
        auto* graphObservation=!EnableCrossEdges && p_space.m_nativeHooks &&
            p_space.m_nativeHooks->afterGraph?&graphWork:nullptr;
        expandEdges(currentLocal,currentNode,currentIndex,node,0,localEdgeCount,false,graphObservation);
        if constexpr (!EnableCrossEdges)
            arbitrate(currentLocal,currentNode,currentIndex,graphWork,localEdgeCount);
    ...
    int maxCheck, bool searchDeleted) const
{
    ...
    if (!hooks.diagnostics && !hooks.filteredAdmission && !hooks.afterGraph) {

...
import time
import numpy as np
from prepare import HERE, DATA, TOOL, OUTPUT, BASE, MATCHED, V1, sha, write
...
def protect():
    ...
    for name,digest in parent["base_source"].items():
        require(sha(BASE/"source"/name)==digest,"Frozen v1 source changed")
    for name,digest in parent["v1_runtime"]["libraries"].items():
        require(sha(BASE/"source/Release"/name)==digest,"Frozen v1 library changed")
    require(sha(parent["v1_runtime"]["binary"])==parent["v1_runtime"]["binary_sha256"],"Frozen v1 binary changed")
    for name in ("report","summary"):
        require(sha(V1/(name+".json"))==parent["v1_"+name+"_sha256"],"Frozen v1 result changed")
    for path,digest in json.loads((OUTPUT/"production_protection.json").read_text()).items():
        require(sha(path)==digest,"Protected production file changed")
    for core in ("original","current"):
        require(sha(TOOL/f"matched-{core}")==sha(MATCHED/"snapshot"/f"matched-{core}"),
                "Authenticated control executable changed")
    return reg["index_fingerprint"]


def controls():
    # Configs are byte-preserved from v1, including all bounded controls.
    if (HERE/"configs/fixture_extreme_tag_graph.ini").exists(): return
...
def invoke(label,scenario,case,config,diagnostic=False):
    ...
    if case not in ("original","h3"):
        ...
        decisions=np.fromfile(str(prefix)+".decisions.f64",dtype="<f8").reshape(-1,44)
        ...
        require(np.all(graph_selected[:,30:36]==0),"Graph-selected decision expanded CSR")
        if scenario=="unfilter":
        ...
        if case in ("graph","estimate_only"):
            require(np.all(native[:,[2,4,5,7,10,11]]==0),"Graph control did posting expansion")
        if case=="graph": require(np.all(native[:,6:12]==0),"Graph control did estimator work")

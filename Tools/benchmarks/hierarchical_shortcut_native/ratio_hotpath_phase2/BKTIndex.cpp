                      ...
                      const std::function<bool(SizeType)>& p_traversalFilter) const
{
    ...
    const auto finishSearch = [&]() {
        ...
        const auto expandEdges =
            [&](SizeType currentLocal, int currentNode,
    ...
    while (true)
    {
        ...
        if constexpr (!EnableCrossEdges) {
            if (ShortcutBench::active && ShortcutBench::active->edges) {
                ...
                expandEdges(currentLocal, currentNode, currentIndex, supplierFresh, supplierEffective, neighbors, 0, n, false);
            } else {
                expandEdges(currentLocal, currentNode, currentIndex, supplierFresh, supplierEffective, node, 0, localEdgeCount, false);
            }
        } else {
            expandEdges(currentLocal, currentNode, currentIndex, supplierFresh, supplierEffective, node, 0, localEdgeCount, false);
        }
        ...
        if (auto* c = H1Supplier::Active()) {
            {
                c->engine->Supply(currentLocal, supplierFresh, connectivity->audit.before, connectivity->audit.physical, [&](int id) {
                    const bool connection = connectivity->AddSupplied(id);
                    const int before = supplierFresh;
                    const int effectiveBefore = supplierEffective;
                    expandEdges(currentLocal, currentNode, currentIndex, supplierFresh, supplierEffective, &id, 0, 1, false);
                    return (supplierFresh != before ? 1 : 0) |
                           (supplierEffective != effectiveBefore ? 2 : 0) | (connection ? 4 : 0);
                }, [&]() { return p_space.m_iNumberOfCheckedLeaves < p_space.m_iMaxCheck; });
            }
        }
        expandEdges(currentLocal, currentNode, currentIndex, supplierFresh, supplierEffective, 

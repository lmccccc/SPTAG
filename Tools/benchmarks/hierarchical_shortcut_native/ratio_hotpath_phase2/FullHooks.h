...
struct Observation {
    std::uint64_t supplyIntrinsicNs = 0, supplyIntrinsicBytes = 0, supplyPrepareNs = 0, supplySearchNs = 0, supplyFinishNs = 0;
...
void Configure(const Parameters& parameters) {
    ...
    for (const auto& p : parameters)
        if (p.first.rfind("shortcut", 0) == 0 &&
            ...
            p.first != "shortcuthotpath" && p.first != "shortcutintrinsiccache")

"""Execute only the preregistered focused ratio Phase2; no full-sweep entrypoint."""
import argparse
import csv
import hashlib
...
def audit(path, count, probe, case, mask, truth):
    ...
    for q, line in enumerate(old.query_stream(path)):
        ...
        old.digest_update(payload, {k: v for k, v in row.items()
                                   if k not in ("calls", "adjacency", "supplyTrace", "supplyIntrinsicBytes") and not k.endswith("Ns")})
...
class Phase2:
    def __init__(self):
        ...
        require(self.prereg["phase"] == "2_only_no_full_sweep", "Wrong experiment scope")
    ...
    def fixtures(self):
        ...
        write(self.root / "fixture_validation.json", {"native_processes": 6, "ratio_semantics_valid": True, "exact_phase1_payload_and_core_work_parity": True,
    ...
    def measure(self):
        ...
        write(self.root / "status.json", {"state": "completed_phase2", "full_sweep_allowed": False})

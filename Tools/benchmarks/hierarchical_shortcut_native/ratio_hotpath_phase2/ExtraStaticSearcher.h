...
#define ProcessPostingOffset() \
        ...
        public:
                               ...
                               int p_numTagsPerVec) override
            {
                ...
                m_staticBuildTagsView = nullptr;
                m_staticBuildTagCount = 0;
                ...
                int p_numTagsPerVec) override
            {
                ...
                m_staticBuildTagCount = 0;
                m_staticBuildNumTagsPerVec = 0;
                if (p_tags == nullptr || p_numVectors <= 0 ||
                    p_numTagsPerVec <= 0) {
                    return;
                }
                m_staticBuildTagsView = p_tags;
                m_staticBuildTagCount =
                    static_cast<size_t>(p_numVectors) *
                    static_cast<size_t>(p_numTagsPerVec);
                m_staticBuildNumTagsPerVec = p_numTagsPerVec;
            }

            const uint32_t* StaticBuildTagsData() const
            {
                return m_staticBuildTagsView != nullptr
                           ? m_staticBuildTagsView
                           : m_staticBuildTags.data();
            }

            size_t StaticBuildTagsSize() const
            {
                return m_staticBuildTagsView != nullptr
                           ? m_staticBuildTagCount
                    ...
                    const auto* headAttributes =
                        ...
                        static_cast<size_t>(
                            ...
                            static_cast<size_t>(
                                m_staticNumTagsPerVec);
                    for (int record = 0;
                         record < postingCount; ++record) {
                        const SizeType vectorID =
                           begin[recordStart + record].tonode;
                        if (vectorID < 0 ||
            ...
            bool BuildCompactOriginalSelections(
                Selection& p_selections,
                std::vector<std::atomic<std::uint8_t>>&
                    p_replicaCount,
                std::vector<std::atomic_int>&
                    p_postingListSize,
                const std::unordered_map<SizeType, SizeType>&
                    p_headVectorIDs,
                std::shared_ptr<VectorSet> p_fullVectors,
                ...
                const Options& p_opt,
                std::uint64_t& p_originalChecked)
            {
                ...
                if (p_fullVectors == nullptr ||
                    ...
                    p_fullVectors->Count() != p_fullCount ||
                    p_fullCount <= 0 ||
                    headCount <= 0 ||
                    p_replicaCount.size() !=
                        static_cast<size_t>(p_fullCount) ||
                    p_postingListSize.size() !=
                    ...
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                        "Compact original placement has invalid vectors, "
                        "heads, or RNG parameters.\n");
                    return false;
                }
                ...
                std::atomic<std::uint64_t> rngRejected(0);
                std::atomic<std::uint64_t> originalChecked(0);
                std::vector<std::thread> threads;
                threads.reserve(
                    static_cast<size_t>(threadCount));
                for (int thread = 0;
                     thread < threadCount; ++thread) {
                    threads.emplace_back([&, thread]() {
                        ...
                        while (!failed.load(
                            ...
                            if (vectorID >= p_fullCount)
                                return;

                            const auto head =
                                p_headVectorIDs.find(
                                    vectorID);
                            if (head !=
                                p_headVectorIDs.end()) {
                                if (!p_opt.m_excludehead) {
                            ...
                            selected.clear();
                            for (int rank = 0;
                                 rank < p_candidateCount &&
                                 rank < results.GetResultNum() &&
                                 selected.size() <
                                     static_cast<size_t>(
                                         p_opt
                                             .m_replicaCount);
                ...
                if (failed.load()) {
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                        "Compact original RNG placement failed "
                        "(vector=%d status=%d).\n",
                        failedVector.load(),
                        failedStatus.load());
                    return false;
                ...
                for (const auto& emitted :
                    ...
                    if (emitted.size() >
                        ...
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Compact original selection count "
                            "overflow.\n");
                        return false;
                    }
                    selectionCount += emitted.size();
                }
                p_selections.m_selections.clear();
                p_selections.m_selections.reserve(
                    selectionCount);
                for (auto& emitted : threadSelections) {
                    p_selections.m_selections.insert(
                        p_selections.m_selections.end(),
                        emitted.begin(), emitted.end());
                    std::vector<Edge>().swap(emitted);
                }
                p_selections.m_start = 0;
                p_selections.m_end = selectionCount;
                p_selections.m_totalsize =
                    selectionCount;
                SPTAGLIB_LOG(
                    Helper::LogLevel::LL_Info,
                    "Compact original placement retained %zu "
                    "RNG edges (rejected=%llu) instead of "
                    "initializing %zu slots (checked=%llu).\n",
                    selectionCount,
                    static_cast<unsigned long long>(
                        rngRejected.load()),
                    static_cast<size_t>(p_fullCount) *
                ...
                LimitedTagSupport& p_support)
            {
                const SizeType headCount =
                    p_headIndex == nullptr
                        ...
                        : p_headIndex->GetNumSamples();
                const auto keyTagAt =
                    [this, &p_opt](SizeType p_vectorID) {
                        return StaticBuildTagsData()[
                            static_cast<size_t>(p_vectorID) *
                                static_cast<size_t>(
                                    m_staticNumTagsPerVec) +
                            static_cast<size_t>(
                                p_opt.m_limitedTagColumn)];
                    };
                if (p_fullVectors == nullptr ||
                    p_headIndex == nullptr ||
                    p_headIndex->m_pQuantizer != nullptr ||
                    ...
                    !m_staticHasMetadata ||
                    m_staticNumTagsPerVec <= 0 ||
                    p_opt.m_limitedTagColumn < 0 ||
                    !p_opt.Schema().IsCategorical(p_opt.m_limitedTagColumn) ||
                    p_opt.m_limitedTagColumn >=
                        m_staticNumTagsPerVec ||
                    StaticBuildTagsSize() !=
                        static_cast<size_t>(p_fullCount) *
                ...
                std::vector<SizeType> headSourceVectorIDs(
                    static_cast<size_t>(headCount),
                    MaxSize);
                for (const auto& head :
                     p_headVectorIDs) {
                ...
                if (!SelectRetainedOriginalBaseTags(
                        p_support, headOwnTags, p_originalPosting.m_selections,
                        p_originalPostingSizes, p_fullCount, keyTagAt, &supportError)) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                        "Cannot select retained-O base support: %s\n", supportError.c_str());
                    return false;
                }
                for (SizeType head = 0;
                     head < headCount; ++head) {
                ...
                if (!p_support.SetTagVectorCounts(
                        static_cast<std::uint64_t>(
                            p_fullCount),
                        tagVectorCounts)) {
                    SPTAGLIB_LOG(
                ...
                if (p_opt.m_enableLimitedTagSupportExpansion)
                {
                    ...
                    if (!expansion.Initialize(
                            p_support, observedTags, p_opt.m_limitedTagMinHeadCount, &supportError))
                    {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                    ...
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                        ...
                        "sourceCappedTags=%llu retainedOAssignments=%llu.\n",
                        p_opt.m_limitedTagMinHeadCount,
                ...
                std::vector<std::atomic_int> postingListSize(
                    static_cast<size_t>(headCount));
                std::vector<std::uint8_t> retainedReplicas(
                    static_cast<size_t>(p_fullCount), 0);
                const int threadCount =
                    (std::max)(
                        1, p_opt.m_iSSDNumberOfThreads);
                // Keep only emitted RNG edges. The previous fixed
                // N*ReplicaCount array initialized tens of gigabytes of
                // unused slots at billion scale.
                std::vector<std::vector<Edge>>
                    threadSelections(
                        static_cast<size_t>(threadCount));
                for (auto& size : postingListSize) size = 0;
                ...
                for (const auto& emitted :
                    ...
                    if (emitted.size() >
                        ...
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Limited-tag selection count overflow.\n");
                        return false;
                    }
                    selectionCount += emitted.size();
                }
                ...
                limitedSelections.m_selections.reserve(
                    selectionCount);
                for (auto& emitted : threadSelections) {
                    limitedSelections.m_selections.insert(
                        limitedSelections.m_selections.end(),
                        emitted.begin(), emitted.end());
                    std::vector<Edge>().swap(emitted);
                }
                limitedSelections.m_totalsize =
                    selectionCount;
                limitedSelections.m_start = 0;
                limitedSelections.m_end = selectionCount;

                VectorIndex::SortSelections(
                    &limitedSelections.m_selections);
                // Count the normal H prefixes before compaction destroys the
                // discarded original RNG edges. Only zero-H vectors need state.
                size_t prefixRead = 0;
                for (SizeType head = 0; head < headCount; ++head) {
                    const size_t begin = prefixRead;
                    while (prefixRead < selectionCount &&
                           limitedSelections.m_selections[prefixRead].node == head)
                        ++prefixRead;
                    const size_t kept = (std::min)(
                        prefixRead - begin, static_cast<size_t>(p_postingSizeLimit));
                    for (size_t row = begin; row < begin + kept; ++row) {
                        const auto vid = limitedSelections.m_selections[row].tonode;
                        if (vid < 0 || vid >= p_fullCount ||
                            retainedReplicas[static_cast<size_t>(vid)] >= p_opt.m_replicaCount) {
                            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                "Invalid or excess original H replica before rescue.\n");
                            return false;
                        }
                        ++retainedReplicas[static_cast<size_t>(vid)];
                    }
                }
                std::uint64_t zeroHBefore = 0;
                for (SizeType vid = 0; vid < p_fullCount; ++vid)
                    zeroHBefore += p_headVectorIDs.count(vid) == 0 &&
                        retainedReplicas[static_cast<size_t>(vid)] == 0;
                std::unordered_map<SizeType, Edge> bestDiscarded;
                if (zeroHBefore != 0) {
                    for (const auto& edge : limitedSelections.m_selections) {
                        if (edge.tonode < 0 || edge.tonode >= p_fullCount ||
                            edge.node < 0 || edge.node >= headCount) {
                            SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                                "Invalid original H edge before rescue.\n");
                            return false;
                        }
                        if (retainedReplicas[static_cast<size_t>(edge.tonode)] != 0)
                            continue;
                        const auto found = bestDiscarded.emplace(edge.tonode, edge);
                        if (!found.second &&
                            (edge.distance < found.first->second.distance ||
                             (edge.distance == found.first->second.distance &&
                              edge.node < found.first->second.node)))
                            found.first->second = edge;
                    }
                    if (bestDiscarded.size() != zeroHBefore) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                            "H rescue source inconsistency: zeroH=%llu originalCandidates=%zu "
                            "despite successful pre-cut placement.\n",
                            static_cast<unsigned long long>(zeroHBefore), bestDiscarded.size());
                        return false;
                    }
                }
                std::vector<Edge> rescueEdges;
                rescueEdges.reserve(bestDiscarded.size());
                for (const auto& entry : bestDiscarded) rescueEdges.push_back(entry.second);
                bestDiscarded.clear();
                bestDiscarded.rehash(0);
                VectorIndex::SortSelections(&rescueEdges);
                std::fill(retainedReplicas.begin(), retainedReplicas.end(), 0);
                size_t rescueRead = 0;
                std::uint64_t rescuedPostings = 0, overflowPages = 0;
                size_t read = 0;
                size_t write = 0;
                std::uint64_t assignmentCount = 0;
                ...
                for (SizeType head = 0;
                    ...
                    const int kept = (std::min)(available, p_postingSizeLimit);
                    const size_t rescueBegin = rescueRead;
                    while (rescueRead < rescueEdges.size() && rescueEdges[rescueRead].node == head)
                        ++rescueRead;
                    const size_t rescued = rescueRead - rescueBegin;
                    if (rescued > static_cast<size_t>(available - kept)) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Error,
                            "H rescue exceeds original discarded posting %d.\n", head);
                        return false;
                    }
                    const int finalKept = kept + static_cast<int>(rescued);
                    postingListSize[
                        static_cast<size_t>(head)] = finalKept;
                    assignmentCount +=
                        static_cast<std::uint64_t>(finalKept);
                    if (rescued != 0) {
                        ++rescuedPostings;
                        const auto pages = [this](int records) {
                            return (static_cast<std::uint64_t>(records) *
                                static_cast<std::uint64_t>(m_vectorInfoSize) + PageSize - 1) / PageSize;
                        };
                        overflowPages += pages(finalKept) - pages(kept);
                    }
                    std::unordered_set<std::uint32_t>
                        headTags;
                    for (int record = 0;
                         record < finalKept; ++record) {
                        const Edge edge =
                            record >= kept ? rescueEdges[rescueBegin + static_cast<size_t>(record - kept)] :
                            limitedSelections
                                .m_selections[
                                    begin +
                                    static_cast<size_t>(
                        ...
                        auto& retained =
                            retainedReplicas[
                                static_cast<size_t>(
                                    edge.tonode)];
                        if (retained >= p_opt.m_replicaCount) {
                            SPTAGLIB_LOG(
                                Helper::LogLevel::LL_Error,
                                "Limited-tag serialization emitted an unexpected extra replica for vector %d.\n",
                                edge.tonode);
                            return false;
                        }
                        ++retained;
                        limitedSelections.m_selections[write++] = edge;
                    }
                    if (headTags.size() >
                        static_cast<size_t>(
                            p_opt
                    ...
                    const unsigned retained =
                        static_cast<unsigned>(
                            retainedReplicas[
                                static_cast<size_t>(
                                    vectorID)]);
                    zeroHAfter += retained == 0;
                }
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info,
                    "H tail rescue: zeroHBefore=%llu zeroHAfter=%llu rescuedVectors=%zu "
                    "rescuedRecords=%zu rescuedPostings=%llu overflowPages=%llu "
                    "(original RNG edges only; H cut=%d records; O unchanged).\n",
                    static_cast<unsigned long long>(zeroHBefore),
                    static_cast<unsigned long long>(zeroHAfter),
                    rescueEdges.size(), rescueEdges.size(),
                    static_cast<unsigned long long>(rescuedPostings),
                    static_cast<unsigned long long>(overflowPages), p_postingSizeLimit);
                limitedSelections.m_selections.resize(write);
                ...
                SPTAGLIB_LOG(
                    ...
                    "retained %llu of %llu RNG assignments after native posting cut "
                    "(originalChecks=%llu placementChecks=%llu "
                    "exactFallbacks=%llu/%llu; baseSource=retainedO).\n",
                    observedTags.size(),
                    static_cast<int>(headCount),
            ...
            virtual bool LoadIndex(Options& p_opt, COMMON::VersionLabel& p_versionMap, COMMON::Dataset<std::uint64_t>& p_vectorTranslateMap,  std::shared_ptr<VectorIndex> m_index) {
                ...
                if (constrainedPurePostings) {
                    ...
                    for (const auto& list :
                        ...
                        int tailListPages = 0;
                        if (tailCount > 0) {
                            const size_t tailBeginBytes =
                                static_cast<size_t>(
                                    list.pageOffset) +
                                pureListBytes;
                            const size_t tailEndBytes =
                                static_cast<size_t>(
                                    list.pageOffset) +
                                static_cast<size_t>(
                                    list.listEleCount) *
                    ...
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Info,
                        "Loaded one primary constrained H|O posting: pure "
                        "avg records %.2f, avg pages %.2f, avg bytes %.2f; "
                        "global tail avg records %.2f, avg pages %.2f, "
                        "avg bytes %.2f; max pure pages %d.\n",
                        m_hybridAvgRecordsPerList,
                        m_hybridAvgPagesPerList,
                        m_hybridAvgBytesPerList,
                        m_avgRecordsPerList,
                ...
                int p_pureCount = -1)
            {
                ...
                for (int i = 0; i < p_postingListSize; ++i)
                {
                    ...
                    if (m_staticHasMetadata) {
                        ...
                        vectorID.append(
                            reinterpret_cast<const char*>(StaticBuildTagsData() + tagOffset),
                ...
                std::vector<int>& p_pureCountPerHead)
            {
                ...
                // Compact the post-cut array in place so dropped replicas do
                // not require a second full selection allocation.
                size_t read = 0;
                size_t write = 0;
                ...
                p_selections.m_selections.resize(write);
                p_selections.m_start = 0;
                ...
                if (admittedTailCount == 0) {
                    std::vector<std::vector<Edge>>().swap(
                        tailCandidatesByHead);
                    p_selections.m_start = 0;
                    p_selections.m_end =
                        p_selections.m_selections.size();
                    p_selections.m_totalsize =
                    ...
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Info,
                        "Static Phase 4 done: pure=%zu tail=0 "
                        "skippedDuplicate=%zu skippedCapacity=%zu "
                        "trimmed=0 final=%zu cap=%s\n",
                        pureSelectionCount,
                ...
                size_t retainedSelectionCount = 0;
                size_t tailRecordsTrimmed = 0;
                for (SizeType head = 0; head < headCount; ++head) {
                    const int pureCount =
                        p_pureCountPerHead[static_cast<size_t>(head)];
                    const size_t candidateCount =
                        tailCandidatesByHead[static_cast<size_t>(head)].size();
                    if (pureCount < 0 ||
                        candidateCount >
                            static_cast<size_t>(
                        ...
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Static tail posting %d exceeds the supported record count.\n",
                            head);
                        return false;
                    }
                    const int totalCount =
                        pureCount + static_cast<int>(candidateCount);
                    ...
                    if (retainedSelectionCount >
                        (std::numeric_limits<size_t>::max)() -
                            static_cast<size_t>(keepCount)) {
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Static tail selection count overflow.\n");
                        return false;
                    }
                    retainedSelectionCount +=
                        static_cast<size_t>(keepCount);
                }

                // Materialize only the retained prefix for each posting.
                // Building an untrimmed merge first doubled the tail working
                // set for page-capped billion-scale indexes.
                std::vector<Edge> retainedSelections;
                retainedSelections.reserve(retainedSelectionCount);
                size_t pureRead = 0;
                for (SizeType head = 0; head < headCount; ++head) {
                    const size_t pureBegin = pureRead;
                    while (pureRead < pure.size() &&
                           pure[pureRead].node == head) {
                        ++pureRead;
                    }
                    const int pureCount =
                        p_pureCountPerHead[static_cast<size_t>(head)];
                    if (pureRead - pureBegin !=
                        static_cast<size_t>(pureCount)) {
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Static tail posting %d contains %zu/%d pure records.\n",
                            head, pureRead - pureBegin, pureCount);
                        return false;
                    }
                    retainedSelections.insert(
                        retainedSelections.end(),
                        pure.begin() + pureBegin,
                        pure.begin() + pureRead);

                    auto& candidates =
                        tailCandidatesByHead[static_cast<size_t>(head)];
                    const int totalCount =
                        pureCount + static_cast<int>(candidates.size());
                    const int keepCount =
                        p_postingListSize[static_cast<size_t>(head)].load();
                    if (keepCount < pureCount ||
                        keepCount > totalCount) {
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Static tail posting %d has an invalid retained count.\n",
                            head);
                        return false;
                    }
                    retainedSelections.insert(
                        retainedSelections.end(),
                        candidates.begin(),
                        candidates.begin() + (keepCount - pureCount));
                    std::vector<Edge>().swap(candidates);
                }
                std::vector<std::vector<Edge>>().swap(
                    tailCandidatesByHead);
                if (pureRead != pure.size() ||
                    retainedSelections.size() !=
                        retainedSelectionCount) {
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                        "Static tail merge produced an invalid retained selection count.\n");
                    return false;
                }
                ...
                p_selections.m_selections.swap(retainedSelections);
                p_selections.m_start = 0;
                p_selections.m_end = p_selections.m_selections.size();
                p_selections.m_totalsize = p_selections.m_end;
                SPTAGLIB_LOG(
                ...
                const std::vector<int>& p_originalPostingSizes)
            {
                ...
                for (SizeType head = 0;
                    ...
                    std::unordered_set<SizeType> pureVIDs;
                    pureVIDs.reserve(
                        static_cast<size_t>(hybridCount) +
                        static_cast<size_t>(
                    ...
                    for (int record = 0;
                        ...
                        if (edge.tonode < 0 ||
                            !pureVIDs.insert(edge.tonode).second) {
                            SPTAGLIB_LOG(
                                Helper::LogLevel::LL_Error,
                    ...
                    const int originalCount =
                        p_originalPostingSizes[
                            static_cast<size_t>(head)];
                    // Phase 3 applies posting cut by reducing the authoritative
                    // count; its sorted scratch buffer may still contain the
                    // rejected suffix.
                    if (originalRead - originalBegin <
                        static_cast<size_t>(originalCount)) {
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Error,
                            "Original posting %d contains only %zu/%d records.\n",
                    ...
                    suffix.reserve(
                        static_cast<size_t>(originalCount));
                    std::unordered_set<SizeType> originalVIDs;
                    originalVIDs.reserve(
                        static_cast<size_t>(originalCount));
                    for (int record = 0;
                        ...
                        if (edge.tonode < 0 ||
                            ...
                            SPTAGLIB_LOG(
                                Helper::LogLevel::LL_Error,
                                "Original posting %d contains an invalid or duplicate VID.\n",
                                head);
                            return false;
                        }
                        if (pureVIDs.count(
            ...
            bool BuildIndex(std::shared_ptr<Helper::VectorSetReader>& p_reader, std::shared_ptr<VectorIndex> p_headIndex, Options& p_opt, COMMON::VersionLabel& p_versionMap, COMMON::Dataset<std::uint64_t>& p_vectorTranslateMap, SizeType upperBound = -1) {
                ...
                if (useNodeAwareBuild) {
                    vectorOwner.assign(
                        static_cast<size_t>(fullCount), -1);
                ...
                if (p_opt.m_enableLimitedTagPosting &&
                    ...
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                        "Limited-tag static postings require one raw STM1 file, "
                        "a valid categorical LimitedTagColumn, one global BKT "
                        "placement, one build batch, and a support sidecar path.\n");
                    return false;
                }
                ...
                if (p_opt.m_replicaCount <= 0 ||
                    ...
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                        "Static ReplicaCount must be in [1,255], got %d.\n",
                        p_opt.m_replicaCount);
                    return false;
                }
                const bool compactOriginalPlacement =
                    p_opt.m_enableLimitedTagPosting;
                std::uint64_t limitedTagOriginalChecked = 0;
                Selection selections(
                    compactOriginalPlacement
                        ? 0
                        : (useNodeAwareBuild
                               ? plannedAssignmentCount
                               : static_cast<size_t>(
                ...
                if (useNodeAwareBuild) {
                    ...
                    if (compactOriginalPlacement) {
                        auto fullVectors =
                            p_reader->GetVectorSet();
                        if (p_opt.m_distCalcMethod ==
                                DistCalcMethod::Cosine &&
                            !p_reader->IsNormalized()) {
                            fullVectors->Normalize(
                                p_opt
                                    .m_iSSDNumberOfThreads);
                        }
                        if (!BuildCompactOriginalSelections(
                                selections,
                                replicaCount,
                                postingListSize,
                                headVectorIDS,
                ...
                if (p_opt.m_enableHybridDistance ||
                    ...
                    if (p_opt.m_enableHybridDistance) {
                        ...
                        SPTAGLIB_LOG(
                            Helper::LogLevel::LL_Info,
                            "Limited-tag O suffix uses the complete original "
                            "SPANN placement without supplemental tail replicas.\n");
                    }
                    originalPostingSizes.resize(
                ...
                if (!orderedPageStartAttrs.empty()) {
                    ...
                    for (SizeType vid = 0; vid < fullCount; ++vid) {
                        for (size_t attrIndex = 0; attrIndex < orderedPageStartAttrs.size(); ++attrIndex) {
                            ...
                            const std::uint32_t tag =
                                StaticBuildTagsData()[static_cast<size_t>(vid) * m_staticNumTagsPerVec + attr];
        ...
        private:
                                           ...
                                           const char* p_record) const
            {
                ...
                for (int ti = 0; ti < filterTagCols; ++ti) {
                    if (m_opt != nullptr && !m_opt->m_columnTypes.empty() &&
                        !m_opt->Schema().IsCategorical(ti)) continue;
            ...
            static constexpr int kStaticMetadataSelfContainedTailVersion = 3;
            ...
            bool ConfigureStaticMetadataForBuild(const Options& p_opt, size_t p_expectedCount)
            {
                ...
                if (m_staticBuildNumTagsPerVec != p_opt.m_numTagsPerVec ||
                    StaticBuildTagsSize() != expectedTagCount) {
                    SPTAGLIB_LOG(
                        Helper::LogLevel::LL_Error,
                ...
                m_staticNumTagsPerVec = p_opt.m_numTagsPerVec;
                m_staticACLTagCols = !p_opt.m_columnTypes.empty() ? m_staticNumTagsPerVec : p_opt.m_staticACLTagCols > 0
            ...
            int LoadingHeadInfo(const std::string& p_file, std::vector<ListInfo>& p_listInfos)
            {
                ...
                if (fileSize < 0) {
                    throw std::runtime_error(
                        "Cannot determine static posting file size");
                }
                ...
                std::uint64_t serializedMetadataBytes = 0;
                auto readInt =
                    [&ptr, &serializedMetadataBytes](
                        int& value) {
                        if (ptr->ReadBinary(
                                sizeof(value),
                                reinterpret_cast<char*>(
                ...
                if (metadataHeader) {
                    ...
                    if (metadataVersion ==
                        ...
                        metadataVersion ==
                            kStaticMetadataSelfContainedTailVersion) {
                        int generationLo = 0;
                        int generationHi = 0;
                    ...
                    const std::int64_t metadataBytes64 =
                        static_cast<std::int64_t>(
                            sizeof(int)) +
                        static_cast<std::int64_t>(
                            numTagsPerVec) *
                    ...
                    if (m_opt != nullptr &&
                        ...
                        metadataVersion !=
                           kStaticMetadataSelfContainedTailVersion) {
                        throw std::runtime_error(
                           "Constrained pure mode requires self-contained H|O STM1 version 3; rebuild the index");
                    }
                    ...
                    m_staticACLTagCols =
                        m_opt != nullptr && !m_opt->m_columnTypes.empty() ? m_staticNumTagsPerVec :
                    ...
                    const std::int64_t
                        rawRecordBytes64 =
                            static_cast<std::int64_t>(
                                ...
                                sizeof(ValueType) +
                            sizeof(int);
                    if (m_iDataDimension <= 0 ||
                        rawRecordBytes64 >
                            (std::numeric_limits<
                                 int>::max)()) {
                        throw std::runtime_error(
                            "Invalid legacy static record size");
                    }
                ...
                if (m_listCount < 0 ||
                    m_totalDocumentCount < 0 ||
                    m_iDataDimension <= 0 ||
                    m_vectorInfoSize <= 0 ||
                    m_listPageOffset < 0 ||
                    fileBytes % PageSize != 0) {
                    throw std::runtime_error(
                        "Invalid static posting header bounds");
                }
                ...
                if (static_cast<std::uint64_t>(
                    ...
                    serializedMetadataBytes +
                        ...
                        dataOffset) {
                    throw std::runtime_error(
                        "Static posting directory overlaps payload");
                }
                size_t totalListCount = p_listInfos.size();
                if (static_cast<size_t>(m_listCount) >
                    p_listInfos.max_size() -
                        totalListCount) {
                    throw std::runtime_error(
                        "Static posting directory is too large");
                }
                p_listInfos.resize(totalListCount + m_listCount);

                size_t totalListElementCount = 0;
                std::uint64_t maxListEnd =
                    dataOffset;
                std::vector<int> payloadOrder;
                payloadOrder.reserve(
                    static_cast<size_t>(
                        m_listCount));
                ...
                for (int i = 0; i < m_listCount; ++i)
                {
                    ...
                    if (pageNum < 0 ||
                        listInfo->pageOffset >=
                            PageSize ||
                        listInfo->listEleCount < 0) {
                        throw std::runtime_error(
                            "Invalid static posting directory entry");
                    }
                            ...
                            (std::numeric_limits<
                                ...
                                static_cast<
                                    std::uint64_t>(
                                        m_vectorInfoSize)) {
                            throw std::runtime_error(
                                "Static posting record extent overflow");
                        }
                        payloadBytes =
                            static_cast<std::uint64_t>(
                                listInfo
                    ...
                    if (listInfo->listEleCount == 0) {
                        if (payloadBytes != 0 ||
                            listInfo->listPageCount != 0 ||
                            listInfo->pageOffset != 0 ||
                            pageNum != 0) {
                            throw std::runtime_error(
                                "Invalid empty static posting entry");
                        }
                    } else {
                        if (payloadBytes == 0 ||
                            payloadBytes >
                                (std::numeric_limits<
                                    ...
                                    listInfo
                                        ->pageOffset) {
                            throw std::runtime_error(
                                "Invalid static posting payload extent");
                        }
                        const std::uint64_t
                            occupiedBytes =
                                static_cast<
                        ...
                        if (computedPages == 0 ||
                            ...
                            computedPages !=
                                listInfo
                                    ->listPageCount) {
                            throw std::runtime_error(
                                "Static posting page-count mismatch");
                        }
                    }
                    const std::uint64_t
                        relativePageBytes =
                            static_cast<
                                std::uint64_t>(
                                    pageNum) <<
                            PageSizeEx;
                    if (relativePageBytes >
                            (std::numeric_limits<
                                 std::uint64_t>::max)() -
                                dataOffset) {
                        throw std::runtime_error(
                            "Static posting page offset overflow");
                    }
                    listInfo->listOffset =
                        dataOffset +
                        relativePageBytes;
                        ...
                        allocatedBytes >
                            fileBytes -
                                listInfo
                                    ->listOffset) {
                        throw std::runtime_error(
                            "Static posting extent exceeds file");
                    }
                    if (listInfo->listEleCount > 0) {
                        const std::uint64_t
                            payloadBegin =
                        ...
                        if (payloadBegin >
                            ...
                            payloadBytes >
                                fileBytes -
                                    payloadBegin) {
                            throw std::runtime_error(
                                "Static posting payload exceeds file");
                        }
                        payloadOrder.push_back(i);
                    }
                    maxListEnd = (std::max)(
                        maxListEnd,
                        listInfo->listOffset +
                    ...
                    if (static_cast<size_t>(
                        ...
                        throw std::runtime_error(
                            "Static posting element count overflow");
                    }
                    totalListElementCount +=
                        static_cast<size_t>(
                            listInfo
                    ...
                    [&](int left, int right) {
                        const auto& leftInfo =
                            p_listInfos[
                                ...
                                static_cast<size_t>(
                                    left)];
                        const auto& rightInfo =
                            p_listInfos[
                                totalListCount +
                                static_cast<size_t>(
                                    right)];
                        return leftInfo.listOffset +
                                   leftInfo.pageOffset <
                            rightInfo.listOffset +
                                rightInfo.pageOffset;
                    });
                std::uint64_t previousPayloadEnd = 0;
                for (int list : payloadOrder) {
                    const auto& listInfo =
                        p_listInfos[
                            totalListCount +
                            static_cast<size_t>(
                    ...
                    if (payloadBegin <
                        previousPayloadEnd) {
                        throw std::runtime_error(
                            "Static posting payloads overlap");
                    }
                    previousPayloadEnd =
                        payloadBegin +
                        static_cast<std::uint64_t>(
                ...
                if (m_enableDataCompression && m_enableDictTraining)
                {
                    ...
                    if (dictBufferSize >
                        ...
                        serializedMetadataBytes +
                            ...
                            dataOffset) {
                        throw std::runtime_error(
                            "Invalid static compression dictionary size");
                    }
                ...
                if (serializedMetadataBytes >
                        dataOffset ||
                    maxListEnd != fileBytes) {
                    throw std::runtime_error(
                        "Static posting file extent mismatch");
                }
                    ...
                    if (iter == listRestSize.end() || (listInfo.rest != PageSize && iter->rest == 0))
                    {
                        if (currPageNum ==
                            (std::numeric_limits<
                                 int>::max)()) {
                            throw std::runtime_error(
                                "Static posting page number overflow");
                        }
                        ++currPageNum;
                        currOffset = 0;
                    }
                ...
                bool p_manageOrderedPageStarts = true)
            {
                ...
                for (int i = 0; i < p_postingListSizes.size(); ++i)
                {
                    ...
                    {
                        if (p_postPageNum[i] < 0 ||
                            p_postPageOffset[i] >=
                                PageSize) {
                            throw std::runtime_error(
                                "Invalid static posting directory output");
                        }
                            ...
                            pageOffset +
                                ...
                                pageCount *
                                    static_cast<size_t>(
                                        PageSize)) {
                            throw std::runtime_error(
                                "Static posting exceeds on-disk page-count capacity");
                        }
                        listPageCount =
        ...
        private:
            ...
            const uint32_t* m_staticBuildTagsView = nullptr;
            size_t m_staticBuildTagCount = 0;

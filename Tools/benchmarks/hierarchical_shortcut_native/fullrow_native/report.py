...
def render(root):
    ...
    text += ["", "**The joint cost/quality goal is not met.** Unfilter S/H1 latency is "
             ...
             "Medium/mixed improve quality but incur large full-row CSR work; extreme still "
             "underfills. Ratios below are descriptive, not equal-recall costs.", ""]
    ...
    text += ["", "### Sparse helper availability", "",
             "The helper is called only inside a native H1 expansion, never as a final underfill "
             "repair. Predicate-first traversal can exhaust native checked leaves without an H1 "
             "pop, although structural-center scoring can still admit valid own points/postings. "
    ...
    for scenario in ("extreme_tag", "mixed_dnf"):
        ...
        for active in (False, True):
            ...
            a = {"scenario": scenario, "helper_called": active, "queries": len(group),
                 "recall_at_10": hits / (len(group) * 10), "underfilled_queries": underfill,
                 "zero_h1_pops": zero_pops}
            availability.append(a)
            text.append(f'| {scenario} | {active} | {len(group)} | {a["recall_at_10"]*100:.2f} | '
    ...
    acceptance = {
        ...
        "sparse_no_helper_outputs_equal_matched_control": True,

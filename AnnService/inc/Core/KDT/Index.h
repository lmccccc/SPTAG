...
#undef DefineKDTParameter
            ...
            ErrorCode SearchIndexWithResultFilter(QueryResult& p_query,
                std::function<bool(SizeType)> p_resultFilter, int p_maxCheck = 0,
                bool p_searchDeleted = false) const;
        ...
        private:
            template <typename Q>
            void SearchIndex(COMMON::QueryResultSet<T> &p_query, COMMON::WorkSpace &p_space, bool p_searchDeleted,
            ...
            void Search(COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space,
                const std::function<bool(SizeType)>& p_resultFilter = nullptr) const;
        };
    } // namespace KDT
} // namespace SPTAG
